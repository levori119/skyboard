...
app.post('/api/airfield-element-types', async (req, res) => {
  try {
    const { name, color, icon, can_change_status, allowed_statuses, open_icon, close_icon } = req.body;
    const r = await pool.query(
      'INSERT INTO airfield_element_types (name,color,icon,can_change_status,allowed_statuses,open_icon,close_icon) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
      [name, color || '#f59e0b', icon || '🔧', can_change_status === true, JSON.stringify(allowed_statuses || []), open_icon || null, close_icon || null]
    );
    res.json(r.rows[0]);
  } catch (err) { res.status(500).json({ error: 'Failed' }); }
});
app.put('/api/airfield-element-types/:id', async (req, res) => {
  try {
    const { name, color, icon, can_change_status, allowed_statuses, open_icon, close_icon } = req.body;
    const r = await pool.query(
      'UPDATE airfield_element_types SET name=$1,color=$2,icon=$3,can_change_status=$4,allowed_statuses=$5,open_icon=$6,close_icon=$7 WHERE id=$8 RETURNING *',
      [name, color || '#f59e0b', icon || '🔧', can_change_status === true, JSON.stringify(allowed_statuses || []), open_icon || null, close_icon || null, req.params.id]
    );
    res.json(r.rows[0] || {});
  } catch (err) { res.status(500).json({ error: 'Failed' }); }
});
...
app.put('/api/airfield-elements/:id', async (req, res) => {
  try {
    const { element_type_id, name, status, note, x_pct, y_pct, category, display_state, blink_rate, blink_colors, open_icon_key, close_icon_key, rotation, camera_url, relevant_routes, blocking_statuses } = req.body;
    const r = await pool.query(
