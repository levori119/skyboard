...
- **Workstation Contacts (קשרים)**: Per-workstation communication channel registry with fields: מהות (purpose), או"ק (callsign), תדר/עורק (frequency/channel), הערה (note). Admin manages default contacts in the "📡 קשרים" tab in ניהול מבצעי (select workstation → add/edit rows → 💾 save). When a user enters a workstation the defaults load into session-only state — editable freely but reverted to defaults on re-entry. The right sidebar shows a collapsible "📡 קשרים" panel with inline-editable rows plus a "ריכוז" button that opens a floating draggable summary window listing all related workstations (those sharing relevant_sectors or classic_transfer_points) with their frequency lists. DB: `workstation_contacts` table (id SERIAL PK, preset_id FK, mahut, oketz, frequency, note, sort_order). API: GET `/api/workstation-contacts?preset_id=X`, GET `/api/workstation-contacts/all`, POST/PUT/DELETE `/api/workstation-contacts/:id`.
