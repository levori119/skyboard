...
- **GROUND Workstation Mode (מגרש)**: Preset type `ground` with associated `airfield_id`. Three-panel layout: Right=strip list with per-aircraft cards (click to cycle status, draggable), Center=airfield map, Left=transfer sectors. Statuses: `none` (Pre-Call, gray), `taxi` (green), `lineup` (blue), `takeoff` (red, pulsing flash animation). Aircraft chips on map: click to open quick status menu popover; drag to reassign to another point. Density warning: point shows amber pulsing ring + count badge when ≥3 aircraft are at it. Transfer dialog asks single-aircraft or full strip. Admin "שדות תעופה" tab: airfield CRUD + interactive point placement (click on map image), name/color/marker-type per point. Points rendered with `GroundMarkerSVG` (9 marker types). DB tables: `airfields`, `airfield_points` (with `color VARCHAR`, `marker VARCHAR`); strip columns: `aircraft_positions`, `ground_status`; preset columns: `preset_type`, `airfield_id`. **Whole-strip drag**: the strip header in the right panel is itself draggable (cursor: grab, ≡ handle icon, tooltip 'גרור להעברת כל הפמ"מ'). Drag payload uses `{ stripId, all: true }` for whole-strip moves vs `{ stripId, idx }` for single aircraft. Dropping a whole-strip on an airfield point assigns ALL aircraft of the formation to that point in one update; dropping on a transfer sector skips the single-vs-formation dialog and directly transfers the entire strip. **Map merge**: when ALL aircraft of a strip are at the same point AND share the same status, the map renders a single merged "whole-strip" chip showing `CALL ×N` (with thicker border and stronger shadow) instead of N stacked individual chips. The merged chip is itself draggable as a whole-strip, and its quick-status menu changes status for the entire formation. If statuses diverge or only some aircraft are at the point, falls back to per-aircraft chips with stack offset (existing behavior).
