...
- **Classic Strip Display (תצוגת סטריפים קלאסית)**: Alternative workstation display mode replacing the map/table view with 3 equal vertical panels: Right=Receive (📥 ממי מקבל), Center=My Strips (🎯 שלי), Left=Transfer (📤 למי מעביר). Each panel groups strips by configured sector points. Drag strips from Mine→Transfer panel drop zones to initiate transfer; drag from Receive panel→Mine to accept incoming transfer. Strip cards show 3 configurable rows — each row supports multiple fields joined with a configurable separator (e.g. `callSign + sq` joined by ` / `). Row config includes label, font size, color, bold/italic/underline, text align, editability (only single-field rows are editable). Configured per workstation preset (`display_mode='classic'`, `classic_strip_table_id`, `classic_receive_points`, `classic_transfer_points` JSONB columns). Strip templates managed in admin "סטריפים קלאסי" tab. DB tables: `classic_strip_tables`, `classic_strip_rows` (with `fields JSONB` and `separator VARCHAR` columns).
- **Classic Preset Type — Dual Transfer Modes**: Classic-type presets (`preset_type='classic'`) have two distinct transfer configurations in the admin form: (1) **📋 עמדות סטריפים שותפות (העברה ישירה)** — `classic_partner_preset_ids` filtered to ONLY classic presets, used for direct station-to-station transfers without sectors (via `to_preset_id`/`from_preset_id` columns and `/api/strips/:id/transfer-to-preset`). (2) **📍 נקודות העברה לעמדות שאינן סטריפים** — `classic_receive_points`/`classic_transfer_points` (JSONB sector lists) used to bridge classic↔non-classic stations via shared sector points. ClassicView's right and left panels render BOTH partner stations AND sector points side-by-side with section dividers. Server endpoints `/api/presets/:presetId/classic-incoming` and `/api/presets/:presetId/classic-outgoing` return both direct (preset-to-preset) and sector-based transfers (where `to_sector_id IN classic_receive_points` and `to_preset_id IS NULL`, or `from_workstation_id = presetId` and `to_preset_id IS NULL`). Sector JOINs are LEFT JOINs to handle null `from_sector_id` from classic strips. UI filters `!t.from_preset_id`/`!t.to_preset_id` keep direct vs sector transfers separated cleanly. A help "?" button (`ClassicTransferHelpModal` component) explains both modes with text + simple SVG diagram, available in the workstation classic view (floating top-right) and in the admin classic preset form (next to the green section header).
