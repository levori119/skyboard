...
- **GROUND Workstation Mode (מגרש)**: Preset type `ground` with associated `airfield_id`. Three-panel layout: Right=strip list with per-aircraft cards (click to cycle taxi/lineup/takeoff status, draggable), Center=airfield map with aircraft markers at assigned points (draggable to reassign), Left=transfer sector drop zones. Aircraft positions stored as JSONB `aircraft_positions` on strips. Status values: `none|taxi|lineup|takeoff`. Transfer dialog asks single-aircraft or full strip. Admin "שדות תעופה" tab: airfield CRUD + interactive point placement — select airfield, define point name/color/marker-type, press "הנח על מפה" then click on the airfield map image to place the point. Points rendered with `GroundMarkerSVG` (9 marker types: circle/square/diamond/triangle/cross/star/runway/parking/hangar) in the chosen color. DB tables: `airfields`, `airfield_points` (with `color VARCHAR` and `marker VARCHAR` columns); strip columns: `aircraft_positions`, `ground_status`; preset columns: `preset_type`, `airfield_id`.
