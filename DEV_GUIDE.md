...
| **נקודת הצטרפות (STAR)** | נקודת כניסה לשדה שבה מטוסים מצטרפים לתנועה. **דומה לנקודת העברה** (אותו מנגנון העברות) אבל התצוגה שונה: נפרסת ל**טבלת בלוקי גבהים**. שייכת ל**שדה** (`airfield_joining_points`), רלוונטית רק לעמדת שדה |
| **משטח גובה** | בתלת מימד: מישור אופקי מקווקו בגובה של תווית על ציר הגבהים. עצם שיושב עליו נמצא באותו גובה - **בלי קשר למרחקו לעומק**, וזה מה שהופך את הציר לקריא גם בפינת המפה |
| **בלוק גובה (בהצטרפות)** | שורה בטבלת נקודת ההצטרפות. הגובה נשמר **ברגל** ומוצג **במאות** (4000 -> `040`); ההפרש בין בלוקים יכול להשתנות לאורך הטווח |
| **מתואם** | אישור של הפקח ששני פ"ממים באותו בלוק גובה מתואמים ביניהם - מוריד את הבלוק מהאדום ונרשם ביומן הביקורת |
| **שים בהקפה** | הוצאת מטוס מטבלת ההצטרפות אל ההקפה. עד אז הוא מסומן על צלע "עם הרוח" ב**מסגרת מקווקוות**; אחרי כן המסגרת **קבועה**. כל מטוסי הפ"מ בהקפה = הפ"מ נעלם מהנקודה |
| **ירוקים / אישור לנחות / נחיתה** | סטטוס ה**מטוס** (`strip_aircraft.flight_status`) - נשאר נכון גם אחרי שעזב את נקודת ההצטרפות |
...
| **אזור סגור** | אזור מפה שאסור לאייש, ולכן שיוך פ"מ אליו **נחסם** ויוצאת התראה. נקבע בעמדה ב**לחיצה על קו האזור** (התפריט נפתח משם - פנים האזור נשאר יעד שחרור של פ"מ), ותמיד עם **טווח גבהים**: טווח ריק = כל הגבהים. פ"מ שהאזור נסגר **מתחתיו** נשאר בו ומקבל התראת "אויש אזור סגור" - זו הדרך היחידה שנשארה להיות בתוך אזור סגור. ההכרעה ב-[`src/utils/zoneRestriction.ts`](src/utils/zoneRestriction.ts), המצב ב-`map_zone_operational_state.restriction` |
| **אזור מוגבל** | אותו מנגנון, אבל השיוך **מותר** ורק מתריע - וההתראה נוקבת ב**גבהים הפתוחים** ובהערת המגבלה, כלומר מה כן אפשר. ההבדל מ"סגור" הוא מי מכריע: בסגור המערכת מכריעה שלא, ובמוגבל היא מוסרת את ההכרעה לפקח ומוודאת שהוא יודע - ולכן "מוגבל" אינו דרגה חלשה של "סגור" אלא כלי אחר. אל תבלבל עם **מגבלת אזור** (`limitation_note`) - טקסט חופשי שמוצג ליד שם האזור ואינו מצב |
| **אישור בתפריט האזור** | תפריט האזור אינו שומר בכל נגיעה: הכול נאסף בטיוטה ו"✓ אשר" שולח **בקשה אחת**. ה-✕ וה"ביטול" סוגרים בלי להחיל. הכפתור נקרא "ביטול" ולא "סגור" כי "סגור" הוא **מצב האזור** באותו חלון |
| **היקף ההגבלה** | על אילו גבהים ההגבלה חלה. באזור **מפוצל** מסמנים **בלוקים** בתפריט (בחירה מרובה, `restriction_range_ids`); באזור לא מפוצל - טווח מספרי. סימון כל הבלוקים נשמר כרשימה ריקה, שהיא "כל הגבהים" - כדי שלא יהיו שני מצבים שנראים זהים ומתנהגים אחרת |
| **רצועת גובה** | החלוקה האופקית של אזור בתצוגת "⇅ פצל לגבהים" (צפון=גבוה → דרום=נמוך). ה**רצועה** שעליה שוחרר הפ"מ היא ההכרעה על ההגבלה, ולא האזור כולו: רצועה סגורה חוסמת, ורצועה פתוחה באותו אזור ממשיכה לקבל |
| **הלאמת אזור זמני** | מרחב ש**נולד בציור** על המפה, לזמן קצוב, ומופץ לעמדות אחרות. להבדיל מאזור סגור/מוגבל (אזור **קיים** שהעמדה מסמנת לעצמה) - כאן נולדת ישות חדשה שמשותפת לכל העמדות, ולכן היא נשמרת ב**נ"צ** (`polygon_geo`) ומוקרנת בכל עמדה דרך עוגני המפה שלה. **מתריעה ואינה חוסמת**. ההכרעה ב-[`src/utils/tempZoneSeizure.ts`](src/utils/tempZoneSeizure.ts), אפיון ב-[TEMP_ZONE_SEIZURE_SPEC.md](TEMP_ZONE_SEIZURE_SPEC.md) |
...
| **פקד (על סטריפ)** | רכיב שהמנהל מגדיר בתא של תבנית הסטריפ ("משבצת") והפקח משנה בלחיצה: כפתור מחזורי, שדה (מקלדת/כתב יד), דגל, תפריט יחיד או תפריט מרובה. הערך הוא שמשפיע על שאילתות, עיצוב מותנה ומעבר בין עמדות. ראה [CIV_STRIP_CONTROLS.md](CIV_STRIP_CONTROLS.md) |
| **ב"מ** | ברירת המחדל של פקד. פקד בלי ערך שמור מציג אותה; "מתאפס לב"מ" = אין שורת ערך (ולא ערך שנכתב) |
| **היקף פקד** | **פנימי ללוח** - הערך נשמר על (פ"מ, עמדה) ובלוח אחר הפקד מתאפס לב"מ · **גלובלי לפ"מ** - הערך נוסע עם הפ"מ לכל מקום ונחשף כשדה בשאילתות |
| **דסק משימה כללי** | סוג עמדה גנרי לרישום - שירותי אמצעים/טקסט חופשי/טבלה חכמה/תמונה/טקסט קבוע/**מפה**/**פ"ממים** (MissionDeskBody, רץ בתוך SectorDashboard כמו כל עמדה) |
| **חלון מפה בדסק** | שירות `map` בפריסת הדסק. הדסק קובע שיש כאן מפה; *איזו* מפה, אילו נקודות העברה ואילו מפות סקטור - נקבע פר-עמדה ב-`mission_desk_map_config`. לכל חלון אפשר לקשר שירות `strips` = חלון הפ"ממים שלו |
