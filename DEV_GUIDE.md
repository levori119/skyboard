...
| **אזור מוגבל** | אותו מנגנון, אבל השיוך **מותר** ורק מתריע. ההבדל מ"סגור" הוא מי מכריע: בסגור המערכת מכריעה שלא, ובמוגבל היא מוסרת את ההכרעה לפקח ומוודאת שהוא יודע - ולכן "מוגבל" אינו דרגה חלשה של "סגור" אלא כלי אחר. אל תבלבל עם **מגבלת אזור** (`limitation_note`) - טקסט חופשי שמוצג ליד שם האזור ואינו מצב |
