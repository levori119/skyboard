...
| **אזור סגור** | אזור מפה שאסור לאייש. נקבע בעמדה ב**לחיצה על קו האזור** (התפריט נפתח משם - פנים האזור נשאר יעד שחרור של פ"מ), ותמיד עם **טווח גבהים**: טווח ריק = כל הגבהים. שיוך פ"מ לאזור סגור בגובה שההגבלה חלה עליו מוציא התראה "אויש אזור סגור" ואינו נחסם. ההכרעה ב-[`src/utils/zoneRestriction.ts`](src/utils/zoneRestriction.ts), המצב ב-`map_zone_operational_state.restriction` |
| **אזור מוגבל** | אותו מנגנון בדיוק, בדרגה נמוכה יותר: התראה כתומה ולא אדומה. אל תבלבל עם **מגבלת אזור** (`limitation_note`) - טקסט חופשי שמוצג ליד שם האזור ואינו מצב |
