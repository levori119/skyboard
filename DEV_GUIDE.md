...
7. [בדיקת עבודה בנתק](#בדיקת-עבודה-בנתק)
8. [DB - עבודה עם הסכמה](#db--עבודה-עם-הסכמה)
9. [QA - לפני commit](#qa--לפני-commit)
10. [FAQ ובעיות נפוצות](#faq-ובעיות-נפוצות)
11. [סקילים זמינים](#סקילים-זמינים)
...
| `npm run station` | **עמדה אמיתית עם מאגר מקומי** - כך בודקים נתק. ראה §בדיקת עבודה בנתק |
...
> ⚠️ **ב-`npm run dev` אי אפשר לבדוק נתק.** הדפדפן מדבר ישירות עם ה-API דרך
> ה-proxy של Vite: אין שרת עמדה, אין מאגר מקומי, ואין לאן לכתוב. כפתור הנתק
> יגיע שם למצב "צפייה בלבד" - ההעברות ייחסמו, והערה וגובה פשוט לא יישמרו.
> זו אינה תקלה, זו ההגדרה של עמדה בלי מאגר.

כדי לבדוק נתק צריך **עמדה** - כפי שהיא בשדה:
...
```bash
# חלון 1 - השרת המרכזי + Vite
npm run dev

# חלון 2 - העמדה (מרימה גם את המאגר המקומי)
npm run station
```

ואז פותחים את הכתובת ש-`npm run station` מדפיס (**5100**, לא 5000).

| דגל | מה עושה |
|---|---|
| `--port=5101` | עמדה שנייה במקביל |
| `--station=twr-2` | מפתח עמדה - קובע את **גוש המזהים המקומי** (`localIds.js`). שתי עמדות שמנותקות בו-זמנית חייבות מפתחות שונים |
| `--api=<url>` | שרת מרכזי אחר (ברירת מחדל: `127.0.0.1:3001`) |
| `--dist` | להגיש את ה-build במקום את Vite |
| `--db=<path>` | תיקיית המאגר המקומי |

### התרחיש המלא

1. פותחים את העמדה, נכנסים, ומחכים ~45ש' שהמראה תמשוך את מצב השדה.
2. **"נתק את העמדה"** בפקד שבפינה השמאלית התחתונה.
3. עובדים: גוררים פ"מ, מעדכנים גובה והערה, מעבירים לעמדה אחרת. הכל נשמר -
   במאגר המקומי.
4. בעמדה שנייה (או בדפדפן על 5000) רואים שהשדה **ממשיך כרגיל**. זה כל העניין.
5. **"החזר קשר"** - הדחיפה יוצאת, ומה שנעשה בנתק מגיע למרכז. מה ששתי העמדות
   נגעו בו מוכרע לפי "האחרון מנצח", ומופיע ביומן ההכרעות.

### ובעמדה שבשדה?

אותו דבר בדיוק, בשתי צורות:

| | |
|---|---|
| **חלון kiosk** | ברירת המחדל של ההתקנה - העמדה פותחת את עצמה |
| **סוכן עמדה** | `"AGENT": true` ב-config.json של העמדה. אין חלון; הפקח פותח `http://127.0.0.1:5100` בכרום ומקבל את אותו מאגר. זו הדרך היחידה שעמדה שעובדים בה ב-WEB מחזיקה מאגר על המחשב |

**שתי דרכים לפתוח, ואחת מהן פשוטה יותר:**

| איך | מה מקבלים |
|---|---|
| **`http://127.0.0.1:5100`** (מומלץ) | הכל **מאותו מקור**: האפליקציה, ה-API והמאגר המקומי. בלי CORS, בלי הרשאת "גישה לרשת המקומית" בכרום, ובלי העברת זהות בין מקורות |
| הכתובת הרגילה של SKY-KING | הדף **מוצא** לבד את הסוכן ומפנה אליו את ה-`/api`, אבל כרום ידרוש אישור חד-פעמי |

**ובדפדפן?** הפקח פותח את הכתובת הרגילה של SKY-KING, והדף **מוצא** לבד את
הסוכן שעל המחשב (`127.0.0.1:5100`) ומפנה אליו את כל ה-`/api`. אין מה להגדיר
בדפדפן; אם הסוכן רץ - יש מאגר, ואם לא - הפקד אומר זאת ומציין את הכתובת שחיפש.
...
- **אין מאגר מקומי → "צפייה בלבד".** הפקד אומר זאת במפורש. אם רואים את
  המשפט הזה, פותחים את הכתובת הלא נכונה (או שאין סוכן על המחשב).
- **הזהות.** לשרת המקומי סוד חתימה משלו, ולכן אסימון מרכזי נדחה אצלו.
  שרת העמדה מנפיק אסימון מקומי מקביל בכל פעם שהמרכז מקבל את האסימון
  (`authBridge.noteAccepted`) - כולל אחרי רענון דף, שבו אין כניסה חדשה.
  אם **כל** הבקשות בנתק חוזרות 401, זו החוליה שנשברה.
