...
### 20. דסק משימה כללי
**מה:** דסק גנרי לרישום - סוג עמדה חדש (`preset_type='mission_desk'`). האדמין בונה דסק במסך הניהול (פריסת BSP כמו חלון סטריפים) משלושה סוגי שירותים: **מסך ניהול אמצעים** (כפתורים עם מצבים/צבעים/גודל, פעולות מגע גלויות - ➕ יצירה ו-✏️ עריכה, מיקום חופשי בגרירה, טריגר התראה מתפרצת לעמדה אחרת), **טקסט חופשי** (כתב יד/דיו על canvas, שורות הפרדה וכותרת; פלנלית = ניקוי מלא או מחיקה לפי מיקום הסמן), **טבלה חכמה** (עמודות טקסט/מספר/V-X/תפריט, עמודות חישוב, עיצוב מותנה, שורת סיכום), **תמונה** (הדבקת print-screen ב-paste או קובץ בהגדרה - read-only בעמדה), **טקסט קבוע** (טקסט + פונט/גודל/עיצוב שנקבעים בהגדרה - read-only בעמדה). **טבלאות מתצוגה** (משבצת שמציגה טבלאות מתפריט "תצוגה" - אלמנטים בבסיס, ניהול נסיעות, ניהול נהגים, כמויות, הודעות, קונטיינר חלונות; אילו טבלאות נבחר פר-עמדה בהגדרת הדסק, וכמה טבלאות במשבצת = לשוניות). **אמצעים קבועים (📌) ונתוני טבלה** מוגדרים בהגדרת העמדה דרך "פתח דסק להגדרה" - תצוגת הדסק האמיתי במצב הגדרה; קבועים אינם ניתנים למחיקה/עריכה בעמדה. שירות ניתן לשיתוף בין עמדות - שינוי בעמדה אחת מתעדכן בשנייה (fan-out בשרת + polling).
