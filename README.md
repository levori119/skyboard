...
| `MIRAGE_DRIVER_APP_NAME` | לא | שם אפליקציית הנהג במיראז' (ברירת מחדל `SKY-KING DRIVER`). הנהג נכנס ל-`/driver` עם **ת"ז** וסיסמה, ורק מי שיש לו הרשאה לאפליקציה הזו **ולפחות בסיס אחד** שנבחר לו במסך הניהול של המיראז'. `DRIVER_ACCESS_CODE` בוטל |
