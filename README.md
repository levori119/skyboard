...
| `MIRAGE_DRIVER_APP_NAME` | לא | שם אפליקציית הנהג במיראז' (ברירת מחדל `DRIVER`). הנהג נכנס ל-`/driver` עם **ת"ז** וסיסמה, ורק מי שיש לו הרשאה לאפליקציה הזו. `DRIVER_ACCESS_CODE` בוטל |
