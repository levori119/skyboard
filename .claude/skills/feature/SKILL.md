...
> ⛔ **worktree אחד לכל סוכן - לא שניים באותו עץ.** זה כלל, לא המלצה
> (CLAUDE.md §worktree נפרד לכל סוכן). שני סוכנים שכתבו לאותו working tree
> דרסו זה את עבודתו של זה **בשקט** (`git` אינו מתריע - הכתיבה האחרונה מנצחת),
> קימטו זה את הקוד של זה, ושברו זה לזה את ה-`tsc` בעץ המשותף.
> אם אתה לא לבד על הריפו - הקם worktree **לפני** השורה הראשונה של קוד.
...
## שלב 4.5 - לפני קימוט: לוודא שהעץ שלך

```bash
git status --short
```

קובץ שאינו שלך בעץ = **מישהו אחר עובד כאן**. לא לקמט אותו ולא "לנקות" -
לדווח למשתמש. קומיט שסוחב עבודה של סוכן אחר אינו ניתן להפרדה ברמת קובץ,
ואם היא באמצע עבודה הוא גם שובר את הבנייה לכולם.

ואחרי עריכת קובץ - לוודא שהעריכה **על הדיסק** (`grep`) לפני `git add`:
סשן מקביל יכול לדרוס אותה בין העריכה לקימוט, והקומיט יוצא ריק מהשינוי.
