...
import { DockPositionPicker } from '../shared/WindowContainer';
import type { DockPosition } from '../../utils/windowDock';
...
export const ManagementPage = ({ onBack, onBackToOptions, crewMember, mode }: { onBack: () => void; onBackToOptions?: () => void; crewMember?: CrewMember | null; mode?: 'admin' | 'team_lead' }) => {
  ...
  const [stripsSource, setStripsSource] = useState<'all' | 'gapi' | 'local'>('all');
  ...
  // ── מקור הפ"מ: GAPI מול רישום מקומי ───────────────────────────────────────
  // פ"מ שנקלט מהשו"ב החיצוני נושא gapi_id (GAPI-CONTRACT.md §6.1). הסינון
  // והחיפוש חיים באותו memo כדי שהטבלה והספירה יסתמכו על אותה רשימה בדיוק.
  const isGapiStrip = (s: any) => !!s.gapi_id;
  const gapiStripsCount = globalStrips.filter(isGapiStrip).length;
  const filteredGlobalStrips = React.useMemo(() => {
    const q = stripsSearch.trim().toLowerCase();
    return globalStrips.filter(s => {
      if (stripsSource === 'gapi' && !isGapiStrip(s)) return false;
      if (stripsSource === 'local' && isGapiStrip(s)) return false;
      if (!q) return true;
      return ['callSign', 'sq', 'task', 'koteret'].some(f => (s[f] || '').toLowerCase().includes(q));
    });
  }, [globalStrips, stripsSearch, stripsSource]);
  ...
  // תג מקור לשורת פ"מ - מזהה GAPI מלא + גרסה + זמן סנכרון אחרון ב-tooltip
  const stripSourceCell = (s: any) => isGapiStrip(s) ? (
    <span
      title={[`GAPI: ${s.gapi_id}`,
        s.gapi_version != null ? `v${s.gapi_version}` : null,
        s.gapi_synced_at ? `${tr('admin.gapiSyncedAt')} ${formatTakeoffDisplay(s.gapi_synced_at)}` : null,
      ].filter(Boolean).join(' · ')}
      style={{ display: 'inline-block', padding: '2px 8px', borderRadius: '4px', background: '#2e1065', color: '#d8b4fe', border: '1px solid #7c3aed', fontSize: '11px', fontWeight: 'bold', whiteSpace: 'nowrap', cursor: 'help' }}
    >GAPI</span>
  ) : (
  ...
  const [presetForm, setPresetForm] = useState({
    ...
    can_seize_zone: false as boolean,
     ...
     * `airLogicWhenOff` (לוגיקות תמונ"א גם כשאינה מוצגת) דולק - הכרעת הפקח 2026-09-15.
     */
    zone_watch_settings: { alerts: true, airLogicWhenOff: true } as { alerts: boolean; airLogicWhenOff: boolean },
    ...
    show_data_windows: false, show_window_container: false as boolean, window_container_position: 'beforeAids',
  });
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          load_relevant: presetForm.load_relevant,
          ...
          can_seize_zone: (presetForm as any).can_seize_zone === true,
          ...
          zone_watch_settings: (presetForm as any).zone_watch_settings || { alerts: true, airLogicWhenOff: true },
          ...
          show_window_container: (presetForm as any).show_window_container === true,
          window_container_position: (presetForm as any).window_container_position || 'beforeAids',
        })
      });
      if (!res.ok) {
  ...
  const editPreset = (preset: any) => {
    ...
    const f = {
      ...
      load_relevant: preset.load_relevant !== false,
      ...
      can_seize_zone: preset.can_seize_zone === true,
      ...
      // לוגיקות תמונ"א גם כשאינה מוצגת - ברירת מחדל **כן**, גם לעמדה ותיקה (stationLogicWhenOff)
      zone_watch_settings: { alerts: (preset as any).zone_watch_settings?.alerts !== false, airLogicWhenOff: (preset as any).zone_watch_settings?.airLogicWhenOff !== false },
      ...
      show_window_container: preset.show_window_container === true,
      window_container_position: preset.window_container_position || 'beforeAids',
    };
    setPresetForm(f);
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: 'white', direction: 'rtl' }}>
      ...
      <div style={{ padding: '10px 14px 14px', display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
        ...
        <div style={{ width: '220px', flexShrink: 0, background: '#1e293b', borderRadius: '12px', alignSelf: 'flex-start', position: 'sticky', top: '80px', maxHeight: 'calc(100vh - 90px)', overflowY: 'auto', paddingBottom: '8px' }}>
          ...
          {availableTabs.includes('work_groups') && <button onClick={() => setActiveTab('work_groups')} style={sideNavItemStyle(activeTab === 'work_groups')}>{tr('admin.kbvtsvtAbvdh2')}</button>}
          <div style={{ height: '1px', background: '#334155', margin: '10px 0 0' }} />
          ...
          }} style={sideNavItemStyle(activeTab === 'contacts')}>{tr('admin.kshrym')}</button>}
          <div style={{ height: '1px', background: '#334155', margin: '10px 0 0' }} />
        ...
        <div style={{ flex: 1, background: '#1e293b', borderRadius: '12px', padding: '18px', minHeight: '500px', minWidth: 0 }}>
          ...
          {activeTab === 'presets' && (
            <div>
              ...
              {(!!editingPreset || showNewPresetModal) && <MaybeSettingsModal
                ...
                onClose={() => { setEditingPreset(null); setShowNewPresetModal(false); setPresetFormInitial(null); setPresetForm({ name: '', map_id: '', relevant_sectors: [], table_mode_id: '', partial_load: 3, full_load: 5, load_relevant: true, conflict_alt_delta: 500, relevant_control_stations: [], filter_query: null, block_table_ids: [], vertical_time_based: true, view_alt_min: '', view_alt_max: '', display_mode: 'complex', classic_strip_table_id: '', classic_strip_table_id_night: '', classic_receive_points: [], classic_transfer_points: [], preset_type: 'normal', airfield_id: '', classic_partner_preset_ids: [], classic_incoming_partner_preset_ids: [], classic_outgoing_partner_preset_ids: [], show_serials: true, allow_view_switching: true, show_base_statuses: false, base_status_ids: [], preset_role: '', parent_base_id: '', can_update_pressure: false, show_dashboard: false, flight_zones_mode: false, fz_pin_display: 'handwrite', datk_show_minutes: '', can_update_mazaa: false, mazaa_update_base_id: '', can_update_atis: false, can_update_notam: false, can_add_vehicle: false, use_map_zones: false, can_seize_zone: false, civilian_columns: [], civilian_board_bg: '', civilian_strip_table_id: '', dual_map_mode: false, map2_id: '', dual_map_layout: 'side-by-side', dual_map_split: 50, suggest_alt_range: false, show_full_picture: false, blind_map_default: false, conflict_alt_rules: [], sector_maps_enabled: false, sector_map_ids: [] as number[], map2_sector_maps_enabled: false, map2_sector_map_ids: [] as number[], data_windows: [] as DataWindowDef[], air_picture_enabled: false, zone_watch_settings: { alerts: true, airLogicWhenOff: true }, show_data_windows: false, show_window_container: false, window_container_position: 'beforeAids' }); }}
                wide
              >
              <div style={{ borderRadius: '8px', padding: '0', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.shmAmdh')}</label>
                    <input
                      type="text"
                      value={presetForm.name}
                      onChange={(e) => setPresetForm(p => ({ ...p, name: e.target.value }))}
                      placeholder={tr('admin.ldvgmhMrchby305')}
                ...
                {presetForm.preset_type === 'classic' ? (
                  <div style={{ marginTop: '12px', padding: '10px 14px', background: '#0f172a', borderRadius: '8px', border: (presetForm as any).strip_window_id ? '1px solid #7c3aed' : '1px solid #1e293b' }}>
                    <label style={{ display: 'block', marginBottom: '6px', color: (presetForm as any).strip_window_id ? '#c4b5fd' : '#94a3b8', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.chlvnStrypym2')}</label>
                    <select
                    ...
                    >
                      <option value=''>{tr('admin.llaChlvnStrypym')}</option>
                ...
                {(presetForm.preset_type === 'ground' || presetForm.preset_type === 'ground_mgmt') ? (
                  <div style={{ marginBottom: '15px' }}>
                    <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '14px' }}>{tr('shared.airfield')}</label>
                    <select value={presetForm.airfield_id}
                      ...
                      <option value="">{tr('admin.llaShdh')}</option>
                      {adminAirfields.map((af: any) => <option key={af.id} value={af.id}>{af.name}</option>)}
                    </select>
                    {adminAirfields.length === 0 && <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: '#ef4444' }}>{tr('admin.tsvrShdhTavphBlshvnyt')}</p>}
                    {(() => {
                    ...
                    {presetForm.preset_type === 'ground_mgmt' && (
                      <div style={{ marginTop: '12px' }}>
                        <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.mph')}</label>
                        <select value={presetForm.map_id}
                          onChange={e => setPresetForm(p => ({ ...p, map_id: e.target.value }))}
                          style={{ width: '100%', padding: '10px', background: '#1e293b', border: '1px solid #475569', borderRadius: '6px', color: 'white', fontSize: '14px', direction: 'rtl' }}>
                          <option value="">{tr('admin.llaMph')}</option>
                    ...
                    {presetForm.preset_type !== 'ground_mgmt' && <div style={{ marginTop: '12px', padding: '12px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                      <label style={{ display: 'block', marginBottom: '6px', color: '#7dd3fc', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.htsgtMtvsLydDt')}</label>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        />
                        <span style={{ color: '#94a3b8', fontSize: '13px' }}>{tr('admin.dkvtLpnyHmrah')}</span>
                      </div>
                      <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b', lineHeight: '1.5' }}>
                        {tr('admin.anAircraftWhoseParking')}<br/>
                  ...
                  <div style={{ marginBottom: '15px', padding: '14px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    <label style={{ display: 'block', marginBottom: '10px', color: '#7dd3fc', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.hgdrtAmdtStrypym')}</label>
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
                      <div style={{ flex: 1 }}>
                        <label style={{ display: 'block', marginBottom: '6px', color: '#fbbf24', fontSize: '13px' }}>{tr('admin.tbnytYvm')}</label>
                        <select value={presetForm.classic_strip_table_id}
                          ...
                          <option value="">{tr('admin.llaTbnyt')}</option>
                          {(classicTables || []).map((ct: any) => <option key={ct.id} value={ct.id}>{ct.name}</option>)}
                        </select>
                      </div>
                      <div style={{ flex: 1 }}>
                        <label style={{ display: 'block', marginBottom: '6px', color: '#818cf8', fontSize: '13px' }}>{tr('admin.tbnytLylh')}</label>
                        <select value={presetForm.classic_strip_table_id_night}
                          ...
                          style={{ padding: '6px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '13px', direction: 'rtl', width: '100%' }}>
                          <option value="">{tr('admin.kmvYvm')}</option>
                          {(classicTables || []).map((ct: any) => <option key={ct.id} value={ct.id}>{ct.name}</option>)}
                        </select>
                      </div>
                    </div>
                  ...
                  <div style={{ marginBottom: '15px' }}>
                    ...
                    >
                      <option value="">{tr('admin.bchrMph')}</option>
                      {maps.map(m => (
                ...
                <div style={{ marginBottom: '15px', padding: '12px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#7dd3fc', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.tpkydAmdh')}</label>
                  ...
                  <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.mgdlMtsygSidBp')}</p>
                </div>

                {/* Parent base selection */}
                <div style={{ marginBottom: '15px', padding: '12px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#7dd3fc', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.bsysAb')}</label>
                  <select
                  ...
                  {adminAviationBases.length === 0 && (
                    <p style={{ margin: '4px 0 0 0', fontSize: '11px', color: '#f59e0b' }}>{tr('admin.hgdrBsysyTavphBlshvnyt')}</p>
                  )}
                  <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.hbsysShalyvShyyktHamdh')}</p>
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ marginTop: '18px', padding: '14px', background: '#0a1628', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                      <div style={{ color: '#7dd3fc', fontSize: '13px', fontWeight: 'bold', marginBottom: '4px' }}>{tr('admin.lvchHvdavtMagrHvdavt')}</div>
                      <p style={{ margin: '0 0 8px 0', fontSize: '11px', color: '#475569' }}>{tr('admin.lklHvdah')} <b>{tr('admin.nmanym')}</b> {tr('admin.lmyTvptsV')}<b>{tr('admin.bM')}</b> {tr('admin.kptvrShyvpyaAvtvmtytBlvch')}</p>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', marginBottom: '8px' }}>
                        {cat.length === 0 && <span style={{ fontSize: '11px', color: '#475569' }}>{tr('admin.aynHvdavtBmagr')}</span>}
                        {cat.map((it, i) => (
                          <div key={i} style={{ background: '#0f1d33', border: '1px solid #1e3a5f', borderRadius: '6px', padding: '7px 9px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                              ...
                              <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#cbd5e1', cursor: 'pointer' }}>
                                <input type="checkbox" checked={it.to_all} onChange={e => upd(i, { to_all: e.target.checked })} /> {tr('shared.all')}
                              </label>
                      ...
                      <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '6px', minHeight: '90px', alignItems: 'stretch', background: '#07090c', borderRadius: '8px', padding: '10px', border: '1px solid #1e293b' }}>
                        ...
                        {civCols.length < 3 && (
                          <button type="button"
                            ...
                            <span>+</span>
                            <span style={{ fontSize: '10px', color: '#334155' }}>{tr('admin.amvdh2')}</span>
                          </button>
                        )}
                      </div>
                      <p style={{ margin: '6px 0 0 0', fontSize: '10px', color: '#334155' }}>
                        {civCols.length} {tr('admin.columnsDragToReorder')}
                      </p>
                    </div>
                  );
                })()}
                ...
                {presetForm.preset_type === 'normal' && <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.mtsbTtsvghBryrtMchdl')}</label>
                  <div style={{ display: 'flex', gap: '10px', marginBottom: presetForm.table_mode_id ? '14px' : '0' }}>
                    ...
                    >{tr('admin.mvdTblh')}</button>
                  </div>
                  {presetForm.table_mode_id !== '' && tableModes.length > 0 && (
                    <div>
                      <div style={{ color: '#64748b', fontSize: '12px', marginBottom: '8px' }}>{tr('admin.bchrTblh')}</div>
                ...
                <div style={{ marginTop: '15px', padding: '14px', background: '#1e293b', borderRadius: '8px', border: '1px solid #334155' }}>
                  <label style={{ display: 'block', marginBottom: '10px', color: '#f59e0b', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.mvdAvms')}</label>
                  {/* עומס רלוונטי לעמדה? - כבוי: אין ספים כאן ואין עומס בעמדה (isLoadRelevant) */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: presetForm.load_relevant ? '12px' : 0 }}>
                    <span style={{ color: '#94a3b8', fontSize: '13px' }}>{tr('admin.loadRelevantQuestion')}</span>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      {([{ val: true, label: tr('admin.loadRelevantYes') }, { val: false, label: tr('admin.loadRelevantNo') }] as { val: boolean; label: string }[]).map(opt => (
                        <button key={String(opt.val)} type="button" data-testid={`load-relevant-${opt.val}`}
                          onClick={() => setPresetForm(p => ({ ...p, load_relevant: opt.val }))}
                          style={{ padding: '4px 14px', borderRadius: '6px', border: `1px solid ${presetForm.load_relevant === opt.val ? '#f59e0b' : '#334155'}`, background: presetForm.load_relevant === opt.val ? '#1c1107' : '#0f172a', color: presetForm.load_relevant === opt.val ? '#fbbf24' : '#94a3b8', cursor: 'pointer', fontSize: '13px', fontWeight: presetForm.load_relevant === opt.val ? 'bold' : 'normal' }}>
                          {opt.label}
                        </button>
                      ))}
                    </div>
                  </div>
                  {presetForm.load_relevant && <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', color: '#fbbf24', fontSize: '13px' }}>{tr('admin.avmsChlkyKtvmMspr')}</label>
                      <input
                        type="number"
                        min="1"
                        max="99"
                    ...
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', color: '#f87171', fontSize: '13px' }}>{tr('admin.avmsMlaAdvmMspr')}</label>
                      <input
                        type="number"
                        min="1"
                        max="99"
                  ...
                  <p style={{ margin: '8px 0 0 0', color: '#64748b', fontSize: '11px', direction: 'rtl' }}>
                    {tr('admin.countsAirborneFormationsAt')}
                  </p>
                  </>}
                  <div style={{ marginTop: '12px', borderTop: '1px solid #334155', paddingTop: '12px' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#f472b6', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.kvnplyktGvbhLpyMark')}</label>
                    ...
                    {(presetForm.conflict_alt_rules || []).map((rule, idx) => (
                      <div key={idx} style={{ display: 'flex', gap: '6px', marginBottom: '6px', alignItems: 'center', direction: 'rtl' }}>
                        <input
                          ...
                          placeholder={tr('admin.markLpyShmTyyst')}
                          value={rule.maarav}
                    ...
                    >{tr('admin.hvsfMark')}</button>
                    {/* Fallback general threshold */}
                    <div style={{ borderTop: '1px solid #334155', paddingTop: '10px' }}>
                      <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '12px' }}>{tr('admin.sfKllyLmyShayn')}</label>
                      <input
                        type="number"
                        min="0"
                        max="99000"
                ...
                <div style={{ marginTop: '14px' }}>
                  <label style={{ display: 'block', marginBottom: '6px', color: '#38bdf8', fontSize: '13px' }}>{tr('admin.tayShlythRlvvntyymLamdh')}</label>
                  {(() => {
                    ...
                    if (allAdminStations.length === 0) {
                      return <p style={{ color: '#64748b', fontSize: '11px', margin: 0 }}>{tr('admin.aynSprvrymBmarktYsh')}</p>;
                    }
                    return (
                      <>
                        ...
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <button type="button" onClick={() => setPresetForm(p => ({ ...p, relevant_control_stations: allAdminStations }))} style={{ fontSize: '11px', background: 'transparent', border: '1px solid #334155', color: '#94a3b8', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer' }}>{tr('admin.bchrHkl')}</button>
                          <button type="button" onClick={() => setPresetForm(p => ({ ...p, relevant_control_stations: [] }))} style={{ fontSize: '11px', background: 'transparent', border: '1px solid #334155', color: '#94a3b8', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer' }}>{tr('shared.clearAll')}</button>
                        </div>
                        <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '11px', direction: 'rtl' }}>
                          {tr('admin.ifNoControlCell')}
                        </p>
                      </>
                    );
                  })()}
                ...
                {/* קונטיינר החלונות - עמודה בצד שאליה הפקח דוחף חלונות צפים.
                    כאן נקבע רק אם היכולת קיימת בעמדה; הפתיחה/סגירה בפועל היא
                    בתפריט "תצוגה" בעמדה. */}
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('dock.adminLabel')}</label>
                  <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                    {[{ val: true, label: tr('dataWindows.onByDefault') }, { val: false, label: tr('dataWindows.offByDefault') }].map(opt => (
                      <button key={String(opt.val)} type="button"
                        onClick={() => setPresetForm(p => ({ ...p, show_window_container: opt.val }))}
                        style={{ padding: '6px 16px', borderRadius: '6px', border: `1px solid ${((presetForm as any).show_window_container === true) === opt.val ? '#0ea5e9' : '#334155'}`, background: ((presetForm as any).show_window_container === true) === opt.val ? '#0c2a40' : '#1e293b', color: ((presetForm as any).show_window_container === true) === opt.val ? '#7dd3fc' : '#94a3b8', cursor: 'pointer', fontSize: '13px' }}>
                        {opt.label}
                      </button>
                    ))}
                  </div>
                  <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('dock.adminHint')}</p>
                  {/* מיקום ברירת המחדל - אותו בורר ויזואלי שבעמדה, במצב מבוקר */}
                  <div style={{ marginTop: '10px' }}>
                    <label style={{ display: 'block', marginBottom: '6px', color: '#94a3b8', fontSize: '13px' }}>{tr('dock.adminPosition')}</label>
                    <DockPositionPicker
                      themeMode="dark"
                      value={((presetForm as any).window_container_position || 'beforeAids') as DockPosition}
                      onChange={pos => setPresetForm(p => ({ ...p, window_container_position: pos }))}
                    />
                  </div>
                </div>
                ...
                {presetForm.preset_type !== 'ground_mgmt' && <div style={{ marginTop: '15px' }}>
                  ...
                  <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '11px', direction: 'rtl' }}>
                    {tr('admin.determinesWhetherTheSerials')}
                  </p>
                </div>}

                {/* Allow view switching toggle */}
                {presetForm.preset_type !== 'ground_mgmt' && <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.apshrMabrBynMph')}</label>
                  <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                  ...
                  <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '11px', direction: 'rtl' }}>
                    {tr('admin.whenOffTheView')}
                  </p>
                </div>}

                {/* Base statuses toggle */}
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.htsgSttvsBsysymBpanl')}</label>
                  ...
                  {presetForm.show_base_statuses && (
                    <div style={{ direction: 'rtl' }}>
                      <div style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '6px' }}>{tr('admin.bchrBsysymLhtsghRyk')}</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: '160px', overflowY: 'auto', background: '#0f172a', borderRadius: '6px', padding: '8px', border: '1px solid #334155' }}>
                        {adminBaseStatuses.length === 0 ? (
                          <div style={{ fontSize: '11px', color: '#475569' }}>{tr('admin.aynBsysymMvgdrymHvsf')}</div>
                ...
                <div style={{ marginTop: '15px', padding: '12px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#7dd3fc', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.shdhTavphTbnyt')}</label>
                  <select
                  ...
                  >
                    <option value="">{tr('admin.llaShdhTavph')}</option>
                  ...
                  {adminAirfields.length === 0 && <p style={{ margin: '0', fontSize: '11px', color: '#ef4444' }}>{tr('admin.tsvrShdvtTavphBlshvnyt')}</p>}
                </div>
                )}

                {/* can_update_mazaa toggle */}
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.hrshatAdkvnMtsbMz')}</label>
                  ...
                  <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.amdvtMadknYkvlvtLshnvt2')}</p>
                </div>
                ...
                {presetForm.can_update_mazaa && presetForm.preset_role === 'tower' && (
                  <div style={{ marginTop: '10px', padding: '10px 12px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#fbbf24', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.bsysYadLadkvnMz')}</label>
                    <select
                      value={(presetForm as any).mazaa_update_base_id || ''}
                      onChange={e => setPresetForm(p => ({ ...p, mazaa_update_base_id: e.target.value }))}
                      style={{ width: '100%', padding: '8px 10px', background: '#1e293b', border: '1px solid #475569', borderRadius: '6px', color: 'white', fontSize: '13px', direction: 'rtl' }}
                    >
                      <option value="">{tr('admin.rashvnBBaseStatus')}</option>
                      {adminBaseStatuses.map((bs: any) => (
                        <option key={bs.id} value={bs.id}>{bs.name}{bs.code ? ` (${bs.code})` : ''}</option>
                      ))}
                    </select>
                    <p style={{ margin: '5px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.bchrBmpvrshAtHbsys')}</p>
                  </div>
                )}
                ...
                {(presetForm.preset_type === 'ground' || presetForm.preset_type === 'ground_mgmt') && (
                  <div style={{ marginTop: '12px', padding: '12px', background: '#0a1628', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    <div style={{ marginBottom: '10px', color: '#7dd3fc', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.hrshavtAdkvnAtisNotam')}</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div>
                        <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '12px' }}>{tr('admin.atisMyYkvlLadkn')}</label>
                        <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                          {([{ val: true, label: '✏️ מעדכן' }, { val: false, label: '👁 קריאה בלבד' }] as { val: boolean; label: string }[]).map(opt => (
                            <button key={String(opt.val)} type="button"
                      ...
                      <div>
                        <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '12px' }}>{tr('admin.notamMyYkvlLadkn')}</label>
                        <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                          {([{ val: true, label: '✏️ מעדכן' }, { val: false, label: '👁 קריאה בלבד' }] as { val: boolean; label: string }[]).map(opt => (
                            <button key={String(opt.val)} type="button"
                    ...
                    <p style={{ margin: '8px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.amdvtMadknYkvlvtLshnvt')}</p>
                  </div>
                )}

                {/* מד עומס לפי מצב מז"א */}
                {editingPreset && presetForm.load_relevant && (
                  <div style={{ marginTop: '15px', padding: '12px', background: '#0f172a', borderRadius: '8px', border: '1px solid #334155' }}>
                    <label style={{ display: 'block', marginBottom: '10px', color: '#fbbf24', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.mdAvmsLpyMtsb')}</label>
                      ...
                      <div style={{ marginBottom: '10px' }}>
                        {editingPresetMazaaRows.map(row => (
                          <div key={row.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', direction: 'rtl' }}>
                            ...
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.chlky')}</span>
                            ...
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.mla')}</span>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', direction: 'rtl', paddingTop: editingPresetMazaaRows.length > 0 ? '8px' : '0', borderTop: editingPresetMazaaRows.length > 0 ? '1px solid #334155' : 'none' }}>
                      <select value={newMazaaRow.mazaa_status} onChange={e => setNewMazaaRow(p => ({ ...p, mazaa_status: e.target.value }))}
                        ...
                        <option value="">{tr('admin.bchrMtsbMzA')}</option>
                      ...
                      <button onClick={async () => {
                        ...
                        style={{ padding: '4px 12px', background: newMazaaRow.mazaa_status ? '#1c4532' : '#1e293b', border: `1px solid ${newMazaaRow.mazaa_status ? '#22c55e' : '#334155'}`, color: newMazaaRow.mazaa_status ? '#86efac' : '#64748b', borderRadius: '5px', cursor: newMazaaRow.mazaa_status ? 'pointer' : 'default', fontSize: '12px', fontWeight: 'bold' }}>{tr('shared.add')}</button>
                    </div>
                    <p style={{ margin: '8px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.whenThresholdsAreSet')}{presetForm.partial_load}{tr('admin.full')}{presetForm.full_load}.</p>
                  </div>
                )}

                {presetForm.preset_type !== 'ground_mgmt' && <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.dshBvrd')}</label>
                  <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                ...
                {presetForm.map_id && (
                  <div style={{ marginTop: '15px', padding: '12px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#7dd3fc', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.mtsbAzvryTysh')}</label>
                    <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                    ...
                    <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.kshmvpalLvchHmphMtsyg')}</p>
                  </div>
                )}
                ...
                {presetForm.flight_zones_mode && presetForm.map_id && (
                  <div style={{ marginTop: '10px', padding: '12px', background: '#0f1a2a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    ...
                    <p style={{ margin: '4px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.strypKrtysyyhAmAvt')}</p>
                  </div>
                )}
                ...
                {(presetForm.block_table_ids?.length > 0) && (
                  <div style={{ marginTop: '15px', padding: '12px', background: '#0f1a0f', borderRadius: '8px', border: '1px solid #14532d' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#86efac', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.htsatTvvchBlvkGbhym')}</label>
                    <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                    ...
                    <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#4ade80' }}>{tr('admin.kshmvpalLachrHgdrtGvbh')}</p>
                  </div>
                )}
                ...
                {!!presetForm.table_mode_id && (
                  <div style={{ marginTop: '15px', padding: '12px', background: '#1a1a2e', borderRadius: '8px', border: '1px solid #7c3aed' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#c4b5fd', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.kptvrHtsgLyAt')}</label>
                    <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                    ...
                    <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#a78bfa' }}>{tr('admin.kshmvpalMvpyaBamdhKptvr')}</p>
                  </div>
                )}

                {/* use_map_zones toggle */}
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.htchshbBazvrymAlMph')}</label>
                  <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                    {[{ val: true, label: '✅ פעיל' }, { val: false, label: '⬜ כבוי' }].map(opt => (
                      <button key={String(opt.val)} type="button"
                ...
                {/* הרשאת הלאמת אזור זמני - ב"מ **לא**. רלוונטית רק לעמדה עם מפה
                    מעוגנת: בלי עוגנים אין דרך להקרין את המרחב לעמדה אחרת. */}
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.seizePerm')}</label>
                  <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                    {[{ val: true, label: tr('admin.seizePermOn') }, { val: false, label: tr('admin.seizePermOff') }].map(opt => (
                      <button key={String(opt.val)} type="button"
                        onClick={() => setPresetForm(p => ({ ...p, can_seize_zone: opt.val }))}
                        style={{ padding: '6px 16px', borderRadius: '6px', border: `1px solid ${(presetForm as any).can_seize_zone === opt.val ? '#f97316' : '#334155'}`, background: (presetForm as any).can_seize_zone === opt.val ? '#431407' : '#1e293b', color: (presetForm as any).can_seize_zone === opt.val ? '#fdba74' : '#94a3b8', cursor: 'pointer', fontSize: '13px', fontWeight: (presetForm as any).can_seize_zone === opt.val ? 'bold' : 'normal' }}>
                        {opt.label}
                      </button>
                    ))}
                  </div>
                  <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('admin.seizePermHint')}</p>
                </div>
                ...
                {/* לוגיקות התמונ"א גם כשהיא אינה מוצגת - זיהוי חריגה מאזור ומעקב הקפה
                    אוטומטי. ברירת המחדל **כן** (הכרעת הפקח 2026-09-15). זו ברירת המחדל
                    של העמדה בלבד: בעמדה עצמה הפקח מדליק ומכבה בפאנל התמונ"א. */}
                {(presetForm as any).air_picture_enabled === true && (
                  <div style={{ marginTop: '15px' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('airPicture.adminLogicWhenOff')}</label>
                    <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                      {[{ val: true, label: tr('zoneWatch.optOn') }, { val: false, label: tr('zoneWatch.optOff') }].map(opt => {
                        const cur = (presetForm as any).zone_watch_settings?.airLogicWhenOff !== false;
                        return (
                          <button key={String(opt.val)} type="button" data-testid={`preset-ap-logic-when-off-${opt.val ? 'on' : 'off'}`}
                            onClick={() => setPresetForm(p => ({ ...p, zone_watch_settings: { ...((p as any).zone_watch_settings || {}), airLogicWhenOff: opt.val } }))}
                    ...
                    <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#64748b' }}>{tr('airPicture.adminLogicWhenOffHint')}</p>
                  </div>
                )}
                ...
                {presetForm.map_id && (
                  <div style={{ marginTop: '12px' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.mphAyvvrtKbryrtMchdl')}</label>
                    <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                      {[{ val: true, label: '✅ פעיל' }, { val: false, label: '⬜ כבוי' }].map(opt => (
                        <button key={String(opt.val)} type="button"
                ...
                {(presetForm.preset_type === 'normal' || !presetForm.preset_type) && presetForm.map_id && (
                  <div style={{ marginTop: '15px', padding: '12px', background: '#0d1f35', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#7dd3fc', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.mtsbShtyMpvt')}</label>
                    ...
                    {(presetForm as any).dual_map_mode && (<>
                      <div style={{ marginBottom: '10px' }}>
                        <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.mphShnyyh')}</label>
                        <select value={(presetForm as any).map2_id} onChange={e => setPresetForm(p => ({ ...p, map2_id: e.target.value }))}
                          style={{ width: '100%', padding: '8px 10px', background: '#1e293b', border: '1px solid #475569', borderRadius: '6px', color: 'white', fontSize: '13px', direction: 'rtl' }}>
                          <option value="">{tr('admin.bchrMphShnyyh')}</option>
                          {maps.filter(m => m.id !== Number(presetForm.map_id)).map((m: any) => <option key={m.id} value={m.id}>{m.name}</option>)}
                        </select>
                      </div>
                      <div style={{ marginBottom: '10px' }}>
                        <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.prysh')}</label>
                        <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                      ...
                      <div>
                        <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.chlvkhRashvnytMphRashyt')} <strong style={{ color: '#7dd3fc' }}>{(presetForm as any).dual_map_split}%</strong></label>
                ...
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.ttsvghVvrtyklytTsyrZmn')}</label>
                  <div style={{ display: 'flex', gap: '8px', direction: 'rtl' }}>
                ...
                <div style={{ marginTop: '15px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center', direction: 'rtl' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                      <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.mynymvmFl')}</span>
                      <input type="number" placeholder={tr('admin.rykAvtvmty')}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                      <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.mksymvmFl')}</span>
                      <input type="number" placeholder={tr('admin.rykAvtvmty')}
                  ...
                  <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '11px', direction: 'rtl' }}>
                    {tr('admin.theMinimumRangeAlways')}
                  </p>
                </div>
                ...
                {presetForm.preset_type !== 'classic' && presetForm.preset_type !== 'ground' && <div style={{ marginTop: '20px', padding: '14px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e293b' }}>
                  <label style={{ display: 'block', marginBottom: '10px', color: '#94a3b8', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.mtsbTtsvgtAmdh')}</label>
                  ...
                  {presetForm.display_mode === 'classic' && (
                    <div>
                      <label style={{ display: 'block', marginBottom: '6px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.tbnytAytsvbStryp')}</label>
                      <select value={presetForm.classic_strip_table_id}
                        onChange={e => setPresetForm(p => ({ ...p, classic_strip_table_id: e.target.value }))}
                        style={{ padding: '6px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '13px', direction: 'rtl', width: '100%' }}>
                        <option value="">{tr('admin.llaTbnyt')}</option>
                      ...
                      {classicTables.length === 0 && <p style={{ margin: '4px 0 0 0', fontSize: '11px', color: '#ef4444' }}>{tr('admin.tsvrTbnytBlshvnytTtsvgt')}</p>}
                    </div>
                  )}
                </div>}
                ...
                {(presetForm.preset_type === 'classic' || presetForm.display_mode === 'classic') && (
                  <div style={{ marginTop: '20px', padding: '14px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    <label style={{ display: 'block', marginBottom: '10px', color: '#93c5fd', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.shvtpvtVkshrySktvrStrypym')}</label>
                ...
                {presetForm.preset_type !== 'ground_mgmt' && blockTables.length > 0 && (
                  <div style={{ marginTop: '15px' }}>
                    <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.tblavtBlvkymRlvvntyvtLamdh')}</label>
                ...
                {!(presetForm as any).strip_window_id && <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.nkvdvtHabrhLchtsLbchyrh')}</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    ...
                    {sectors.length === 0 && (
                      <span style={{ color: '#64748b', fontSize: '14px' }}>{tr('admin.aynNkvdvtHabrhMvgdrvt')}</span>
                    )}
                  </div>
                </div>}
                ...
                {presetForm.dual_map_mode && (presetForm as any).map2_id && <div style={{ marginTop: '15px', padding: '10px 14px', background: '#0a1628', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#7dd3fc', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.nkvdvtHabrhMph2')}</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {sectors.map(sector => {
                    ...
                    {sectors.length === 0 && <span style={{ color: '#64748b', fontSize: '13px' }}>{tr('admin.aynSktvrymMvgdrym')}</span>}
                  </div>
                </div>}
                ...
                {presetForm.preset_type !== 'classic' && presetForm.relevant_sectors.length > 0 && (
                  <div style={{ marginTop: '14px', padding: '10px 14px', background: '#0f172a', borderRadius: '8px', border: '1px solid #1e3a5f' }}>
                    <label style={{ display: 'block', marginBottom: '6px', color: '#fbbf24', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.tnayGvbhZvgyvtLnkvdvt')}</label>
                    <p style={{ margin: '0 0 8px 0', fontSize: '11px', color: '#64748b', direction: 'rtl' }}>{tr('admin.nytnLhgdyrGvbhMyn')}</p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {presetForm.relevant_sectors.map((sid: number) => {
                        ...
                        return (
                          <div key={sid} style={{ display: 'flex', alignItems: 'center', gap: '8px', direction: 'rtl', padding: '5px 8px', background: '#0a1628', borderRadius: '6px', border: '1px solid #1e3a5f', flexWrap: 'wrap' }}>
                            ...
                            <span style={{ color: '#64748b', fontSize: '11px' }}>{tr('shared.minAltitude')}</span>
                            ...
                            <span style={{ color: '#64748b', fontSize: '11px' }}>{tr('shared.max')}</span>
                            <input type="number" placeholder="—" value={pt.alt_max ?? ''}
                              onChange={e => updatePt({ alt_max: e.target.value !== '' ? Number(e.target.value) : null })}
                              style={{ width: '58px', padding: '3px 5px', background: '#0f172a', border: '1px solid #92400e', borderRadius: '4px', color: '#fbbf24', fontSize: '11px', textAlign: 'center' }} />
                            <span style={{ color: '#64748b', fontSize: '11px' }}>{tr('shared.parity')}</span>
                ...
                {editingPreset && (
                  <div style={{ marginTop: '20px', borderTop: '1px solid #334155', paddingTop: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                      <label style={{ color: '#a78bfa', fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.kyshvrym')}</label>
                    ...
                    {showAddLinkForm && (
                      <div style={{ background: '#1e1b4b', border: '1px solid #4c1d95', borderRadius: '7px', padding: '12px', marginBottom: '10px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                          <div>
                            <label style={{ display: 'block', fontSize: '11px', color: '#94a3b8', marginBottom: '3px' }}>{tr('admin.shmKyshvr')}</label>
                            <input value={newLinkForm.name} onChange={e => setNewLinkForm(v => ({...v, name: e.target.value}))}
                              placeholder={tr('admin.shm')} style={{ width: '100%', padding: '6px 8px', background: '#0f172a', color: 'white', border: '1px solid #334155', borderRadius: '4px', fontSize: '13px', direction: 'rtl', boxSizing: 'border-box' }} />
                          </div>
                          <div>
                            <label style={{ display: 'block', fontSize: '11px', color: '#94a3b8', marginBottom: '3px' }}>{tr('shared.category2')}</label>
                            <input value={newLinkForm.category} onChange={e => setNewLinkForm(v => ({...v, category: e.target.value}))}
                              placeholder={tr('admin.ktgvryh')} style={{ width: '100%', padding: '6px 8px', background: '#0f172a', color: 'white', border: '1px solid #334155', borderRadius: '4px', fontSize: '13px', direction: 'rtl', boxSizing: 'border-box' }} />
                          </div>
                        </div>
                        <div style={{ marginBottom: '8px' }}>
                          <label style={{ display: 'block', fontSize: '11px', color: '#94a3b8', marginBottom: '3px' }}>{tr('admin.ktvbtUrl2')}</label>
                          <div style={{ display: 'flex', gap: '6px' }}>
                        ...
                        <div style={{ marginBottom: '10px' }}>
                          <label style={{ display: 'block', fontSize: '11px', color: '#94a3b8', marginBottom: '3px' }}>{tr('admin.harh')}</label>
                          <input value={newLinkForm.note} onChange={e => setNewLinkForm(v => ({...v, note: e.target.value}))}
                            placeholder={tr('shared.note2')} style={{ width: '100%', padding: '6px 8px', background: '#0f172a', color: 'white', border: '1px solid #334155', borderRadius: '4px', fontSize: '13px', direction: 'rtl', boxSizing: 'border-box' }} />
                        </div>
                        ...
                        }} style={{ background: newLinkForm.name.trim() && newLinkForm.url.trim() ? '#5b21b6' : '#334155', color: 'white', border: 'none', borderRadius: '5px', padding: '6px 18px', fontSize: '13px', cursor: 'pointer' }}>{tr('admin.hvsfKyshvr')}</button>
                      </div>
                    )}
                    {editingPresetLinks.length === 0 && !showAddLinkForm && (
                      <div style={{ color: '#64748b', fontSize: '12px', textAlign: 'center', padding: '8px 0' }}>{tr('admin.aynKyshvrymMvgdrymLamdh')}</div>
                    )}
                          ...
                          <div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginBottom: '6px' }}>
                              <input value={editLinkForm.name} onChange={e => setEditLinkForm(v => ({...v, name: e.target.value}))}
                                placeholder={tr('shared.name')} style={{ padding: '4px 7px', background: '#1e293b', color: 'white', border: '1px solid #334155', borderRadius: '4px', fontSize: '12px', direction: 'rtl', boxSizing: 'border-box' }} />
                              <input value={editLinkForm.category} onChange={e => setEditLinkForm(v => ({...v, category: e.target.value}))}
                                placeholder={tr('shared.category')} style={{ padding: '4px 7px', background: '#1e293b', color: 'white', border: '1px solid #334155', borderRadius: '4px', fontSize: '12px', direction: 'rtl', boxSizing: 'border-box' }} />
                            </div>
                            ...
                            <input value={editLinkForm.note} onChange={e => setEditLinkForm(v => ({...v, note: e.target.value}))}
                              placeholder={tr('shared.note')} style={{ width: '100%', padding: '4px 7px', background: '#1e293b', color: 'white', border: '1px solid #334155', borderRadius: '4px', fontSize: '12px', direction: 'rtl', boxSizing: 'border-box', marginBottom: '6px' }} />
                            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                              <button type="button" onClick={async () => {
                                ...
                                setEditingLinkId(null);
                                loadPresetLinks(editingPreset.id);
                              }} style={{ background: '#5b21b6', color: 'white', border: 'none', borderRadius: '4px', padding: '3px 12px', fontSize: '11px', cursor: 'pointer' }}>{tr('shared.save')}</button>
                              <button type="button" onClick={async () => {
                                if (!await customConfirm('למחוק קישור זה?')) return;
                                await fetch(`${API_URL}/preset-links/${link.id}`, { method: 'DELETE' });
                                loadPresetLinks(editingPreset.id);
                              }} style={{ background: '#7f1d1d', color: '#fca5a5', border: 'none', borderRadius: '4px', padding: '3px 12px', fontSize: '11px', cursor: 'pointer' }}>{tr('shared.delete2')}</button>
                              <button type="button" onClick={() => setEditingLinkId(null)} style={{ background: '#334155', color: 'white', border: 'none', borderRadius: '4px', padding: '3px 10px', fontSize: '11px', cursor: 'pointer' }}>{tr('shared.cancel')}</button>
                            </div>
                          </div>
                        ) : (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              <div key={preset.id} data-testid="admin-preset-row" data-preset-name={preset.name} style={{ background: '#0f172a', borderRadius: '8px', padding: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderInlineStart: `2px solid ${group.border}` }}>
                                <div>
                                  ...
                                  <div style={{ color: '#94a3b8', fontSize: '13px', marginTop: '4px' }}>
                                    {tr('admin.mph')} {maps.find(m => m.id === preset.map_id)?.name || 'לא מוגדר'}
                                ...
                                <div style={{ display: 'flex', gap: '8px' }}>
                                  <button onClick={() => editPreset(preset)} style={{ padding: '6px 15px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.edit')}</button>
                                  <button onClick={() => duplicatePreset(preset)} style={{ padding: '6px 15px', background: '#0f766e', color: '#99f6e4', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('admin.shkpl4')}</button>
                                  <button onClick={() => deletePreset(preset.id)} style={{ padding: '6px 15px', background: '#dc2626', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.delete')}</button>
                                </div>
                              </div>
                            );
                          })}
          ...
          {activeTab === 'sectors' && (
            <div>
              <h2 style={{ margin: '0 0 20px 0', fontSize: '18px' }}>{tr('admin.nyhvlNkvdvtHabrh')}</h2>
              ...
              <div style={{ background: editingSector ? 'transparent' : '#0f172a', borderRadius: '8px', padding: editingSector ? '0' : '20px', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.kvd2')}</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.shmBabryt')}</label>
                    <input
                      type="text"
                      value={sectorForm.label_he}
                      onChange={(e) => setSectorForm(f => ({ ...f, label_he: e.target.value }))}
                      placeholder={tr('admin.tspvn')}
                      style={{ width: '100%', padding: '10px', border: '1px solid #475569', borderRadius: '6px', background: '#1e293b', color: 'white', fontSize: '14px', boxSizing: 'border-box' }}
                    />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '14px' }}>{tr('shared.category2')}</label>
                    <input
                      type="text"
                      value={sectorForm.category}
                      onChange={(e) => setSectorForm(f => ({ ...f, category: e.target.value }))}
                      placeholder={tr('admin.lmshlMrchbGyshhMslvl')}
                      style={{ width: '100%', padding: '10px', border: '1px solid #475569', borderRadius: '6px', background: '#1e293b', color: 'white', fontSize: '14px', boxSizing: 'border-box' }}
                    />
                  </div>
                </div>
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '5px', color: '#94a3b8', fontSize: '14px' }}>{tr('admin.harvtLhabrtMydaByn')}</label>
                  <textarea
                    value={sectorForm.notes}
                    onChange={(e) => setSectorForm(f => ({ ...f, notes: e.target.value }))}
                    placeholder={tr('admin.ldvgmhZvgyTspvnyAy')}
                    rows={3}
                ...
                <div style={{ marginTop: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '5px', color: '#f472b6', fontSize: '14px' }}>{tr('admin.sfKvnplyktGvbhRglyym')}</label>
                  <input
                    type="number"
                    min="0"
                    max="99000"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <div key={sector.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '15px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        {hasAny ? (
                          <div style={{ marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '5px', alignItems: 'center' }}>
                            <span style={{ color: '#475569', fontSize: '11px' }}>{tr('admin.amdvt2')}</span>
                            {recvPresets.map((p: any) => (
                              <span key={`recv-${p.id}`} title={tr('admin.nkvdtKblhStrypymKlasy')} style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', padding: '2px 8px', background: '#052e16', border: '1px solid #166534', borderRadius: '10px', fontSize: '11px', color: '#86efac' }}>
                                📥 {p.name}
                              </span>
                            ))}
                            {xferPresets.map((p: any) => (
                              <span key={`xfer-${p.id}`} title={tr('admin.nkvdtHabrhStrypymKlasy')} style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', padding: '2px 8px', background: '#422006', border: '1px solid #92400e', borderRadius: '10px', fontSize: '11px', color: '#fcd34d' }}>
                                📤 {p.name}
                              </span>
                            ))}
                            {relPresets.map((p: any) => (
                              <span key={`rel-${p.id}`} title={tr('admin.nkvdtHabrhRlvvntytAmdt')} style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', padding: '2px 8px', background: '#1e293b', border: '1px solid #334155', borderRadius: '10px', fontSize: '11px', color: '#94a3b8' }}>
                                📍 {p.name}
                              </span>
                            ))}
                          </div>
                          ...
                          <div style={{ marginTop: '6px' }}>
                            <span style={{ color: '#374151', fontSize: '11px', fontStyle: 'italic' }}>{tr('admin.laBshymvshBafAmdh')}</span>
                          </div>
                        )}
                        {sector.notes && (
                      ...
                      <div style={{ display: 'flex', gap: '8px', marginRight: '15px' }}>
                        <button onClick={() => editSector(sector)} style={{ padding: '6px 15px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.edit')}</button>
                        <button onClick={() => duplicateSector(sector)} style={{ padding: '6px 15px', background: '#0f766e', color: '#99f6e4', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('admin.shkpl4')}</button>
                        <button onClick={() => deleteSector(sector.id)} style={{ padding: '6px 15px', background: '#dc2626', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.delete')}</button>
                      </div>
                    </div>
                  </div>
                  );
                ...
                {sectors.length === 0 && (
                  <div style={{ textAlign: 'center', color: '#64748b', padding: '40px' }}>
                    {tr('admin.noTransferPointsDefined')}
                  </div>
                )}
              </div>
            </div>
          ...
          {activeTab === 'strips' && (
            <div>
              ...
              <div style={{ marginBottom: '28px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px', flexWrap: 'wrap' }}>
                  <h2 style={{ margin: 0, fontSize: '18px' }}>
                    {tr('admin.formationList')}{filteredGlobalStrips.length}
                    {filteredGlobalStrips.length !== globalStrips.length ? ` / ${globalStrips.length}` : ''})
                  </h2>
                  <button
                  ...
                  >{tr('admin.pmmChdsh')}</button>
                  <button onClick={loadGlobalStrips} style={{ padding: '6px 12px', background: '#1e293b', color: '#94a3b8', border: '1px solid #334155', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{tr('admin.rann')}</button>
                  <button
                    onClick={async () => {
                  ...
                  >{tr('admin.mtvsymLkvlm')}</button>
                  <input
                    value={stripsSearch}
                    onChange={e => setStripsSearch(e.target.value)}
                    placeholder={tr('admin.chypvshLpyKryahTyyst')}
                ...
                {/* סינון לפי מקור הרשומה - מה הגיע מהשו"ב החיצוני (GAPI) ומה נרשם מקומית */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' }}>
                  <span style={{ color: '#64748b', fontSize: '12px', marginInlineEnd: '2px' }}>{tr('admin.gapiSourceFilter')}</span>
                  {([
                    { val: 'all', label: tr('admin.gapiSourceAll'), count: globalStrips.length, accent: '#94a3b8', bg: '#1e293b' },
                    { val: 'gapi', label: tr('admin.gapiSourceGapi'), count: gapiStripsCount, accent: '#c084fc', bg: '#2e1065' },
                    { val: 'local', label: tr('admin.gapiSourceLocal'), count: globalStrips.length - gapiStripsCount, accent: '#7dd3fc', bg: '#0c2b3f' },
                  ] as const).map(opt => (
                    <button
                      key={opt.val}
                      onClick={() => setStripsSource(opt.val)}
                      style={{
                        padding: '4px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px',
                        border: `1px solid ${stripsSource === opt.val ? opt.accent : '#334155'}`,
                        background: stripsSource === opt.val ? opt.bg : '#0f172a',
                        color: stripsSource === opt.val ? opt.accent : '#64748b',
                        fontWeight: stripsSource === opt.val ? 'bold' : 'normal',
                      }}
                    >{opt.label} ({opt.count})</button>
                  ))}
                  {gapiStripsCount === 0 && (
                    <span style={{ color: '#475569', fontSize: '11px' }}>{tr('admin.gapiNoneYet')}</span>
                  )}
                </div>
                ...
                {showNewStripForm && (
                  <div style={{ background: '#0f172a', border: '1px solid #22c55e', borderRadius: '8px', padding: '16px', marginBottom: '14px' }}>
                    <div style={{ fontWeight: 'bold', marginBottom: '10px', color: '#22c55e', fontSize: '14px' }}>{tr('admin.pmmChdsh2')}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '8px' }}>
                      ...
                      <div>
                        <div style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '3px' }}>{tr('shared.takeoffTime')}</div>
                        <input
                          type="datetime-local"
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
                      ...
                      <button onClick={() => setShowNewStripForm(false)} style={{ padding: '6px 14px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.cancel')}</button>
                    </div>
                  </div>
                )}
              ...
              <h2 style={{ margin: '0 0 20px 0', fontSize: '18px' }}>{tr('admin.tayntPmmymMkvbts')}</h2>
              ...
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '20px', marginBottom: '20px' }}>
                <p style={{ color: '#94a3b8', marginBottom: '15px', fontSize: '14px', lineHeight: '1.6' }}>
                  {tr('admin.loadFormationsFromA')} <strong style={{color:'#60a5fa'}}>Excel (.xlsx)</strong> {tr('admin.av')} <strong style={{color:'#60a5fa'}}>CSV (.csv)</strong>.<br/>
                  <strong>{tr('admin.avKHvaShdh')}</strong>
                </p>
                  ...
                  <div style={{ marginTop: '20px', padding: '15px', background: '#1e293b', borderRadius: '8px' }}>
                    {csvImportResult.imported > 0 && (
                      <div style={{ color: '#22c55e', marginBottom: '8px', fontSize: '15px' }}>
                        {tr('admin.newAdded')} {csvImportResult.imported} {tr('admin.formations')}
                      </div>
                    )}
                    {csvImportResult.updated > 0 && (
                      <div style={{ color: '#60a5fa', marginBottom: '8px', fontSize: '15px' }}>
                        {tr('admin.updated2')} {csvImportResult.updated} {tr('admin.formations')}
                      </div>
                    )}
                    {csvImportResult.skipped > 0 && (
                      <div style={{ color: '#94a3b8', marginBottom: '8px', fontSize: '14px' }}>
                        {tr('admin.unchanged')} {csvImportResult.skipped} {tr('admin.formations')}
                      </div>
                    )}
                    {csvImportResult.errors.length > 0 && (
                      <div style={{ color: '#dc2626', fontSize: '13px' }}>
                        {tr('admin.errors2')} {csvImportResult.errors.join(', ')}
                      </div>
                    )}
                    {csvImportResult.unresolvedAirfields && csvImportResult.unresolvedAirfields.length > 0 && (
                      <div style={{ color: '#f59e0b', fontSize: '13px', marginTop: '8px', padding: '8px 10px', background: '#1c1a0a', borderRadius: '5px', border: '1px solid #78350f' }}>
                        {tr('admin.unrecognizedAirfields')} <strong>{csvImportResult.unresolvedAirfields.join(', ')}</strong><br/>
                        <span style={{ fontSize: '11px', color: '#d97706' }}>{tr('admin.vvdaShhshmBkvbtsZhh')}</span>
                      </div>
                    )}
                    {csvImportResult.airfieldDebug && csvImportResult.airfieldDebug.length > 0 && (
                      <div style={{ marginTop: '10px', padding: '8px 10px', background: '#0d1117', borderRadius: '5px', border: '1px solid #334155', fontSize: '12px', color: '#94a3b8', direction: 'ltr', textAlign: 'left' }}>
                        <div style={{ color: '#64748b', marginBottom: '4px', fontWeight: 'bold', direction: 'rtl', textAlign: 'right' }}>{tr('admin.abchvnShdvtTavph3')}</div>
                    ...
                    {csvImportResult.detectedColumns && csvImportResult.detectedColumns.length > 0 && (
                      <div style={{ marginTop: '8px', padding: '8px 10px', background: '#0d1117', borderRadius: '5px', border: '1px solid #1e293b', fontSize: '11px', color: '#475569', direction: 'ltr', textAlign: 'left' }}>
                        <span style={{ color: '#334155' }}>{tr('admin.amvdvtShnmtsav')} </span>{csvImportResult.detectedColumns.join(' | ')}
                      </div>
                    )}
                  </div>
                )}
              ...
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '20px' }}>
                <h3 style={{ margin: '0 0 15px 0', fontSize: '16px', color: '#94a3b8' }}>{tr('admin.pvrmtHkvbts')}</h3>
                
                <div style={{ marginBottom: '16px', fontSize: '13px', color: '#94a3b8', lineHeight: '2' }}>
                  <div><strong style={{color:'white'}}>{tr('admin.shvrh1')}</strong> {tr('admin.kvtrvtAmvdvtChvbh')}</div>
                  <div><strong style={{color:'white'}}>{tr('admin.amvdvtChvbh')}</strong> <code style={{background:'#334155', padding:'1px 6px', borderRadius:'3px'}}>callSign</code></div>
                  <div><strong style={{color:'white'}}>{tr('admin.amvdvtAvptsyvnlyvt')}</strong></div>
                  <div style={{paddingRight:'16px'}}>
                    <code style={{background:'#334155', padding:'1px 6px', borderRadius:'3px', marginLeft:'8px'}}>sq</code> {tr('admin.tyystGm')} <code style={{background:'#1e293b', padding:'1px 4px', borderRadius:'3px'}}>SQ</code>, <code style={{background:'#1e293b', padding:'1px 4px', borderRadius:'3px'}}>squadron</code>, <code style={{background:'#1e293b', padding:'1px 4px', borderRadius:'3px'}}>{tr('shared.squadron')}</code>)<br/>
                    <code style={{background:'#334155', padding:'1px 6px', borderRadius:'3px', marginLeft:'8px'}}>NUMBEROFFORMATION</code> {tr('admin.msprMarkGm')} <code style={{background:'#1e293b', padding:'1px 4px', borderRadius:'3px'}}>numberOfFormation</code>, <code style={{background:'#1e293b', padding:'1px 4px', borderRadius:'3px'}}>number_of_formation</code>)<br/>
                    <code style={{background:'#334155', padding:'1px 6px', borderRadius:'3px', marginLeft:'8px'}}>alt</code> {tr('admin.gvbh')}<br/>
                    <code style={{background:'#334155', padding:'1px 6px', borderRadius:'3px', marginLeft:'8px'}}>task</code> {tr('admin.mshymh2')}<br/>
                    <code style={{background:'#334155', padding:'1px 6px', borderRadius:'3px', marginLeft:'8px'}}>DATE</code> {tr('admin.tarykHmrahPvrmt')} <code style={{background:'#1e293b', padding:'1px 6px', borderRadius:'3px'}}>DD/MM/YYYY</code> {tr('admin.av')} <code style={{background:'#1e293b', padding:'1px 6px', borderRadius:'3px'}}>DDMMYYYY</code><br/>
                    ...
                    <code style={{background:'#0c4a6e', color:'#7dd3fc', padding:'1px 6px', borderRadius:'3px', marginLeft:'8px'}}>{tr('shared.landingAirfield')}</code> — שם שדה הנחיתה (חייב להתאים לשם בסיס תעופה מוגדר במערכת; גם: <code style={{background:'#1e293b', padding:'1px 4px', borderRadius:'3px'}}>landing_airfield</code>)
                  </div>
                  <div style={{marginTop:'10px', padding:'10px 14px', background:'#0c2218', border:'1px solid #16a34a', borderRadius:'6px', fontSize:'12px', color:'#86efac', lineHeight:'1.7'}}>
                    <strong>{tr('admin.shymvshBamvdtChlkMp')}</strong><br/>
                    {tr('admin.toImportAFormation')} <code style={{background:'#1a3a28', padding:'1px 5px', borderRadius:'3px'}}>BLUE01</code> {tr('admin.bamvdtChlkMpM')}<br/>
                    {tr('admin.aFormationNotBelonging')}
                  </div>
                </div>

                <h4 style={{ margin: '0 0 8px 0', fontSize: '14px', color: '#94a3b8' }}>{tr('admin.dvgmhCsv')}</h4>
...
VIPER07,117,1,FL400,STRIKE,23/03/2026,0945,,GBU12:2; GBU31:1,BRIDGE_A:IP_SOUTH,,`}
                ...
                <h4 style={{ margin: '15px 0 8px 0', fontSize: '14px', color: '#94a3b8' }}>{tr('admin.dvgmhExcel')}</h4>
              ...
              <div style={{ marginTop: '32px', borderTop: '2px solid #334155', paddingTop: '24px' }}>
                ...
                <div style={{ background: '#0f172a', borderRadius: '8px', padding: '20px', marginBottom: '16px' }}>
                  <p style={{ color: '#94a3b8', marginBottom: '12px', fontSize: '13px', lineHeight: '1.7' }}>
                    {tr('admin.loadAircraftDataParking')}<br/>
                    <strong style={{ color: '#f59e0b' }}>{tr('admin.hpmmymChyybymLhyvtKyymym')}</strong><br/>
                    {tr('admin.linkingToAFormation')} <code style={{ background: '#1e293b', padding: '1px 5px', borderRadius: '3px' }}>formation_callsign</code>.
                  </p>

                  <div style={{ marginBottom: '14px', padding: '10px 14px', background: '#1e293b', borderRadius: '6px', fontSize: '12px', color: '#94a3b8', lineHeight: '2' }}>
                    <strong style={{ color: 'white' }}>{tr('admin.amvdvt')}</strong><br/>
                    <code style={{ background: '#334155', padding: '1px 6px', borderRadius: '3px' }}>formation_callsign</code> {tr('admin.avKHpM2')}<br/>
                    <code style={{ background: '#334155', padding: '1px 6px', borderRadius: '3px' }}>idx</code> {tr('admin.msprHmtvsBttsvrh1')}<br/>
                    <code style={{ background: '#334155', padding: '1px 6px', borderRadius: '3px' }}>datk</code> {tr('admin.dtKMsprAvptsyvnly')}<br/>
                    <code style={{ background: '#334155', padding: '1px 6px', borderRadius: '3px' }}>kipa</code> {tr('admin.kyphTkstAvptsyvnly')}<br/>
                    <code style={{ background: '#92400e', color: '#fcd34d', padding: '1px 6px', borderRadius: '3px' }}>armaments</code> {tr('admin.chymvshymTtsvrhPvrmt')} <code style={{ background: '#1e293b', padding: '1px 5px', borderRadius: '3px' }}>שם:כמות; שם2:כמות2</code><br/>
                    <code style={{ background: '#0e3a4a', color: '#67e8f9', padding: '1px 6px', borderRadius: '3px' }}>systems</code> {tr('admin.markvtPvrmt')} <code style={{ background: '#1e293b', padding: '1px 5px', borderRadius: '3px' }}>שם:סטטוס; שם2:סטטוס2</code> {tr('admin.statusServiceablePartialUnserviceable')}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    >{tr('admin.bchrKvbtsCsvExcel')}</button>
                    <button
                      onClick={() => {
                      ...
                      style={{ padding: '12px 20px', background: '#16a34a', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: 'bold' }}
                    >{tr('admin.hvrdTbnytExcel')}</button>
                  </div>
                  ...
                  {acDiag && !acImportResult && (
                    <div style={{ marginTop: '16px', padding: '14px', background: '#1e293b', borderRadius: '8px', fontSize: '12px' }}>
                      <div style={{ marginBottom: '10px', color: '#94a3b8' }}>
                        <strong style={{ color: '#e2e8f0' }}>{tr('admin.columnsFoundInThe')}{acDiag.cols.length}):</strong><br/>
                        <span style={{ fontFamily: 'monospace', color: '#60a5fa' }}>{acDiag.cols.join(' | ') || '—'}</span>
                      </div>
                      <div style={{ marginBottom: '10px', lineHeight: 2 }}>
                        <strong style={{ color: '#e2e8f0' }}>{tr('admin.mypvyShzvhh')}</strong><br/>
                      ...
                      <div style={{ marginBottom: '12px', color: acDiag.mapped.length > 0 ? '#22c55e' : '#f87171' }}>
                        {tr('admin.validRowsDetected')} <strong>{acDiag.mapped.length}</strong> {tr('shared.of')} {acDiag.rowCount}
                      </div>
                      {acDiag.mapped.length > 0 && (
                        <div style={{ marginBottom: '12px' }}>
                          <div style={{ color: '#94a3b8', marginBottom: '5px' }}>{tr('admin.ttsvghMkdymh3Rashvnvt')}</div>
                      ...
                      {(!acDiag.colCallsign || !acDiag.colIdx) ? (
                        <div style={{ color: '#f87171', padding: '8px', background: '#2d0f0f', borderRadius: '5px' }}>
                          {tr('admin.requiredColumnsNotFound')} <strong>formation_callsign</strong> {tr('admin.v')}<strong>idx</strong> {tr('admin.orCallsignNumber')}
                        </div>
                      ) : acDiag.mapped.length === 0 ? (
                        <div style={{ color: '#f87171', padding: '8px', background: '#2d0f0f', borderRadius: '5px' }}>
                          {tr('admin.noRowsWithData')}
                        </div>
                      ) : (
                        <button
                          onClick={async () => {
                        ...
                        >{tr('admin.confirmImport')} {acDiag.mapped.length} {tr('shared.aircraft3')}</button>
                      )}
                    </div>
                  )}
                  ...
                  {acImportResult && (
                    <div style={{ marginTop: '16px', padding: '14px', background: '#1e293b', borderRadius: '8px' }}>
                      {acImportResult.colMap && <div style={{ color: '#64748b', fontSize: '11px', marginBottom: '10px', fontFamily: 'monospace' }}>{tr('admin.columnMapping')} {acImportResult.colMap}</div>}
                      {acImportResult.imported > 0 && <div style={{ color: '#22c55e', fontSize: '15px', marginBottom: '6px' }}>{tr('admin.loaded')} {acImportResult.imported} {tr('shared.aircraft3')}</div>}
                      {acImportResult.skipped > 0 && <div style={{ color: '#94a3b8', fontSize: '13px', marginBottom: '6px' }}>{tr('admin.skipped')} {acImportResult.skipped}</div>}
                      {acImportResult.errors.length > 0 && (
                        <div style={{ color: '#f87171', fontSize: '12px' }}>
                          <strong>{tr('admin.errors')}{acImportResult.errors.length}):</strong>
                          <ul style={{ margin: '4px 0 0 0', paddingRight: '18px' }}>
                            {acImportResult.errors.slice(0, 10).map((err, i) => <li key={i}>{err}</li>)}
                            {acImportResult.errors.length > 10 && <li style={{ color: '#64748b' }}>{tr('admin.andMore')} {acImportResult.errors.length - 10}</li>}
                          </ul>
                        </div>
                      )}
                    </div>
...
CHARLIE,1,301,`}
                ...
                <h4 style={{ margin: '14px 0 8px 0', fontSize: '14px', color: '#94a3b8' }}>{tr('admin.mbnhHkvbts')}</h4>
                <div style={{ background: '#1e293b', borderRadius: '6px', overflow: 'auto' }}>
              ...
              {stripsLoading ? (
                <div style={{ color: '#94a3b8', padding: '20px', textAlign: 'center' }}>{tr('shared.loading')}</div>
              ) : (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', direction: 'rtl' }}>
                    <thead>
                      <tr style={{ background: '#1e293b', color: '#94a3b8', textAlign: 'right' }}>
                        {['קריאה', "מ' מערך", 'טייסת', 'גובה', 'משימה', 'כותרת', 'זמן המראה', 'שדה המראה', 'שדה נחיתה', 'סטטוס', 'סטטוס אוירי', tr('admin.gapiSourceCol'), ''].map((h, i) => (
                          <th key={i} style={{ padding: '8px 10px', fontWeight: '600', borderBottom: '1px solid #334155', whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredGlobalStrips.map((s, idx) => {
          ...
          {activeTab === 'blocks' && (() => {
            ...
            <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
              ...
              <div style={{ width: '240px', flexShrink: 0 }}>
                <h2 style={{ margin: '0 0 14px 0', fontSize: '17px', color: '#e2e8f0' }}>{tr('admin.mrchbyBlvkym')}</h2>
              ...
              {/* Right: Block Tables */}
              <div style={{ flex: 1, minWidth: 0 }}>
                <h2 style={{ margin: '0 0 14px 0', fontSize: '17px', color: '#e2e8f0' }}>{tr('admin.tblavtBlvkym')}</h2>
                {/* New Table Form */}
                <div style={{ background: '#0f172a', borderRadius: '8px', padding: '14px', marginBottom: '16px' }}>
                  <div style={{ color: '#94a3b8', fontSize: '12px', marginBottom: '8px' }}>{tr('admin.tblhChdshh')}</div>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                    <input value={blockTableForm.name} onChange={e => setBlockTableForm(f => ({ ...f, name: e.target.value }))}
                      placeholder={tr('admin.shmHtblh')} style={{ flex: 1, minWidth: '140px', padding: '7px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '13px' }} />
                    <input value={blockTableForm.category} onChange={e => setBlockTableForm(f => ({ ...f, category: e.target.value }))}
                      placeholder={tr('admin.ktgvryhAvptsyvnly')} style={{ flex: 1, minWidth: '120px', padding: '7px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '13px' }} />
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                        <div style={{ flex: 1, minWidth: '140px' }}>
                          <label style={{ display: 'block', marginBottom: '6px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.shmHtblh2')}</label>
                          <input value={blockTableForm.name} onChange={e => setBlockTableForm(f => ({ ...f, name: e.target.value }))}
                            placeholder={tr('admin.shmHtblh')} style={{ width: '100%', padding: '9px 12px', background: '#1e293b', border: '1px solid #475569', borderRadius: '7px', color: 'white', fontSize: '14px', boxSizing: 'border-box' }} />
                        </div>
                        <div style={{ flex: 1, minWidth: '120px' }}>
                          <label style={{ display: 'block', marginBottom: '6px', color: '#94a3b8', fontSize: '13px' }}>{tr('shared.category2')}</label>
                          <input value={blockTableForm.category} onChange={e => setBlockTableForm(f => ({ ...f, category: e.target.value }))}
                            placeholder={tr('admin.ktgvryhAvptsyvnly')} style={{ width: '100%', padding: '9px 12px', background: '#1e293b', border: '1px solid #475569', borderRadius: '7px', color: 'white', fontSize: '14px', boxSizing: 'border-box' }} />
                        </div>
                      </div>
                      <div>
                        <label style={{ display: 'block', marginBottom: '6px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.mrchbBlvkym')}</label>
                        <select value={blockTableForm.block_space_id} onChange={e => setBlockTableForm(f => ({ ...f, block_space_id: e.target.value }))}
                      ...
                      <div>
                        ...
                        <textarea value={blockTableForm.note} onChange={e => setBlockTableForm(f => ({ ...f, note: e.target.value }))}
                          placeholder={tr('admin.harhLtblhAvptsyvnly')} rows={3}
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                                        ...
                                        {space && <span style={{ color: '#64748b', fontSize: '11px' }}>{tr('admin.space')} {space.name}</span>}
                                        {bt.updated_at && <span style={{ color: '#334155', fontSize: '10px' }}>{tr('admin.updated')} {fmtDate(bt.updated_at)}</span>}
                                      </div>
                                          ...
                                          <div key={blk.id} style={{ background: '#0c1a2e', border: `2px solid ${blk.color || '#3b82f6'}`, borderRadius: '5px', padding: '8px 10px' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                ...
                                                {blk.workstations?.length > 0 && <span style={{ color: '#64748b', fontSize: '10px' }}>({blk.workstations.length} {tr('shared.workstations2')}</span>}
                                        ...
                                        <div style={{ background: '#0c1a2e', border: '1px dashed #334155', borderRadius: '5px', padding: '10px' }}>
                                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '6px' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                              <label style={{ color: '#64748b', fontSize: '10px' }}>{tr('admin.gvbhM')}</label>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                              <label style={{ color: '#64748b', fontSize: '10px' }}>{tr('admin.gvbhAd')}</label>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', flex: 1 }}>
                                              <label style={{ color: '#64748b', fontSize: '10px' }}>{tr('admin.mshymh')}</label>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                              <label style={{ color: '#64748b', fontSize: '10px' }}>{tr('shared.color')}</label>
                                          ...
                                          <div style={{ marginBottom: '6px' }}>
                                            <label style={{ color: '#64748b', fontSize: '10px', display: 'block', marginBottom: '4px' }}>{tr('shared.note')}</label>
                                          ...
                                          <div style={{ marginBottom: '6px' }}>
                                            <label style={{ color: '#64748b', fontSize: '10px', display: 'block', marginBottom: '4px' }}>{tr('admin.amdvtShyykvtLblvk')}</label>
                                          ...
                                          <div style={{ marginBottom: '8px' }}>
                                            <label style={{ color: '#64748b', fontSize: '10px', display: 'block', marginBottom: '4px' }}>{tr('admin.pltpvrmvtMvprdBpsyk')}</label>
                                            <input value={(blockForm.platforms as string[]).join(',')} onChange={e => setBlockForm(f => ({ ...f, platforms: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))} placeholder={tr('admin.lmshlF16F')} style={{ width: '100%', padding: '4px', background: '#1e293b', border: '1px solid #334155', borderRadius: '4px', color: 'white', fontSize: '12px' }} />
                                          </div>
                                          <div style={{ display: 'flex', gap: '6px' }}>
                                            <button onClick={async () => {
                                            ...
                                            }} style={{ background: '#1d4ed8', color: 'white', border: 'none', borderRadius: '4px', padding: '5px 12px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>{tr('admin.hvsfBlvk')}</button>
                                            <button onClick={() => { setBlockTableForBlock(null); setBlockForm({ ...emptyBlockForm }); }} style={{ background: '#475569', color: 'white', border: 'none', borderRadius: '4px', padding: '5px 10px', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.cancel')}</button>
                                          </div>
                                        </div>
                                      ) : (
                                        <button onClick={() => { setBlockTableForBlock(bt.id); setBlockForm({ ...emptyBlockForm, color: pickDistinctColor(btBlocks) }); setEditingBlock(null); }} style={{ background: 'transparent', color: '#1d4ed8', border: '1px dashed #1d4ed8', borderRadius: '5px', padding: '6px', cursor: 'pointer', fontSize: '12px', width: '100%' }}>{tr('admin.hvsfBlvkLtblh')}</button>
                                      )}
                                    </div>
                                  </div>
                                </div>
              ...
              <div style={{ display: 'flex', gap: '20px', direction: 'rtl' }}>
                ...
                <div style={{ width: '240px', flexShrink: 0 }}>
                  <div style={{ display: 'flex', gap: '6px', marginBottom: '12px' }}>
                    <input value={bdhSearchAdmin} onChange={e => setBdhSearchAdmin(e.target.value)} placeholder={tr('shared.search')} style={{ flex: 1, padding: '6px 8px', background: '#0f172a', border: '1px solid #334155', borderRadius: '5px', color: 'white', fontSize: '12px', direction: 'rtl' }} />
                    <button onClick={openCreate} style={{ background: '#059669', color: 'white', border: 'none', borderRadius: '5px', padding: '6px 10px', cursor: 'pointer', fontSize: '12px', whiteSpace: 'nowrap' }}>{tr('admin.chdsh')}</button>
                  </div>
                  {categories.map(cat => (
                    <div key={cat} style={{ marginBottom: '10px' }}>
                      ...
                      {filteredBdh.filter((d: any) => (d.category || 'כללי') === cat).map((doc: any) => (
                        <div key={doc.id} onClick={() => openEditBdh(doc)}
                          ...
                          <div style={{ color: '#64748b', fontSize: '10px' }}>{(doc.items || []).length} {tr('shared.items')}</div>
                        </div>
                      ))}
                    </div>
                  ))}
                ...
                {editingBdh ? (
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '12px', flexWrap: 'wrap' }}>
                      ...
                      <div style={{ flex: 1, minWidth: '120px' }}>
                        <label style={labelStyle}>{tr('shared.category')}</label>
                        <input value={bdhForm.category} onChange={e => setBdhForm(f => ({ ...f, category: e.target.value }))} placeholder={tr('admin.ldvgmhTrgylymNhlym')} style={inputStyle} />
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '14px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        <label style={labelStyle}>{tr('admin.saypymVkvtrvt')}</label>
                        <div style={{ display: 'flex', gap: '4px' }}>
                          <button onClick={() => setBdhItemsEdit(prev => [...prev, { content: '', is_header: true, _key: Date.now() + Math.random() }])}
                            style={{ background: '#1e3a5f', color: '#93c5fd', border: '1px solid #334155', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer', fontSize: '11px' }}>{tr('admin.kvtrt')}</button>
                          <button onClick={() => setBdhItemsEdit(prev => [...prev, { content: '', is_header: false, _key: Date.now() + Math.random() }])}
                            style={{ background: '#14432a', color: '#86efac', border: '1px solid #166534', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer', fontSize: '11px' }}>{tr('admin.sayf')}</button>
                        </div>
                      </div>
                      {bdhItemsEdit.length > 0 && (
                        <div style={{ display: 'grid', gridTemplateColumns: '18px 22px 1fr 44px 52px', gap: '0', background: '#0c1626', borderRadius: '4px 4px 0 0', borderBottom: '1px solid #334155', padding: '2px 4px' }}>
                          ...
                          <span style={{ color: '#475569', fontSize: '9px', textAlign: 'center' }}>#</span>
                          <span style={{ color: '#475569', fontSize: '9px' }}>{tr('admin.tvkn')}</span>
                          <span style={{ color: '#475569', fontSize: '9px', textAlign: 'center' }}>{tr('admin.pavlvt')}</span>
                          <span style={{ color: '#475569', fontSize: '9px', textAlign: 'center' }}>{tr('admin.aytsvb')}</span>
                        </div>
                      )}
                          ...
                          >
                            ...
                            {item.is_header ? (
                              <button
                                ...
                                title={tr('admin.hvsfSayfTchtKvtrt')}
                      ...
                      {bdhItemsEdit.length === 0 && <div style={{ color: '#475569', fontSize: '12px', textAlign: 'center', padding: '10px 0' }}>{tr('admin.lchtsKvtrtAvSayf')}</div>}
                    </div>

                    {/* Preset assignment */}
                    <div style={{ marginBottom: '14px', background: '#0f172a', border: '1px solid #1e293b', borderRadius: '6px', padding: '10px' }}>
                      <div style={{ color: '#94a3b8', fontSize: '11px', marginBottom: '8px', fontWeight: 'bold' }}>{tr('admin.shyvkLamdvt')}</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                        {presets.map((p: any) => {
                      ...
                      {editingBdh._new && <div style={{ color: '#475569', fontSize: '10px', marginTop: '6px' }}>{tr('admin.shmvrKvdmLachrMkn')}</div>}
                    </div>
              ...
              <div style={{ display: 'flex', gap: '16px', direction: 'rtl' }}>
                ...
                <div style={{ width: '200px', flexShrink: 0, position: 'relative' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#94a3b8' }}>{tr('admin.templates')}{classicTables.length})</span>
                    <button onClick={() => { setShowNewModePicker(true); setNewCivilTableName(''); }} style={{ padding: '4px 10px', background: '#1d4ed8', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' }}>{tr('admin.chdsh')}</button>
                  </div>
                  ...
                  {showNewModePicker && (
                    <div onClick={() => setShowNewModePicker(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 10002, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div onClick={e => e.stopPropagation()} style={{ background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: '12px', padding: '24px', width: '360px', direction: 'rtl', color: '#e2e8f0', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <div style={{ fontSize: '15px', fontWeight: 'bold', color: '#93c5fd', textAlign: 'center' }}>{tr('admin.bchrSvgTbnytChdshh')}</div>
                        <div style={{ display: 'flex', gap: '12px' }}>
                          ...
                          <button onClick={() => { setShowNewModePicker(false); startNew(); }}
                            ...
                            <span style={{ fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.3Shvrvt')}</span>
                            <span style={{ fontSize: '11px', color: '#64748b', textAlign: 'center' }}>{tr('admin.tbnytStrypRgylAm')}</span>
                          </button>
                          {/* Civil/grid mode */}
                          <div style={{ flex: 1, padding: '16px 8px', background: '#0f172a', border: '2px solid #334155', borderRadius: '10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', color: '#e2e8f0' }}>
                            <span style={{ fontSize: '28px' }}>📐</span>
                            <span style={{ fontSize: '14px', fontWeight: 'bold' }}>{tr('admin.mvdAzrchy')}</span>
                            <span style={{ fontSize: '11px', color: '#64748b', textAlign: 'center' }}>{tr('admin.grydChvpshyAmAvrk')}</span>
                            <input value={newCivilTableName} onChange={e => setNewCivilTableName(e.target.value)}
                              placeholder={tr('admin.shmHtbnyt')}
                              onKeyDown={async e => {
                            ...
                            <button disabled={!newCivilTableName.trim()}
                              ...
                              style={{ width: '100%', padding: '5px 8px', background: newCivilTableName.trim() ? '#1d4ed8' : '#1e293b', border: 'none', borderRadius: '6px', color: newCivilTableName.trim() ? 'white' : '#475569', cursor: newCivilTableName.trim() ? 'pointer' : 'not-allowed', fontSize: '12px', fontWeight: 'bold' }}>{tr('admin.tsvrVptchAvrk')}</button>
                          </div>
                        </div>
                        <button onClick={() => setShowNewModePicker(false)} style={{ alignSelf: 'center', padding: '4px 14px', background: 'transparent', border: '1px solid #334155', borderRadius: '6px', color: '#64748b', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.cancel')}</button>
                      </div>
                    </div>
                  )}
                  ...
                  {classicTables.map((ct: any) => {
                    ...
                    return (
                      <div key={ct.id} style={{ marginBottom: '4px', borderRadius: '6px', background: isSelected ? '#1e3a5f' : '#0f172a', border: `1px solid ${isSelected ? '#3b82f6' : '#1e293b'}`, fontSize: '13px', display: 'flex', alignItems: 'center', gap: '4px', overflow: 'hidden' }}>
                        ...
                        {isCivil && (
                          <button title={tr('admin.shnhShm')} onClick={async e => {
                            e.stopPropagation();
                        ...
                        {/* Delete button */}
                        <button title={tr('admin.mchkTbnyt')} onClick={e => { e.stopPropagation(); deleteTable(ct.id); }}
                  ...
                  {classicTables.length === 0 && <div style={{ color: '#475569', fontSize: '12px', textAlign: 'center', padding: '20px 0' }}>{tr('admin.aynTbnyvt')}</div>}
                ...
                <div style={{ flex: 1, background: '#0f172a', borderRadius: '8px', padding: '18px', border: '1px solid #1e293b' }}>
                  ...
                  <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', marginBottom: '12px' }}>
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div>
                        <label style={{ fontSize: '12px', color: '#64748b', display: 'block', marginBottom: '4px' }}>{tr('admin.shmTbnyt')}</label>
                        <input value={classicTableForm.name} onChange={e => setClassicTableForm(p => ({ ...p, name: e.target.value }))}
                          placeholder={tr('admin.ldvgmhMrchbA')} style={{ width: '100%', padding: '7px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '13px', direction: 'rtl', boxSizing: 'border-box' }} />
                      </div>
                      <div>
                        <label style={{ fontSize: '12px', color: '#64748b', display: 'block', marginBottom: '4px' }}>{tr('admin.tyavrAvptsyvnly')}</label>
                        <input value={classicTableForm.description} onChange={e => setClassicTableForm(p => ({ ...p, description: e.target.value }))}
                          placeholder={tr('admin.harhKtsrh')} style={{ width: '100%', padding: '7px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '13px', direction: 'rtl', boxSizing: 'border-box' }} />
                      </div>
                    </div>
                    <div style={{ flexShrink: 0, width: '140px' }}>
                      <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '4px', textAlign: 'center' }}>{tr('admin.ttsvghMkdymh')}</div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    {classicTableRows.map((row, idx) => {
                      ...
                      <div key={idx} style={{ background: '#1e293b', borderRadius: '7px', padding: '12px', border: '1px solid #334155' }}>
                        <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#60a5fa', marginBottom: '8px' }}>{tr('admin.row')} {row.row_number}</div>

                        {/* Fields list */}
                        <div style={{ marginBottom: '10px' }}>
                          <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '5px' }}>{tr('admin.shdvtBshvrhZv')}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                              ...
                              {fi > 0 && (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '3px 0', direction: 'rtl' }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '2px 8px' }}>
                                    <span style={{ fontSize: '10px', color: '#64748b' }}>{tr('admin.mpryd')}</span>
                              ...
                              <div style={{ background: '#0a1628', borderRadius: '5px', padding: '6px 8px', border: '1px solid #1e293b', marginBottom: fi < activeFields.length - 1 ? '0' : '4px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px' }}>
                                    <span style={{ fontSize: '9px', color: '#64748b' }}>{tr('admin.tkst')}</span>
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px' }}>
                                    ...
                                    <input type="color" value={f.bg_color || '#1e293b'}
                                      ...
                                      title={tr('admin.tsbaRkaShdh')}
                                      style={{ width: '28px', height: '24px', padding: '1px', background: 'transparent', border: '1px solid #334155', borderRadius: '4px', cursor: 'pointer' }} />
                                  </div>
                                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px' }}>
                                    <span style={{ fontSize: '9px', color: '#64748b' }}>{tr('admin.gvdl')}</span>
                            ...
                            <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '2px' }}>
                              <button onClick={() => setRowFields([...activeFields, { field_name: '', separator: ' / ' }])}
                                style={{ padding: '3px 10px', background: '#1d4ed8', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('admin.shdh')}</button>
                            </div>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'flex-end' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.gvdl')}</span>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.yyshvr')}</span>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.tsbaTkst')}</span>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.tsbaRka')}</span>
            ...
            <SettingsModal title={`עריכת בלוק: ${editingBlock.alt_from}–${editingBlock.alt_to} ${editingBlock.mission ? '(' + editingBlock.mission + ')' : ''}`} onClose={() => setEditingBlock(null)}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ color: '#94a3b8', fontSize: '13px' }}>{tr('admin.gvbhM')}</label>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ color: '#94a3b8', fontSize: '13px' }}>{tr('admin.gvbhAd')}</label>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1, minWidth: '120px' }}>
                    <label style={{ color: '#94a3b8', fontSize: '13px' }}>{tr('admin.mshymh')}</label>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ color: '#94a3b8', fontSize: '13px' }}>{tr('shared.color')}</label>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', color: '#94a3b8', fontSize: '13px' }}>{tr('shared.note')}</label>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.amdvtShyykvtLblvkZh')}</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {presets.map((p: any) => (
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', color: '#94a3b8', fontSize: '13px' }}>{tr('admin.pltpvrmvtMvprdvtBpsyk')}</label>
                  <input value={(blockForm.platforms as string[]).join(',')} onChange={e => setBlockForm(f => ({ ...f, platforms: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))} placeholder={tr('admin.lmshlF16F')} style={{ width: '100%', padding: '8px', background: '#1e293b', border: '1px solid #475569', borderRadius: '6px', color: 'white', fontSize: '13px', boxSizing: 'border-box' }} />
                </div>
                <div style={{ display: 'flex', gap: '10px', paddingTop: '4px' }}>
                  ...
                  }} style={{ flex: 1, background: '#059669', color: 'white', border: 'none', borderRadius: '7px', padding: '11px', cursor: 'pointer', fontSize: '15px', fontWeight: 'bold' }}>{tr('admin.shmvrShynvyym')}</button>
                  <button onClick={() => setEditingBlock(null)} style={{ background: '#475569', color: 'white', border: 'none', borderRadius: '7px', padding: '11px 18px', cursor: 'pointer', fontSize: '14px' }}>{tr('shared.cancel')}</button>
                </div>
              </div>
            </SettingsModal>
          )}
              ...
              <div style={{ width: '460px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                  <select
                    ...
                    <option value="" style={{ background: '#1e293b', color: '#94a3b8' }}>{tr('admin.bchrShdhTavph')}</option>
                    {adminAirfields.map((af: any) => (
                  ...
                  {selectedAdminAirfieldId && (<>
                    <button onClick={() => duplicateAirfield(selectedAdminAirfieldId)}
                      title={tr('admin.shkplShdhTavph')}
                      style={{ padding: '6px 8px', background: '#1e3a5f', color: '#7dd3fc', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '10px', flexShrink: 0 }}>{tr('admin.shkpl3')}</button>
                    <button onClick={async () => { if (await customConfirm('למחוק את השדה?')) deleteAirfield(selectedAdminAirfieldId); }}
                      style={{ padding: '6px 8px', background: '#7f1d1d', color: '#fca5a5', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '10px', flexShrink: 0 }}>{tr('shared.delete2')}</button>
                  </>)}
                </div>
                ...
                {showAirfieldForm && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ borderTop: '1px solid #334155', paddingTop: '10px' }}>
                      ...
                      {adminAviationBases.length > 0 && <>
                        <label style={{ display: 'block', color: '#64748b', fontSize: '10px', marginBottom: '4px' }}>{tr('admin.bsysAvptsyvnlyYmlaShm')}</label>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: adminAFExpanded.has('sids') ? '800px' : '0', overflow: 'hidden', transition: 'max-height 0.2s ease' }}>
                          ...
                          <div key={i} style={{ background: '#0f172a', border: '1px solid #1e3a5f', borderRadius: '6px', padding: '6px 8px', marginBottom: '2px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '3px' }}>
                              <select defaultValue="" onChange={e => { const secId = Number(e.target.value); if (!secId) return; setAirfieldForm(p => ({ ...p, sids: p.sids.map((s, j) => j === i && !s.sector_ids.includes(secId) ? { ...s, sector_ids: [...s.sector_ids, secId] } : s) })); e.target.value = ''; }}
                                ...
                                <option value="">{tr('shared.addWorkstation')}</option>
                        ...
                        <div style={{ display: 'flex', gap: '4px', marginTop: '4px' }}>
                          <input value={airfieldForm.newSidLabel} onChange={e => setAirfieldForm(p => ({ ...p, newSidLabel: e.target.value }))}
                            placeholder={tr('admin.shmSidLdvgAlpha')}
                          ...
                          <select value={airfieldForm.newSid} onChange={e => setAirfieldForm(p => ({ ...p, newSid: e.target.value }))}
                            ...
                            <option value="">{tr('admin.amdhAvptsyvnly')}</option>
                            {sectors.map((s: any) => (
                              <option key={s.id} value={String(s.id)}>{s.name}</option>
                            ))}
                          </select>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: adminAFExpanded.has('stars') ? '800px' : '0', overflow: 'hidden', transition: 'max-height 0.2s ease' }}>
                        ...
                        <div style={{ display: 'flex', gap: '4px', marginTop: '2px' }}>
                          <input value={airfieldForm.newStar} onChange={e => setAirfieldForm(p => ({ ...p, newStar: e.target.value }))}
                            ...
                            placeholder={tr('admin.shmStar')} style={{ flex: 1, padding: '3px 6px', background: '#0f172a', border: '1px solid #1e3a5f', borderRadius: '4px', color: 'white', fontSize: '11px', fontFamily: 'monospace' }} />
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', maxHeight: adminAFExpanded.has('runways') ? '3000px' : '0', overflow: 'hidden', transition: 'max-height 0.3s ease' }}>
                          ...
                          {adminRunwayForm !== null && (
                            <div style={{ background: '#0f172a', padding: '8px', borderRadius: '6px', marginBottom: '4px', border: '1px solid #166534', direction: 'rtl' }}>
                              ...
                              {/* Overall runway name */}
                              <div style={{ marginBottom: '8px' }}>
                                <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '3px' }}>{tr('admin.shmMslvlKvllShny')}</div>
                              ...
                              {/* Length row — shared */}
                              <div style={{ marginBottom: '8px' }}>
                                <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '3px' }}>{tr('admin.avrkMslvl')}</div>
                              ...
                              <div style={{ background: '#0a1e35', borderRadius: '5px', padding: '6px', marginBottom: '6px', border: '1px solid #1e3a5f' }}>
                                <div style={{ fontSize: '10px', color: '#60a5fa', fontWeight: 'bold', marginBottom: '5px' }}>{tr('admin.tsdA')}</div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px', marginBottom: '5px' }}>
                                  <div>
                                    <div style={{ fontSize: '9px', color: '#64748b', marginBottom: '2px' }}>{tr('shared.name')}</div>
                                  ...
                                  <div style={{ position: 'relative' }}>
                                    <div style={{ fontSize: '9px', color: '#64748b', marginBottom: '2px' }}>{tr('admin.kyvvnAmyty')}</div>
                                ...
                                <div style={{ marginBottom: '5px', background: '#071526', borderRadius: '4px', padding: '5px', border: '1px solid #92400e' }}>
                                  <div style={{ fontSize: '9px', color: '#fcd34d', fontWeight: 'bold', marginBottom: '4px' }}>{tr('admin.mrchkymMvtshrymIcao')}</div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px' }}>
                                  ...
                                  <div>
                                    <div style={{ fontSize: '9px', color: '#64748b', marginBottom: '2px' }}>{tr('admin.mykvmSyvmMslvlA')}</div>
                              ...
                              <div style={{ background: '#1a0e2e', borderRadius: '5px', padding: '6px', marginBottom: '8px', border: '1px solid #3b1e5f' }}>
                                <div style={{ fontSize: '10px', color: '#c084fc', fontWeight: 'bold', marginBottom: '5px' }}>{tr('admin.tsdB')}</div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px', marginBottom: '5px' }}>
                                  <div>
                                    <div style={{ fontSize: '9px', color: '#64748b', marginBottom: '2px' }}>{tr('shared.name')}</div>
                              ...
                              <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                                <button onClick={() => { setAdminRunwayForm(null); setAdminRunwayEditId(null); setPlacingRunwayEndpoint(null); }} style={{ padding: '4px 10px', background: '#1e293b', color: '#94a3b8', border: '1px solid #334155', borderRadius: '4px', cursor: 'pointer', fontSize: '10px' }}>{tr('shared.cancel')}</button>
                                <button onClick={async () => {
                                ...
                                }} style={{ padding: '4px 12px', background: '#16a34a', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('shared.save')}</button>
                              </div>
                            </div>
                          )}
                          {/* Runway list */}
                                  ...
                                  return (
                                    <div style={{ marginTop: '6px', borderTop: '1px solid #92400e', paddingTop: '5px', direction: 'rtl' }}>
                                      ...
                                      <div style={{ display: 'grid', gridTemplateColumns: hasA && hasB ? '1fr 1fr' : '1fr', gap: '6px' }}>
                                        {hasA && (
                                          <div>
                                            <div style={{ fontSize: '8px', color: '#60a5fa', fontWeight: 'bold', marginBottom: '3px' }}>{tr('admin.sideA')}{rw.heading_a || 'A'})</div>
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px' }}>
                                        ...
                                        {hasB && (
                                          <div>
                                            <div style={{ fontSize: '8px', color: '#c084fc', fontWeight: 'bold', marginBottom: '3px' }}>{tr('admin.sideB')}{rw.heading_b || 'B'})</div>
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px' }}>
                    ...
                    {(editingAirfield || selectedAdminAirfieldId) && (
                      <div style={{ borderTop: '1px solid #334155', paddingTop: '10px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: adminAFExpanded.has('cameras') ? '6px' : 0, cursor: 'pointer' }} onClick={() => toggleAFSec('cameras')}>
                          <div style={{ color: '#67e8f9', fontSize: '11px', fontWeight: 'bold', flex: 1 }}>{tr('admin.cameras')}{adminAirfieldElements.filter(e => e.category === 'camera').length})</div>
                          ...
                          {adminAFExpanded.has('cameras') && !showAdminCameraForm && (
                            <button onClick={e => { e.stopPropagation(); setAdminCameraForm({ name: '', camera_url: '' }); setShowAdminCameraForm(true); }}
                              style={{ padding: '2px 8px', background: '#0e7490', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('admin.mtslmh')}</button>
                          )}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', maxHeight: adminAFExpanded.has('cameras') ? '2000px' : '0', overflow: 'hidden', transition: 'max-height 0.2s ease' }}>
                          {showAdminCameraForm && (
                            <div style={{ background: '#0f172a', padding: '8px', borderRadius: '6px', marginBottom: '4px', border: '1px solid #155e75' }}>
                              <input type="text" placeholder={tr('admin.shmHmtslmh')} value={adminCameraForm.name}
                              ...
                              <input type="text" placeholder={tr('admin.ktvbtUrlShlHmtslmh')} value={adminCameraForm.camera_url}
                              ...
                              <div style={{ display: 'flex', gap: '5px' }}>
                                ...
                                }} style={{ flex: 1, padding: '4px', background: '#0e7490', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>{tr('admin.shmvr')}</button>
                                <button onClick={() => { setShowAdminCameraForm(false); setAdminCameraForm({ name: '', camera_url: '' }); }}
                                  style={{ padding: '4px 8px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.cancel')}</button>
                              </div>
                            </div>
                          )}
                          {adminAirfieldElements.filter(e => e.category === 'camera').length === 0 && !showAdminCameraForm
                            ? <div style={{ color: '#475569', fontSize: '11px', textAlign: 'center', padding: '6px 0' }}>{tr('admin.aynMtslmvt')}</div>
                            : adminAirfieldElements.filter(e => e.category === 'camera').map(cam => (
                              <div key={cam.id} style={{ background: '#0f172a', borderRadius: '4px', border: `1px solid ${placingElementId === cam.id ? '#67e8f9' : '#155e75'}`, padding: '5px 7px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                                  ...
                                  {cam.x_pct != null && <span title={tr('admin.mmvkmAlHmph')} style={{ fontSize: '9px', color: '#22d3ee' }}>📍</span>}
                                  ...
                                  {cam.x_pct != null && (
                                    <button onClick={async () => { await fetch(`${API_URL}/airfield-elements/${cam.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ element_type_id: cam.element_type_id, name: cam.name, status: cam.status, note: cam.note, category: cam.category, x_pct: null, y_pct: null, camera_url: cam.camera_url }) }); setAdminAirfieldElements(prev => prev.map(e => e.id === cam.id ? { ...e, x_pct: null, y_pct: null } : e)); }}
                                      style={{ padding: '1px 5px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '9px' }}>{tr('shared.remove')}</button>
                                  )}
                                ...
                                <input type="text" placeholder={tr('admin.ktvbtUrl')} value={cam.camera_url || ''}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', maxHeight: adminAFExpanded.has('elements') ? '2000px' : '0', overflow: 'hidden', transition: 'max-height 0.2s ease' }}>
                              ...
                              return (
                                <div key={el.id} style={{ background: '#0f172a', borderRadius: '4px', border: `1px solid ${placingElementId === el.id ? '#ec4899' : '#1e3a5f'}`, padding: '4px 6px' }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                    ...
                                    }} title={tr('admin.lchtsLmabrLmtsbHba')} style={{ fontSize: '9px', background: (STATUS_COLOR_ADM[el.status] || '#94a3b8') + '22', color: STATUS_COLOR_ADM[el.status] || '#94a3b8', border: `1px solid ${STATUS_COLOR_ADM[el.status] || '#94a3b8'}`, borderRadius: '3px', padding: '0 5px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                      {el.status || '?'}
                                    </button>
                                  </div>
                                  ...
                                  <div style={{ display: 'flex', gap: '2px', marginTop: '4px', flexWrap: 'wrap' }}>
                                    <span style={{ fontSize: '8px', color: '#475569', alignSelf: 'center', marginLeft: '2px' }}>{tr('admin.mtsb')}</span>
                                  ...
                                  <div style={{ display: 'flex', gap: '3px', marginTop: '4px' }}>
                                    ...
                                    {el.x_pct != null && <button onClick={async () => { await adminSaveEl({ x_pct: null, y_pct: null }); }} style={{ padding: '2px 5px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '9px' }}>{tr('admin.hsrMykvm')}</button>}
                                    <button
                              ...
                              <div style={{ background: '#0c1a2e', borderRadius: '8px', border: '2px solid #ec4899', overflow: 'hidden', direction: 'rtl' }}>
                                ...
                                <div style={{ padding: '6px 8px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                  ...
                                  <div style={{ borderRadius: '5px', border: `1px solid ${adminElemFocusField === 'name' ? '#ec4899' : '#1e3a5f'}`, overflow: 'hidden' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 8px', background: '#0f172a', cursor: 'pointer' }}
                                      ...
                                      <span style={{ fontSize: '11px', color: '#94a3b8', whiteSpace: 'nowrap' }}>{tr('admin.shm2')}</span>
                                    ...
                                    {adminElemFocusField === 'name' && (
                                      <div style={{ padding: '6px 8px', background: '#1e293b', borderTop: '1px solid #1e3a5f' }}>
                                        <input autoFocus value={elementForm.name}
                                          ...
                                          placeholder={tr('admin.shmHalmnt')}
                                  ...
                                  <div style={{ borderRadius: '5px', border: `1px solid ${adminElemFocusField === 'category' ? '#a855f7' : '#1e3a5f'}`, overflow: 'hidden' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 8px', background: '#0f172a', cursor: 'pointer' }}
                                      ...
                                      <span style={{ fontSize: '11px', color: '#94a3b8', whiteSpace: 'nowrap' }}>{tr('admin.ktgvryh2')}</span>
                                  ...
                                  <div style={{ borderRadius: '5px', border: `1px solid ${adminElemFocusField === 'type' ? '#f59e0b' : '#1e3a5f'}`, overflow: 'hidden' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 8px', background: '#0f172a', cursor: 'pointer' }}
                                      ...
                                      <span style={{ fontSize: '11px', color: '#94a3b8', whiteSpace: 'nowrap' }}>{tr('admin.svg')}</span>
                                      {selType ? (
                                        ...
                                        <span style={{ flex: 1, fontSize: '12px', color: '#475569' }}>{tr('admin.llaSvg')}</span>
                                      )}
                                  ...
                                  <div style={{ borderRadius: '5px', border: `1px solid ${selStatus.color}44`, background: '#0f172a', padding: '5px 8px' }}>
                                    <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '5px' }}>{tr('admin.sttvs')}</div>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                  ...
                                  <div style={{ borderRadius: '5px', border: `1px solid ${adminElemFocusField === 'note' ? '#64748b' : '#1e3a5f'}`, overflow: 'hidden' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 8px', background: '#0f172a', cursor: 'pointer' }}
                                      ...
                                      <span style={{ fontSize: '11px', color: '#94a3b8', whiteSpace: 'nowrap' }}>{tr('admin.harh2')}</span>
                                    ...
                                    {adminElemFocusField === 'note' && (
                                      <div style={{ padding: '6px 8px', background: '#1e293b', borderTop: '1px solid #1e3a5f' }}>
                                        <textarea autoFocus value={elementForm.note}
                                          ...
                                          placeholder={tr('shared.noteOptional')}
                                          rows={2}
                                ...
                                <div style={{ padding: '6px 8px', borderTop: '1px solid #1e3a5f' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '5px', cursor: 'pointer' }}
                                    ...
                                    <span style={{ fontSize: '11px', color: '#f59e0b', fontWeight: 'bold' }}>{tr('admin.bdyktTpavl')}</span>
                                    {(elementForm.relevant_routes.length > 0 || elementForm.blocking_statuses.length > 0) && (
                                      <span style={{ fontSize: '9px', background: '#78350f', color: '#fde68a', borderRadius: '3px', padding: '1px 4px' }}>
                                        {elementForm.relevant_routes.length} {tr('admin.runway')}{elementForm.relevant_routes.length !== 1 ? 'ות' : ''} · {elementForm.blocking_statuses.length} {tr('admin.status')}{elementForm.blocking_statuses.length !== 1 ? 'ים' : ''}
                                      </span>
                                    )}
                                  ...
                                  {(adminElemFocusField as any) === 'opcheck' && (
                                    <div style={{ background: '#1a1000', borderRadius: '5px', border: '1px solid #78350f', padding: '7px 8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                      {/* Relevant routes */}
                                      <div>
                                        <div style={{ fontSize: '10px', color: '#fbbf24', marginBottom: '4px', fontWeight: 'bold' }}>{tr('admin.mslvlymRlvvntyymHrkbAvbr')}</div>
                                        {adminAirfieldRoutes.filter((r: any) => Number(r.airfield_id) === Number(selectedAdminAirfieldId)).length === 0
                                          ? <div style={{ fontSize: '10px', color: '#475569' }}>{tr('admin.aynMslvlymMvgdrymBshdh')}</div>
                                          : <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                      ...
                                      {/* Blocking statuses */}
                                      <div>
                                        <div style={{ fontSize: '10px', color: '#fbbf24', marginBottom: '4px', fontWeight: 'bold' }}>{tr('admin.sttvsymMpryaymMtsytymHtrah')}</div>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                          {(() => {
                                ...
                                <div style={{ padding: '6px 8px', borderTop: '1px solid #1e3a5f' }}>
                                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', direction: 'rtl' }}>
                                    ...
                                    <span style={{ fontSize: '12px', color: elementForm.show_in_driver ? '#4ade80' : '#94a3b8', fontWeight: elementForm.show_in_driver ? 'bold' : 'normal' }}>
                                      {tr('admin.showEssentialsToThe')}
                                    </span>
                                    {elementForm.show_in_driver && (
                                      <span style={{ fontSize: '10px', color: '#16a34a', background: '#14532d', padding: '1px 6px', borderRadius: '8px', border: '1px solid #16a34a' }}>{tr('admin.payl')}</span>
                                    )}
                                  </label>
                                </div>
                          ...
                          {!showElementForm && adminAirfieldElements.length === 0 && (
                            <div style={{ textAlign: 'center', padding: '10px 0', color: '#64748b', fontSize: '11px' }}>
                              {tr('admin.noElementsYetClick')} <span style={{ color: '#ec4899', fontWeight: 'bold' }}>{tr('shared.add')}</span> {tr('admin.toAdd')}
                            </div>
                          )}
                    ...
                    <div style={{ borderTop: '1px solid #334155', paddingTop: '6px', paddingBottom: '2px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', marginBottom: adminAFExpanded.has('points') ? '4px' : 0 }} onClick={() => toggleAFSec('points')}>
                      <div style={{ color: '#64748b', fontSize: '11px', fontWeight: 'bold' }}>{tr('admin.points2')}{airfieldPoints.filter((p: any) => p.point_type !== 'admin_loc').length})</div>
                      {(() => {
                        ...
                        return (
                          <button onClick={async (e) => {
                            ...
                            style={{ padding: '1px 6px', background: '#064e3b', color: '#34d399', border: '1px solid #065f46', borderRadius: '3px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold', marginLeft: '4px', flexShrink: 0 }}>{tr('admin.agnNTs')}</button>
                        );
                      })()}
                    ...
                    {adminAFExpanded.has('points') && hasMap && (
                      <>
                        <div style={{ borderTop: '1px solid #33415544', paddingTop: '8px' }}>
                          <div style={{ color: '#94a3b8', fontSize: '11px', fontWeight: 'bold', marginBottom: '6px' }}>{tr('admin.nkvdhChdshh')}</div>
                          <input value={airfieldPointForm.name} onChange={e => setAirfieldPointForm(p => ({ ...p, name: e.target.value }))} placeholder={tr('admin.shmHnkvdh')}
                          ...
                          <select value={airfieldPointForm.point_type} onChange={e => setAirfieldPointForm(p => ({ ...p, point_type: e.target.value }))}
                            ...
                            <option value=''>{tr('admin.ktgvryhAvptsyvnly')}</option>
                            <option value='alignment'>{tr('admin.nkvdtHtyyshvrt')}</option>
                          ...
                          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '6px' }}>
                            <div>
                              <div style={{ color: '#64748b', fontSize: '10px', marginBottom: '2px' }}>{tr('shared.color')}</div>
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ color: '#64748b', fontSize: '10px', marginBottom: '2px' }}>{tr('admin.sml')}</div>
                          ...
                          {selectedAdminAirfieldId
                            ...
                            : <div style={{ color: '#f59e0b', fontSize: '11px', textAlign: 'center' }}>{tr('admin.shmvrTchylh')}</div>
                          }
                          {placingPointMode && <div style={{ marginTop: '3px', color: '#fbbf24', fontSize: '10px', textAlign: 'center' }}>{tr('admin.escLbytvl')}</div>}
                        </div>

                        {/* Points list */}
                        <div>
                          <div style={{ color: '#64748b', fontSize: '11px', fontWeight: 'bold', marginBottom: '4px' }}>{tr('admin.points')}{airfieldPoints.length}):</div>
                          {airfieldPoints.length === 0
                            ? <p style={{ color: '#475569', fontSize: '11px', textAlign: 'center', margin: 0 }}>{tr('admin.aynNkvdvt')}</p>
                              ...
                              return (
                                <div key={pt.id} style={{ marginBottom: '4px', borderRadius: '6px', border: `1px solid ${isEditing ? '#3b82f6' : (pt.color || '#1e293b') + '44'}`, overflow: 'hidden' }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '4px 7px', background: '#0f172a' }}>
                                    ...
                                    <button onClick={() => deletePoint(pt.id)} style={{ padding: '1px 5px', background: '#dc2626', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '10px' }}>{tr('shared.delete2')}</button>
                                  </div>
                                  {/* Inline edit form */}
                                  {isEditing && editingPoint && (
                                    <div style={{ padding: '8px', background: '#0a1628', borderTop: '1px solid #1e3a5f', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                      <input
                                        ...
                                        placeholder={tr('admin.shmHnkvdh')}
                                      ...
                                      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                        ...
                                        <div style={{ flex: 1 }} />
                                        <label style={{ fontSize: '10px', color: '#94a3b8' }}>{tr('shared.color2')}</label>
                                      ...
                                      <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                                        <button onClick={() => setEditingPoint(null)}
                                          style={{ padding: '4px 10px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.cancel')}</button>
                                        <button onClick={saveEditingPoint} disabled={!editingPoint.name.trim()}
                                          style={{ padding: '4px 12px', background: editingPoint.name.trim() ? '#1d4ed8' : '#1e293b', color: 'white', border: 'none', borderRadius: '4px', cursor: editingPoint.name.trim() ? 'pointer' : 'not-allowed', fontSize: '11px', fontWeight: 'bold', opacity: editingPoint.name.trim() ? 1 : 0.5 }}>{tr('shared.save')}</button>
                                      </div>
                                    </div>
                                  )}
                                </div>
                    ...
                    {selectedAdminAirfieldId && (
                      <div style={{ borderTop: '1px solid #334155', paddingTop: '6px', paddingBottom: '2px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: adminAFExpanded.has('admin_locs') ? '4px' : 0, cursor: 'pointer' }} onClick={() => toggleAFSec('admin_locs')}>
                          <div style={{ color: '#34d399', fontSize: '11px', fontWeight: 'bold', flex: 1 }}>{tr('admin.administrativePoints')}{airfieldPoints.filter((p: any) => p.point_type === 'admin_loc').length})</div>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
                              <input
                                ...
                                placeholder={tr('admin.shmMkvmEnter')}
                            ...
                            {airfieldPoints.filter((p: any) => p.point_type === 'admin_loc').length === 0
                              ? <div style={{ color: '#475569', fontSize: '11px', padding: '6px 0' }}>{tr('admin.aynNkvdvtMnhltyvtAdyyn')}</div>
                                ...
                                return (
                                  <div key={pt.id} style={{ display: 'flex', flexDirection: 'column', gap: '2px', background: '#0f172a', border: `1px solid ${isEdit ? '#34d399' : '#1e293b'}`, borderRadius: '5px', padding: '4px 7px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                      ...
                                      {isEdit && (
                                        <button onClick={async () => {
                                          ...
                                          const pts = await fetch(`${API_URL}/airfields/${selectedAdminAirfieldId}/points`).then(r => r.json());
                                          setAirfieldPoints(pts); setEditingAdminLoc(null);
                                        }} style={{ padding: '1px 6px', background: '#065f46', border: '1px solid #34d399', borderRadius: '3px', cursor: 'pointer', fontSize: '10px', color: '#6ee7b7', fontWeight: 'bold' }}>{tr('shared.save')}</button>
                                      )}
                                      <button onClick={async () => {
                                    ...
                                    {(pt.lat != null && pt.lng != null)
                                      ...
                                      : pt.x_pct != null && <div style={{ color: '#475569', fontSize: '8px', paddingRight: '22px' }}>📍 ({Math.round(pt.x_pct)}%, {Math.round(pt.y_pct)}{tr('admin.noGps')}</div>
                                    }
                                  </div>
                                );
                              })
                    ...
                    {selectedAdminAirfieldId && (
                      <div style={{ borderTop: '1px solid #334155', paddingTop: '10px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: adminAFExpanded.has('statustypes') ? '6px' : 0, cursor: 'pointer' }} onClick={() => toggleAFSec('statustypes')}>
                          <div style={{ color: '#fb923c', fontSize: '11px', fontWeight: 'bold', flex: 1 }}>{tr('admin.svgySttvsMbtsay')}</div>
                          {adminAFExpanded.has('statustypes') && <button onClick={e => { e.stopPropagation(); setEditingStatusType(null); setStatusTypeForm({ name: '', color: '#6b7280' }); setShowStatusTypeForm(true); }}
                            style={{ padding: '2px 8px', background: '#c2410c', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('shared.add')}</button>}
                        ...
                        {showStatusTypeForm && (
                          <div style={{ background: '#0f172a', borderRadius: '6px', border: '1px solid #c2410c', padding: '8px', marginBottom: '6px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            <input value={statusTypeForm.name} onChange={e => setStatusTypeForm(p => ({ ...p, name: e.target.value }))} placeholder={tr('admin.shmSttvsLdvgShmysh')}
                              style={{ padding: '5px 8px', background: '#1e293b', border: '1px solid #475569', borderRadius: '5px', color: 'white', fontSize: '12px', direction: 'rtl' }} />
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <label style={{ fontSize: '10px', color: '#64748b' }}>{tr('shared.color2')}</label>
                            ...
                            <div style={{ display: 'flex', gap: '5px' }}>
                              ...
                              }} style={{ flex: 1, padding: '4px', background: '#c2410c', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>{tr('shared.save')}</button>
                              <button onClick={() => { setShowStatusTypeForm(false); setEditingStatusType(null); }} style={{ padding: '4px 10px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.cancel')}</button>
                            </div>
                          </div>
                        )}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                          {adminAirfieldStatusTypes.length === 0
                            ? <div style={{ color: '#475569', fontSize: '11px', textAlign: 'center', padding: '6px' }}>{tr('admin.aynSttvsymHvsfRashvn')}</div>
                            : adminAirfieldStatusTypes.map(st => (
                              <div key={st.id} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#0f172a', borderRadius: '4px', padding: '4px 7px', border: `1px solid ${st.color}44` }}>
                                ...
                                <button title={tr('admin.shkpl')} onClick={() => { setEditingStatusType(null); setStatusTypeForm({ name: `${st.name} (העתק)`, color: st.color }); setShowStatusTypeForm(true); }}
                    ...
                    {selectedAdminAirfieldId && (
                      <div style={{ borderTop: '1px solid #334155', paddingTop: '10px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: adminAFExpanded.has('polygons') ? '6px' : 0, cursor: 'pointer' }} onClick={() => toggleAFSec('polygons')}>
                          <div style={{ color: '#a78bfa', fontSize: '11px', fontWeight: 'bold', flex: 1 }}>{tr('admin.azvrymVmslvlymPvlygvnym')}</div>
                          ...
                          {adminAFExpanded.has('polygons') && <button onClick={e => { e.stopPropagation(); setEditingPolygon(null); setPolygonForm({ name: '', color: '#a78bfa', notes: '', parent_id: '' }); setShowPolygonForm(true); }}
                            style={{ padding: '2px 8px', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('shared.add')}</button>}
                        ...
                        {showPolygonForm && (
                          <div style={{ background: '#0f172a', borderRadius: '6px', border: '1px solid #7c3aed', padding: '8px', marginBottom: '6px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            <input value={polygonForm.name} onChange={e => setPolygonForm(p => ({ ...p, name: e.target.value }))} placeholder={tr('admin.shmHazvrLdvgMslvl')}
                              style={{ padding: '5px 8px', background: '#1e293b', border: '1px solid #475569', borderRadius: '5px', color: 'white', fontSize: '12px', direction: 'rtl' }} />
                            <textarea value={polygonForm.notes} onChange={e => setPolygonForm(p => ({ ...p, notes: e.target.value }))} placeholder={tr('shared.noteOptional')} rows={2}
                            ...
                            <select value={polygonForm.parent_id} onChange={e => setPolygonForm(p => ({ ...p, parent_id: e.target.value }))}
                              ...
                              <option value="">{tr('admin.pvlygvnRashyLlaHvrh')}</option>
                              {adminAirfieldPolygons.filter(p => !p.parent_id).map(p => (
                                <option key={p.id} value={p.id}>{p.name} {tr('admin.subPolygon')}</option>
                              ))}
                            </select>
                            <div style={{ display: 'flex', gap: '5px' }}>
                              {hasMap && (
                                ...
                                }} style={{ flex: 1, padding: '4px', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>{tr('admin.tsyyrAlMph')}</button>
                              )}
                              <button onClick={async () => {
                                if (!polygonForm.name.trim()) return;
                              ...
                              }} style={{ padding: '4px 10px', background: '#059669', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>{tr('shared.save')}</button>
                              <button onClick={() => { setShowPolygonForm(false); setEditingPolygon(null); }} style={{ padding: '4px 8px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.cancel')}</button>
                            </div>
                          </div>
                        )}
                        {drawingPolygonId && (
                          <div style={{ background: '#2d1657', border: '1px solid #7c3aed', borderRadius: '5px', padding: '5px 8px', marginBottom: '5px', fontSize: '11px', color: '#c4b5fd' }}>
                            {tr('admin.polygonDrawing')} {polygonDraftPoints.length} {tr('admin.pointsDoubleClickTo2')}
                            {polygonDraftPoints.length >= 3 && (
                              ...
                              }} style={{ marginRight: '8px', padding: '2px 8px', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('admin.shmvr')}</button>
                            )}
                            <button onClick={() => { setDrawingPolygonId(null); setPolygonDraftPoints([]); }} style={{ marginRight: '4px', padding: '2px 8px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '10px' }}>{tr('shared.cancel')}</button>
                          </div>
                        )}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                          {adminAirfieldPolygons.length === 0
                            ? <div style={{ color: '#475569', fontSize: '11px', textAlign: 'center', padding: '6px' }}>{tr('admin.aynPvlygvnymAdyyn')}</div>
                            : adminAirfieldPolygons.map(pg => (
                              <div key={pg.id} style={{ background: '#0f172a', borderRadius: '4px', border: `1px solid ${pg.color}55`, padding: '4px 7px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                  ...
                                  <span style={{ fontSize: '9px', color: '#64748b' }}>{(pg.polygon || []).length} {tr('admin.pts2')}</span>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: '3px', marginTop: '4px' }}>
                                  <button onClick={() => { setDrawingPolygonId(pg.id); setPolygonDraftPoints(Array.isArray(pg.polygon) ? [...pg.polygon] : []); }}
                                    style={{ flex: 1, padding: '2px', background: '#4c1d95', color: '#c4b5fd', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '9px' }}>{tr('admin.tsyyr')}</button>
                                  <button onClick={() => { setEditingPolygon(pg); setPolygonForm({ name: pg.name, color: pg.color || '#a78bfa', notes: pg.notes || '', parent_id: pg.parent_id ? String(pg.parent_id) : '' }); setShowPolygonForm(true); }}
                                    style={{ padding: '2px 5px', background: '#1e3a5f', color: '#93c5fd', border: '1px solid #3b82f6', borderRadius: '3px', cursor: 'pointer', fontSize: '9px' }}>{tr('admin.arvk')}</button>
                                  <button title={tr('admin.shkpl')} onClick={() => { setEditingPolygon(null); setPolygonForm({ name: `${pg.name} (העתק)`, color: pg.color || '#a78bfa', notes: pg.notes || '', parent_id: pg.parent_id ? String(pg.parent_id) : '' }); setShowPolygonForm(true); }}
                    ...
                    {selectedAdminAirfieldId && (
                      <div style={{ borderTop: '1px solid #334155', paddingTop: '10px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: adminAFExpanded.has('sectors') ? '6px' : 0, cursor: 'pointer' }} onClick={() => toggleAFSec('sectors')}>
                          <div style={{ color: '#34d399', fontSize: '11px', fontWeight: 'bold', flex: 1 }}>{tr('admin.sktvryMphAzvryZvm')}</div>
                          ...
                          {adminAFExpanded.has('sectors') && <button onClick={e => { e.stopPropagation(); setEditingAirfieldSector(null); setAirfieldSectorForm({ name: '', notes: '' }); setShowAirfieldSectorForm(true); }}
                            style={{ padding: '2px 8px', background: '#059669', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('shared.add')}</button>}
                        ...
                        {showAirfieldSectorForm && (
                          <div style={{ background: '#0f172a', borderRadius: '6px', border: '1px solid #059669', padding: '8px', marginBottom: '6px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            <input value={airfieldSectorForm.name} onChange={e => setAirfieldSectorForm(p => ({ ...p, name: e.target.value }))} placeholder={tr('admin.shmHsktvrLdvgTspvn')}
                              style={{ padding: '5px 8px', background: '#1e293b', border: '1px solid #475569', borderRadius: '5px', color: 'white', fontSize: '12px', direction: 'rtl' }} />
                            <textarea value={airfieldSectorForm.notes} onChange={e => setAirfieldSectorForm(p => ({ ...p, notes: e.target.value }))} placeholder={tr('shared.noteOptional')} rows={2}
                              style={{ padding: '5px 8px', background: '#1e293b', border: '1px solid #475569', borderRadius: '5px', color: 'white', fontSize: '11px', direction: 'rtl', resize: 'none' }} />
                            {hasMap && <div style={{ fontSize: '10px', color: '#34d399', background: '#064e3b', border: '1px solid #059669', borderRadius: '4px', padding: '4px 8px' }}>{tr('admin.achryShmyrhGrvrRybva')}</div>}
                            <div style={{ display: 'flex', gap: '5px' }}>
                              <button onClick={async () => {
                              ...
                              }} style={{ flex: 1, padding: '4px', background: '#059669', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>{tr('shared.save')}</button>
                              <button onClick={() => { setShowAirfieldSectorForm(false); setEditingAirfieldSector(null); }} style={{ padding: '4px 10px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.cancel')}</button>
                            </div>
                          </div>
                        )}
                        {drawingSectorId && (
                          <div style={{ background: '#064e3b', border: '1px solid #059669', borderRadius: '5px', padding: '5px 8px', marginBottom: '5px', fontSize: '11px', color: '#6ee7b7' }}>
                            {tr('admin.grvrAlHmphLtsyvr')}
                          </div>
                        )}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                          {adminAirfieldSectors.length === 0
                            ? <div style={{ color: '#475569', fontSize: '11px', textAlign: 'center', padding: '6px' }}>{tr('admin.aynSktvrymAdyyn')}</div>
                              ...
                              <div key={sec.id} style={{ background: '#0f172a', borderRadius: '4px', border: '1px solid #05966955', padding: '4px 7px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '3px', marginTop: '4px' }}>
                                  <button onClick={() => { setDrawingSectorId(sec.id); sectorDragStartRef.current = null; }}
                                    style={{ flex: 1, padding: '2px', background: '#064e3b', color: '#6ee7b7', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '9px' }}>{tr('admin.shrttMchdsh')}</button>
                                  <button onClick={() => { setEditingAirfieldSector(sec); setAirfieldSectorForm({ name: sec.name, notes: sec.notes || '' }); setShowAirfieldSectorForm(true); }}
                                    style={{ padding: '2px 5px', background: '#1e3a5f', color: '#93c5fd', border: '1px solid #3b82f6', borderRadius: '3px', cursor: 'pointer', fontSize: '9px' }}>{tr('admin.arvk')}</button>
                                  <button title={tr('admin.shkpl')} onClick={() => { setEditingAirfieldSector(null); setAirfieldSectorForm({ name: `${sec.name} (העתק)`, notes: sec.notes || '' }); setShowAirfieldSectorForm(true); }}
                                    style={{ padding: '2px 5px', background: '#0f766e', color: '#99f6e4', border: '1px solid #14b8a6', borderRadius: '3px', cursor: 'pointer', fontSize: '9px' }}>⧉</button>
                      ...
                      <div style={{ borderTop: '1px solid #334155', paddingTop: '10px' }}>
                        <div data-af-section="routes" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: adminAFExpanded.has('routes') ? '6px' : 0, cursor: 'pointer' }} onClick={() => toggleAFSec('routes')}>
                          ...
                          {adminAFExpanded.has('routes') && <button onClick={e => { e.stopPropagation(); setEditingAirfieldRoute(null); setAirfieldRouteForm({ name: '', airfield_id: String(selectedAdminAirfieldId), color: '#3b82f6', notes: '', category: 'general', is_runway: false, end_a_name: '', end_b_name: '' }); setShowAirfieldRouteForm(true); }}
                            style={{ padding: '2px 8px', background: '#1d4ed8', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('admin.mslvl')}</button>}
                        ...
                        {showAirfieldRouteForm && (
                          <div style={{ background: '#0f172a', padding: '8px', borderRadius: '6px', marginBottom: '6px', border: '1px solid #1e3a5f' }}>
                            <input type="text" placeholder={tr('admin.shmMslvl')} value={airfieldRouteForm.name}
                              onChange={e => setAirfieldRouteForm(p => ({ ...p, name: e.target.value }))}
                              style={{ width: '100%', padding: '5px 8px', background: '#1e293b', border: '1px solid #475569', borderRadius: '5px', color: 'white', fontSize: '12px', direction: 'rtl', boxSizing: 'border-box', marginBottom: '5px' }} />
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '5px' }}>
                              <label style={{ fontSize: '10px', color: '#64748b', flexShrink: 0 }}>{tr('shared.color2')}</label>
                            ...
                            <textarea placeholder={tr('admin.harvtAvptsyvnly')} value={airfieldRouteForm.notes}
                            ...
                            <select value={airfieldRouteForm.category}
                              ...
                              <option value="general">{tr('admin.klly')}</option>
                              <option value="aircraft">{tr('shared.aircraft')}</option>
                              <option value="vehicle">{tr('admin.klyRkb')}</option>
                            </select>
                            <label style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '5px', cursor: 'pointer' }}>
                              ...
                              <span style={{ fontSize: '11px', color: airfieldRouteForm.is_runway ? '#fcd34d' : '#94a3b8' }}>{tr('admin.mslvlHmrah')}</span>
                            </label>
                            ...
                            <div style={{ display: 'flex', gap: '5px' }}>
                              {!editingAirfieldRoute && hasMap ? (
                                ...
                                }} style={{ flex: 1, padding: '4px', background: '#d97706', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>{tr('admin.tsyvr')}</button>
                              ) : (
                                <button onClick={async () => {
                                  if (!airfieldRouteForm.name.trim()) return;
                        ...
                        {drawingRouteId && (
                          <div style={{ background: '#1c1400', border: '1px solid #fbbf24', borderRadius: '5px', padding: '6px 8px', marginBottom: '5px', fontSize: '10px', color: '#fcd34d' }}>
                            ...
                            <div style={{ display: 'flex', gap: '4px', marginTop: '4px' }}>
                              ...
                              }} style={{ flex: 1, padding: '3px', background: '#059669', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>{tr('admin.save')}{routeDraftPoints.length})</button>
                              ...
                              <button onClick={() => setRouteDraftPoints([])} style={{ padding: '3px 6px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '10px' }}>{tr('shared.clear')}</button>
                              <button onClick={() => { setDrawingRouteId(null); setRouteDraftPoints([]); }} style={{ padding: '3px 6px', background: '#7f1d1d', color: '#fca5a5', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '10px' }}>{tr('shared.cancel')}</button>
                            </div>
                          </div>
                        )}
                        {adminAirfieldRoutes.filter((r: any) => Number(r.airfield_id) === Number(selectedAdminAirfieldId)).length === 0
                          ...
                          {adminAFExpanded.has('vehicle_routes') && !drawingVehicleRouteId && (() => {
                            ...
                            return (
                              <button onClick={async (e) => {
                                ...
                                style={{ padding: '2px 7px', background: '#064e3b', color: '#34d399', border: '1px solid #065f46', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold', marginLeft: '4px' }}>{tr('admin.agnNTs')}</button>
                            );
                          })()}
                        ...
                        {adminAFExpanded.has('vehicle_routes') && (<>
                          {showVehicleRouteForm && (
                            <div style={{ background: '#0f172a', padding: '8px', borderRadius: '6px', marginBottom: '6px', border: '1px solid #7c2d12' }}>
                              <input type="text" placeholder={tr('admin.shmHntyb')} value={routeForm.name}
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
                                <label style={{ fontSize: '10px', color: '#64748b', flexShrink: 0 }}>{tr('admin.svgMslvl')}</label>
                                <select value={routeForm.route_type} onChange={e => setRouteForm(p => ({ ...p, route_type: e.target.value }))}
                                  ...
                                  <option value="vehicle">{tr('admin.kbyshRkb')}</option>
                                  <option value="taxiway">{tr('admin.mslvlHsah2')}</option>
                              ...
                              <div style={{ display: 'flex', gap: '5px' }}>
                                {hasMap ? (
                                  ...
                                  }} style={{ flex: 1, padding: '4px', background: '#d97706', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>{tr('admin.tsyvrAlHmph')}</button>
                                ) : (
                                  <div style={{ flex: 1, color: '#ef4444', fontSize: '11px', textAlign: 'center', padding: '3px 0' }}>{tr('admin.aynMphLshdhZh')}</div>
                                )}
                          ...
                          {drawingVehicleRouteId && (
                            <div style={{ background: '#1c0a00', border: '1px solid #f97316', borderRadius: '6px', padding: '6px 8px', marginBottom: '6px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              <div style={{ color: '#fb923c', fontSize: '11px', fontWeight: 'bold' }}>{tr('admin.drawingMode')} {vehicleRouteDraftPoints.length} {tr('admin.pointsClickTheMap')}</div>
                              <div style={{ display: 'flex', gap: '4px' }}>
                                <button onClick={async () => {
                          ...
                          {bRoutes.length === 0 && !showVehicleRouteForm && !drawingVehicleRouteId && (
                            <div style={{ color: '#475569', fontSize: '11px', textAlign: 'center', padding: '6px 0' }}>{tr('admin.aynNtybyNsyahLchts')}</div>
                          )}
                        </>)}
                      </div>
                    )}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: adminAFExpanded.has('taxiways') ? '6px' : 0, cursor: 'pointer' }} onClick={() => toggleAFSec('taxiways')}>
                          ...
                          {adminAFExpanded.has('taxiways') && !twAdminShowAdd && (
                            <button onClick={e => { e.stopPropagation(); setTwAdminNewName(''); setTwAdminShowAdd(true); }}
                              style={{ padding: '2px 8px', background: '#78350f', color: '#fcd34d', border: '1px solid #fbbf2466', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold', marginLeft: '4px' }}>{tr('shared.add')}</button>
                          )}
                        ...
                        {adminAFExpanded.has('taxiways') && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            {twAdminShowAdd && (
                              <div style={{ display: 'flex', gap: '4px', alignItems: 'center', marginBottom: '4px' }}>
                                <input autoFocus type="text" value={twAdminNewName}
                                  ...
                                  placeholder={tr('admin.shmNtybAB1')}
                            ...
                            {adminAirfieldTaxiways.length === 0 && (
                              <div style={{ fontSize: '11px', color: '#475569', textAlign: 'center', padding: '8px 0', direction: 'rtl' }}>{tr('admin.aynTaxiwaysMvgdrymLchts')}</div>
                            )}
                      ...
                      return (
                        <>
                          {!afAnchorMode ? (
                            ...
                            <span style={{ fontSize:'10px', color:'#fbbf24', fontWeight:'bold' }}>{tr('admin.mtsbAygvnLchtsAl')}</span>
                          )}
                        </>
                      );
                    })()}
                      ...
                      <div style={{ background:'#0a1628', borderBottom:'1px solid #1e3a5f', padding:'8px 10px', display:'flex', flexDirection:'column', gap:'6px', flexShrink:0 }}>
                        ...
                        <div style={{ display:'flex', gap:'6px' }}>
                          {afAnchorStep===1 && afPendingAnchor1 && (
                            <button onClick={()=>setAfAnchorStep(2)} style={{ flex:1, background:'#1d4ed8', color:'white', border:'none', borderRadius:'4px', padding:'5px', cursor:'pointer', fontSize:'12px' }}>{tr('shared.goToAnchor2')}</button>
                          )}
                  ...
                  >
                        ...
                        <div style={{ position: 'absolute', left: pos.left, top: pos.top, transform: 'translateX(-50%)', zIndex: 30, background: '#0f172a', border: '1px solid #3b82f6', borderRadius: '8px', padding: '8px 10px', boxShadow: '0 4px 20px rgba(0,0,0,0.7)', direction: 'rtl', minWidth: '140px', pointerEvents: 'all' }}
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            <button onClick={() => {
                              ...
                              setAdminMapElPopup(null);
                            }} style={{ padding: '4px 8px', background: '#1e3a5f', color: '#93c5fd', border: '1px solid #3b82f6', borderRadius: '5px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', textAlign: 'right' }}>{tr('admin.arvk3')}</button>
                            <button onClick={async () => {
                              ...
                              loadAirfieldElements(selectedAdminAirfieldId!);
                              setAdminMapElPopup(null);
                            }} style={{ padding: '4px 8px', background: '#1e293b', color: '#f87171', border: '1px solid #ef4444', borderRadius: '5px', cursor: 'pointer', fontSize: '11px', textAlign: 'right' }}>{tr('admin.hsrMykvm2')}</button>
                            <button onClick={() => {
                              setPlacingElementMode(true);
                              setPlacingElementId(popEl.id);
                              setAdminMapElPopup(null);
                            }} style={{ padding: '4px 8px', background: '#1e293b', color: '#f9a8d4', border: '1px solid #ec4899', borderRadius: '5px', cursor: 'pointer', fontSize: '11px', textAlign: 'right' }}>{tr('admin.shnhMykvm')}</button>
                          </div>
                        </div>
                      );
                    })()}
                    {placingElementMode && placingElementId && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(236,72,153,0.05)', pointerEvents: 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', zIndex: 5 }}>
                        <div style={{ background: '#000000dd', color: '#f9a8d4', padding: '4px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #ec4899' }}>
                          📍 {adminAirfieldElements.find(e => e.id === placingElementId)?.name || 'אלמנט'} {tr('admin.clickTheMapEsc')}
                        </div>
                      </div>
                    )}
                    ...
                    {placingAdminLocMode && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(52,211,153,0.06)', pointerEvents: 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', zIndex: 3 }}>
                        <div style={{ background: '#000000dd', color: '#34d399', padding: '4px 14px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #34d399' }}>{tr('admin.hnchNkvdhMnhltyt')} <strong>{adminLocNewName}</strong> {tr('admin.escLbytvl2')}</div>
                      </div>
                    )}
                    {afAnchorMode && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(249,115,22,0.06)', pointerEvents: 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', zIndex: 3 }}>
                        <div style={{ background: '#000000dd', color: '#fb923c', padding: '4px 14px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #fb923c' }}>{tr('admin.anchor')} {afAnchorStep} {tr('admin.clickAKnownPoint')}</div>
                      </div>
                    )}
                    {drawingRouteId && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(245,158,11,0.04)', pointerEvents: 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', zIndex: 3 }}>
                        <div style={{ background: '#000000dd', color: '#f59e0b', padding: '4px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #f59e0b' }}>{tr('admin.routeDrawing')} {routeDraftPoints.length} {tr('admin.pointsEscToCancel')}</div>
                      </div>
                    )}
                    {drawingVehicleRouteId && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(249,115,22,0.04)', pointerEvents: 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', zIndex: 3 }}>
                        <div style={{ background: '#000000dd', color: '#f97316', padding: '4px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #f97316' }}>{tr('admin.taxiRouteDrawing')} {vehicleRouteDraftPoints.length} {tr('admin.pointsClickTheMap2')}</div>
                      </div>
                    )}
                    ...
                    {drawingPolygonId && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(124,58,237,0.04)', pointerEvents: 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', zIndex: 5 }}>
                        <div style={{ background: '#000000dd', color: '#c4b5fd', padding: '4px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #7c3aed' }}>{tr('admin.polygonDrawing')} {polygonDraftPoints.length} {tr('admin.pointsDoubleClickTo')}</div>
                      </div>
                    )}
                    ...
                    {drawingSectorId && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(5,150,105,0.04)', pointerEvents: 'none', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '10px', zIndex: 5 }}>
                        <div style={{ background: '#000000dd', color: '#6ee7b7', padding: '4px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #059669' }}>{tr('admin.grvrAlHmphLtsyvr')}</div>
                      </div>
                    )}
        ...
        {activeTab === 'base_statuses' && (() => {
          ...
          return (
            <div style={{ direction: 'rtl' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h2 style={{ margin: 0, fontSize: '18px' }}>{tr('shared.baseStatus')}</h2>
                <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '10px', padding: '18px', marginBottom: '20px', direction: 'rtl' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div style={{ gridColumn: '1 / -1' }}>
                      <label style={{ fontSize: '12px', color: '#94a3b8', display: 'block', marginBottom: '4px' }}>{tr('admin.bsys')}</label>
                      {adminAviationBases.length > 0 ? (
                        <select
                        ...
                        >
                          <option value="">{tr('admin.bchrBsys')}</option>
                          {adminAviationBases.map((b: any) => (
                            <option key={b.id} value={b.name}>{b.name}{b.code ? ` (${b.code})` : ''}</option>
                          ))}
                        </select>
                      ) : (
                        <input value={baseStatusForm.name} onChange={e => setBaseStatusForm(p => ({ ...p, name: e.target.value }))} placeholder={tr('admin.hznShmBsys')} style={{ width: '100%', padding: '6px 10px', background: '#1e293b', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '13px', boxSizing: 'border-box', direction: 'rtl' }} />
                      )}
                      {adminAviationBases.length === 0 && <div style={{ fontSize: '11px', color: '#f59e0b', marginTop: '4px' }}>{tr('admin.hgdrBsysyTavphBlshvnyt2')}</div>}
                    </div>
                    <div>
                      <label style={{ fontSize: '12px', color: '#94a3b8', display: 'block', marginBottom: '4px' }}>{tr('admin.rlvvntyL')}</label>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', color: '#94a3b8', display: 'block', marginBottom: '4px' }}>{tr('admin.mtsbMzA')}</label>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', color: '#94a3b8', display: 'block', marginBottom: '4px' }}>{tr('admin.mtsbSpygh')}</label>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', color: '#94a3b8', display: 'block', marginBottom: '4px' }}>{tr('admin.sttvsTsypvry')}</label>
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      <label style={{ fontSize: '12px', color: '#94a3b8', display: 'block', marginBottom: '4px' }}>{tr('admin.shdhTavphMkvshrLrykvz')}</label>
                      <select value={String(baseStatusForm.airfield_id || '')} onChange={e => setBaseStatusForm(p => ({ ...p, airfield_id: e.target.value }))}
                        ...
                        <option value="">{tr('admin.llaShdhMkvshr')}</option>
                        {adminAirfields.map((af: any) => <option key={af.id} value={af.id}>{af.name}</option>)}
                      </select>
                      {baseStatusForm.airfield_id && <p style={{ margin: '3px 0 0 0', fontSize: '10px', color: '#38bdf8' }}>{tr('admin.panlSttvsBsysymYtsyg')}</p>}
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: '8px', marginTop: '14px', justifyContent: 'flex-end' }}>
                    <button onClick={() => { setShowBaseStatusForm(false); setEditingBaseStatus(null); }} style={{ padding: '7px 18px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.cancel')}</button>
                    <button onClick={saveBaseStatus} style={{ padding: '7px 18px', background: '#d97706', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}>{tr('shared.save2')}</button>
                  </div>
                </div>
              )}

              <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '8px' }}>
                {tr('admin.csvImportColumnsName')}
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {adminBaseStatuses.length === 0 ? (
                  <div style={{ textAlign: 'center', color: '#475569', padding: '40px', fontSize: '14px' }}>{tr('admin.aynBsysymLchtsChdsh')}</div>
                ) : adminBaseStatuses.map((bs: any) => (
                  <div key={bs.id} style={{ background: '#0f172a', border: '1px solid #1e3a5f', borderRadius: '8px', padding: '12px 16px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
                    <div style={{ flex: 1, direction: 'rtl' }}>
                      ...
                      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '12px', color: '#94a3b8' }}>
                        {bs.air_defense_status && <span><span style={{ color: '#64748b' }}>{tr('admin.mzA')} </span><span style={{ color: ALL_MAZAA_STATUSES.find(s => s.label === bs.air_defense_status)?.color || '#e2e8f0', fontWeight: 'bold' }}>{bs.air_defense_status}</span></span>}
                        {bs.absorption_status && <span><span style={{ color: '#64748b' }}>{tr('admin.spygh')} </span><span style={{ color: '#e2e8f0' }}>{bs.absorption_status}</span></span>}
                        {bs.bird_status && <span><span style={{ color: '#64748b' }}>{tr('admin.tsypvry')} </span><span style={{ color: '#e2e8f0' }}>{bs.bird_status}</span></span>}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                      <button onClick={() => { setEditingBaseStatus(bs); setBaseStatusForm({ name: bs.name, code: bs.code || '', relevant_to: bs.relevant_to || 'כולם', air_defense_status: bs.air_defense_status || '', absorption_status: bs.absorption_status || '', bird_status: bs.bird_status || '', airfield_id: bs.airfield_id || '' }); setShowBaseStatusForm(true); }} style={{ padding: '5px 12px', background: '#1e3a5f', color: '#93c5fd', border: '1px solid #2563eb', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.edit2')}</button>
                      <button onClick={() => { setEditingBaseStatus(null); setBaseStatusForm({ name: `${bs.name} (העתק)`, code: bs.code || '', relevant_to: bs.relevant_to || 'כולם', air_defense_status: bs.air_defense_status || '', absorption_status: bs.absorption_status || '', bird_status: bs.bird_status || '', airfield_id: bs.airfield_id || '' }); setShowBaseStatusForm(true); }} style={{ padding: '5px 12px', background: '#0f766e', color: '#99f6e4', border: '1px solid #14b8a6', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>{tr('admin.shkpl4')}</button>
                      <button onClick={async () => { if (!await customConfirm(`למחוק את "${bs.name}"?`)) return; await fetch(`${API_URL}/base-statuses/${bs.id}`, { method: 'DELETE' }); loadAdminBaseStatuses(); }} style={{ padding: '5px 12px', background: '#450a0a', color: '#fca5a5', border: '1px solid #dc2626', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.delete3')}</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          return (
            <div style={{ padding: '20px', direction: 'rtl', maxWidth: '900px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h2 style={{ margin: 0, fontSize: '18px', color: '#7dd3fc' }}>{tr('admin.bsysyTavph')}</h2>
              ...
              {showAviationBaseForm && (
                <div style={{ background: '#1e293b', padding: '16px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #334155' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: '8px', marginBottom: '8px' }}>
                    <input type="text" placeholder={tr('admin.shmHbsys2')} value={aviationBaseForm.name}
                    ...
                    <input type="text" placeholder={tr('admin.kvdIcao')} value={aviationBaseForm.code}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                    <div>
                      <div style={{ fontSize: '11px', color: '#7dd3fc', fontWeight: 'bold', marginBottom: '4px' }}>{tr('admin.nTsNKv')}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', direction: 'ltr' }}>
                        <input type="number" min={0} max={90} placeholder={tr('admin.malvt')} value={aviationBaseForm.coord_n_deg}
                        ...
                        <input type="number" min={0} max={59} placeholder={tr('shared.minutes')} value={aviationBaseForm.coord_n_min}
                        ...
                        <span style={{ color: '#94a3b8', fontSize: '13px' }}>′</span>
                        <input type="number" min={0} max={59} placeholder={tr('admin.shnyvt')} value={aviationBaseForm.coord_n_sec}
                    ...
                    <div>
                      <div style={{ fontSize: '11px', color: '#7dd3fc', fontWeight: 'bold', marginBottom: '4px' }}>{tr('admin.nTsEKv')}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', direction: 'ltr' }}>
                        <input type="number" min={0} max={180} placeholder={tr('admin.malvt')} value={aviationBaseForm.coord_e_deg}
                          onChange={e => setAviationBaseForm(p => ({ ...p, coord_e_deg: e.target.value }))}
                          style={{ width: '62px', padding: '6px 6px', background: '#0f172a', border: '1px solid #475569', borderRadius: '6px', color: 'white', fontSize: '13px', textAlign: 'center', fontFamily: 'monospace' }} />
                        <span style={{ color: '#94a3b8', fontSize: '13px' }}>°</span>
                        <input type="number" min={0} max={59} placeholder={tr('shared.minutes')} value={aviationBaseForm.coord_e_min}
                          onChange={e => setAviationBaseForm(p => ({ ...p, coord_e_min: e.target.value }))}
                          style={{ width: '54px', padding: '6px 6px', background: '#0f172a', border: '1px solid #475569', borderRadius: '6px', color: 'white', fontSize: '13px', textAlign: 'center', fontFamily: 'monospace' }} />
                        <span style={{ color: '#94a3b8', fontSize: '13px' }}>′</span>
                        <input type="number" min={0} max={59} placeholder={tr('admin.shnyvt')} value={aviationBaseForm.coord_e_sec}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                    <div>
                      <div style={{ fontSize: '12px', color: '#7dd3fc', fontWeight: 'bold', marginBottom: '6px' }}>{tr('admin.sidNhlyYtsyah')}</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '6px', minHeight: '28px' }}>
                        ...
                        {aviationBaseForm.sids.length === 0 && <span style={{ fontSize: '11px', color: '#475569' }}>{tr('admin.aynSids')}</span>}
                      </div>
                      <div style={{ display: 'flex', gap: '4px' }}>
                        <input type="text" placeholder={tr('admin.hvsfSid')} value={aviationBaseForm.newSid}
                    ...
                    <div>
                      <div style={{ fontSize: '12px', color: '#fbbf24', fontWeight: 'bold', marginBottom: '6px' }}>{tr('admin.starNhlyKnysh')}</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '6px', minHeight: '28px' }}>
                        ...
                        {aviationBaseForm.stars.length === 0 && <span style={{ fontSize: '11px', color: '#475569' }}>{tr('admin.aynStars')}</span>}
                      </div>
                      <div style={{ display: 'flex', gap: '4px' }}>
                        <input type="text" placeholder={tr('admin.hvsfStar')} value={aviationBaseForm.newStar}
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => { setShowAviationBaseForm(false); setEditingAviationBase(null); setAviationBaseForm(emptyForm); }}
                      style={{ padding: '7px 14px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.cancel')}</button>
                  </div>
                </div>
              )}
                    ...
                    <div key={b.id} style={{ display: 'grid', gridTemplateColumns: '44px 1fr 80px 1fr 1fr 80px', gap: '8px', padding: '8px 12px', borderTop: '1px solid #1e293b', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: '4px', justifyContent: 'flex-end' }}>
                        ...
                        <button onClick={async () => { if (!await customConfirm(`למחוק את הבסיס "${b.name}"?`)) return; await fetch(`${API_URL}/aviation-bases/${b.id}`, { method: 'DELETE' }); fetch(`${API_URL}/aviation-bases`).then(r => r.ok ? r.json() : []).then(setAdminAviationBases).catch(() => {}); }}
                          style={{ padding: '3px 8px', background: '#7f1d1d', color: '#fca5a5', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.delete2')}</button>
                      </div>
                    </div>
                  ))
                }
        ...
        {activeTab === 'value_lists' && (() => {
          ...
          const IconPicker = () => (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div>
                <div style={{ color: '#64748b', fontSize: '11px', marginBottom: '4px' }}>{tr('admin.amvgY')}</div>
              ...
              <div>
                <div style={{ color: '#64748b', fontSize: '11px', marginBottom: '4px' }}>{tr('admin.ayykvnyMph')}</div>
                <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
              ...
              {elementTypeForm.icon && (() => {
                ...
                return (
                  <div style={{ padding: '10px 12px', background: '#0a1628', borderRadius: '8px', border: '1px solid #1e3a5f', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div style={{ fontSize: '11px', color: '#94a3b8', textAlign: 'center' }}>{tr('admin.ttsvghMkdymhBchrMtsb')}</div>
                    {/* Mode selector */}
                    ...
                    {(etPreviewMode === 'open' || etPreviewMode === 'close') && isSvgPrev && (
                      <div style={{ borderTop: '1px solid #1e293b', paddingTop: '8px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                          <button type="button" title={tr('admin.bryrtMchdlAyykvnRashy')} onClick={() => setElementTypeFormAndRef(p => ({ ...p, [etPreviewMode === 'open' ? 'open_icon' : 'close_icon']: '' }))}
                    ...
                    {etPreviewMode === 'blink' && (
                      <div style={{ borderTop: '1px solid #1e293b', paddingTop: '6px', textAlign: 'center' }}>
                        <div style={{ fontSize: '10px', color: '#64748b' }}>{tr('admin.hhbhvbMvgdrAY')}</div>
                      </div>
                    )}
                  </div>
                );
          ...
          const CanChangeStatusSection = () => (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', background: '#0f172a', borderRadius: '6px', border: '1px solid #334155' }}>
                ...
                <span style={{ fontSize: '13px', color: '#e2e8f0', cursor: 'pointer' }} onClick={() => setElementTypeFormAndRef(p => ({ ...p, can_change_status: !p.can_change_status }))}>{tr('admin.nytnLshynvySttvsBlchytsh')}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', background: '#0f172a', borderRadius: '6px', border: '1px solid #334155' }}>
                ...
                <span style={{ fontSize: '13px', color: '#e2e8f0', cursor: 'pointer' }} onClick={() => setElementTypeFormAndRef(p => ({ ...p, can_have_route: !p.can_have_route }))}>{tr('admin.nytnLhgdrtMslvlVanymtsyh')}</span>
              </div>
              {elementTypeForm.can_change_status && (
                <div style={{ padding: '10px', background: '#0f172a', borderRadius: '6px', border: '1px solid #1d4ed8' }}>
                  ...
                  <div style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '6px' }}>{tr('admin.bchrMrshymhMhyrh')}</div>
                  ...
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', borderTop: '1px solid #1e3a5f', paddingTop: '8px', marginBottom: '8px' }}>
                    <input
                      ...
                      placeholder={tr('admin.hvsfSttvsYdny')}
                    ...
                    <button type="button"
                      ...
                      style={{ padding: '5px 12px', background: '#1d4ed8', border: 'none', borderRadius: '5px', color: 'white', cursor: 'pointer', fontSize: '12px', whiteSpace: 'nowrap', flexShrink: 0 }}>{tr('shared.add')}</button>
                  </div>
                  ...
                  {elementTypeForm.allowed_statuses.length > 0 && (
                    <div style={{ borderTop: '1px solid #1e3a5f', paddingTop: '8px' }}>
                      <div style={{ fontSize: '11px', color: '#60a5fa', marginBottom: '6px', fontWeight: 'bold' }}>{tr('admin.sttvsymPaylymBchrAyykvn')}</div>
                            ...
                            <div key={s} style={{ background: '#0a1628', borderRadius: '6px', border: `1px solid ${isPickerOpen ? '#3b82f6' : '#1e3a5f'}`, overflow: 'hidden' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 8px' }}>
                                ...
                                <button type="button"
                                  ...
                                  ) : <span style={{ color: '#475569', fontSize: '10px' }}>{tr('admin.lla')}</span>}
                              ...
                              {isPickerOpen && (
                                <div style={{ borderTop: '1px solid #1e3a5f', padding: '8px', background: '#060f1e' }}>
                                  <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '5px' }}>{tr('admin.bchrAyykvnMph')}</div>
                                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                    <button type="button" title={tr('admin.llaAyykvn')}
                  ...
                  {(elementTypeForm.allowed_statuses.includes('פתוח') || elementTypeForm.allowed_statuses.includes('סגור')) && (
                    <div style={{ marginTop: '8px', padding: '6px 8px', background: '#0a1628', borderRadius: '6px', border: '1px dashed #1e3a5f' }}>
                      <div style={{ fontSize: '10px', color: '#64748b' }}>{tr('admin.layykvnPtvchSgvrNytn')}</div>
                    </div>
                  )}
                </div>
              )}
          ...
          return (
            <div style={{ padding: '20px', direction: 'rtl', maxWidth: '700px' }}>
              <h2 style={{ margin: '0 0 20px 0', fontSize: '18px', color: '#c4b5fd' }}>{tr('admin.almntymBbsys')}</h2>
              ...
              <div style={{ background: '#0f172a', borderRadius: '10px', border: '1px solid #7c3aed', overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', background: '#1e1040', borderBottom: '1px solid #7c3aed', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: '#c4b5fd', fontSize: '15px', fontWeight: 'bold' }}>{tr('admin.elementTypes')} {adminElementTypes.length}</span>
                  <span style={{ color: '#64748b', fontSize: '12px' }}>{tr('admin.mshmshymLsyvvgAlmntymBshdvt')}</span>
                </div>
                <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {adminElementTypes.length === 0 && (
                    <div style={{ color: '#475569', fontSize: '13px', textAlign: 'center', padding: '16px 0' }}>{tr('admin.aynSvgymAdyynHvsf')}</div>
                  )}
                  {adminElementTypes.map(et => {
                  ...
                  {editingElementType ? (
                    <div style={{ padding: '14px', background: '#1e1040', borderRadius: '8px', border: '2px solid #7c3aed', display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '4px' }}>
                      <div style={{ color: '#c4b5fd', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.editType')} {editingElementType.name}</div>
                      <input value={elementTypeForm.name} onChange={e => setElementTypeFormAndRef(p => ({ ...p, name: e.target.value }))} placeholder={tr('admin.shmHsvg')}
                      ...
                      <div>
                        <div style={{ color: '#94a3b8', fontSize: '12px', marginBottom: '6px' }}>{tr('admin.bchrAyykvn2')}</div>
                        {IconPicker()}
                      </div>
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <span style={{ fontSize: '13px', color: '#94a3b8' }}>{tr('shared.color2')}</span>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button type="button" onClick={async () => { const form = elementTypeFormRef.current; await fetch(`${API_URL}/airfield-element-types/${editingElementType.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) }); fetch(`${API_URL}/airfield-element-types`).then(r => r.ok ? r.json() : []).then(setAdminElementTypes).catch(() => {}); setEditingElementType(null); setElementTypeFormAndRef({ name: '', color: '#f59e0b', icon: '🔧', can_change_status: false, allowed_statuses: [], open_icon: '', close_icon: '', can_have_route: false, status_icons: {} }); }}
                          style={{ flex: 1, padding: '8px', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}>{tr('shared.save')}</button>
                        <button type="button" onClick={() => { setEditingElementType(null); setElementTypeFormAndRef({ name: '', color: '#f59e0b', icon: '🔧', can_change_status: false, allowed_statuses: [], open_icon: '', close_icon: '', can_have_route: false, status_icons: {} }); }}
                          style={{ padding: '8px 16px', background: '#334155', color: '#94a3b8', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.cancel')}</button>
                      </div>
                    </div>
                  ) : (
                    <div style={{ padding: '14px', background: '#0a1628', borderRadius: '8px', border: '1px dashed #334155', display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '4px' }}>
                      <div style={{ color: '#94a3b8', fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.hvsfSvgChdsh')}</div>
                      <input value={elementTypeForm.name} onChange={e => setElementTypeFormAndRef(p => ({ ...p, name: e.target.value }))} placeholder={tr('admin.shmHsvgLdvgmhKbayt')}
                      ...
                      <button type="button" onClick={async () => { const form = elementTypeFormRef.current; if (!form.name.trim()) return; await fetch(`${API_URL}/airfield-element-types`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) }); fetch(`${API_URL}/airfield-element-types`).then(r => r.ok ? r.json() : []).then(setAdminElementTypes).catch(() => {}); setElementTypeFormAndRef({ name: '', color: '#f59e0b', icon: '🔧', can_change_status: false, allowed_statuses: [], open_icon: '', close_icon: '', can_have_route: false, status_icons: {} }); }}
                        ...
                        style={{ padding: '8px', background: elementTypeForm.name.trim() ? '#059669' : '#1e293b', color: 'white', border: 'none', borderRadius: '6px', cursor: elementTypeForm.name.trim() ? 'pointer' : 'not-allowed', fontSize: '13px', fontWeight: 'bold', opacity: elementTypeForm.name.trim() ? 1 : 0.5 }}>{tr('admin.hvsfSvg')}</button>
                    </div>
                  )}
                </div>
              </div>
        ...
        {activeTab === 'contacts' && (() => {
          ...
          return (
            <div style={{ padding: '20px', direction: 'rtl', maxWidth: '1000px' }}>
              <h2 style={{ margin: '0 0 6px 0', fontSize: '18px', color: '#38bdf8' }}>{tr('admin.nyhvlKshrymLamdvt')}</h2>
              <p style={{ color: '#94a3b8', fontSize: '12px', marginBottom: '16px' }}>
                {tr('admin.setDefaultContactsFor')}
              </p>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '20px' }}>
                ...
                >
                  <option value="">{tr('admin.bchrAmdhLhvsyf')}</option>
                ...
                >{tr('admin.hvsfAmdh')}</button>
                {adminContactsShown.length === 0 && availableToAdd.length === 0 && (
                  <span style={{ color: '#64748b', fontSize: '12px' }}>{tr('admin.klHamdvtMvtsgvt')}</span>
                )}
              </div>
                    ...
                    <div style={{ background: '#1e3a5f', padding: '8px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        {adminContactsSavingId !== null && (adminContactsData[presetId] || []).some((r: any) => r._key === adminContactsSavingId) && (
                          <span style={{ color: '#38bdf8', fontSize: '11px' }}>{tr('admin.shvmr')}</span>
                        )}
                        <button
                        ...
                        >{tr('admin.hstr')}</button>
                      </div>
                    </div>

                    {/* Contacts table */}
                    {rows === undefined ? (
                      <div style={{ padding: '16px', color: '#475569', fontSize: '12px', textAlign: 'center' }}>{tr('shared.loading')}</div>
                    ) : (
                      <>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                          <thead>
                            <tr style={{ background: '#0c1824', color: '#64748b' }}>
                              <th style={{ ...tdS, width: '80px', textAlign: 'center' }}>{tr('admin.rashyMshny')}</th>
                              <th style={{ ...tdS, width: '13%', textAlign: 'right' }}>{tr('admin.svgMkshyr')}</th>
                              <th style={{ ...tdS, width: '17%', textAlign: 'right' }}>{tr('shared.frequencyChannel')}</th>
                              <th style={{ ...tdS, width: '20%', textAlign: 'right' }}>{tr('shared.nature')}</th>
                              <th style={{ ...tdS, width: '12%', textAlign: 'right' }}>{'או"ק'}</th>
                              <th style={{ ...tdS, textAlign: 'right' }}>{tr('shared.note')}</th>
                          ...
                          <tbody>
                              ...
                              <tr key={row._key} style={{ background: row._unsaved ? '#0c1f33' : 'transparent' }}>
                                ...
                                <td style={tdS}>
                                  <input
                                    ...
                                    onFocus={e => (e.target.style.border = inpFocusStyle)}
                                    style={inpS}
                                    placeholder={tr('admin.mhvtHkshr')}
                                  />
                                </td>
                                <td style={tdS}>
                                  <input
                            ...
                            {rows.length === 0 && (
                              <tr><td colSpan={6} style={{ textAlign: 'center', color: '#334155', padding: '14px', fontSize: '12px' }}>{tr('admin.aynKshrymLchtsHvsf')}</td></tr>
                            )}
                          </tbody>
                        </table>
              ...
              {adminContactsShown.length === 0 && (
                <div style={{ textAlign: 'center', color: '#334155', fontSize: '13px', padding: '40px' }}>
                  {tr('admin.selectAWorkstationFrom')}
                </div>
              )}
            </div>
          );
...
function SignalCatalogAdd({ onAdd }: { onAdd: (t: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: '6px' }}>
      <input value={v} onChange={e => setV(e.target.value)} maxLength={120} placeholder={tr('admin.hvdahChdshhLmagr')}
      ...
      <button onClick={submit} style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', padding: '5px 14px', fontWeight: 'bold', fontSize: '12px', cursor: 'pointer' }}>{tr('admin.hvsf')}</button>
    </div>
  );
}
