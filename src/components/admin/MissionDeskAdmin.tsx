...
import { mdDefaultLeaf, mdSplit, mdRemove, mdUpdate, mdGenId, isImageDataUrl } from '../../utils/missionDesk';
...
// ─────────────────────────────────────────────────────────────────────────────
// עורך שירות תמונה — הדבקת print-screen (paste) או בחירת קובץ. נשמר כ-data URL
// ב-config. raster בלבד (SVG נדחה). תמונה גדולה מוקטנת אוטומטית לרוחב ~1600px.
// ─────────────────────────────────────────────────────────────────────────────
const downscaleImage = (dataUrl: string, maxW = 1600): Promise<string> => new Promise((resolve) => {
  const img = new Image();
  img.onload = () => {
...
function ImageConfigEditor({ config, onChange }: { config: MDImageConfig; onChange: (c: MDImageConfig) => void }) {
...
// ─────────────────────────────────────────────────────────────────────────────
function LabelConfigEditor({ config, onChange }: { config: MDLabelConfig; onChange: (c: MDLabelConfig) => void }) {
  return (
    <div>
