        ...
        return (
          <div key={note.id} style={{ position: 'fixed', left: note.x, top: note.y, zIndex: 2100, width: note.minimized ? 220 : 270, boxShadow: '0 6px 24px rgba(0,0,0,0.4)', borderRadius: '8px', overflow: 'visible', userSelect: 'none' }}>
            ...
            >
              ...
              {!editable && (
                <span title={tr('admin.kryahBlbdAynkMvrshh')} style={{ fontSize: '11px', flexShrink: 0 }}>🔒</span>
              )}
              ...
              <button onPointerDown={e => e.stopPropagation()}
                ...
                title={tr('admin.sgvrHptkytTyshmr')}
              ...
              <div style={{ background: note.background_color, borderRadius: '0 0 8px 8px', filter: 'brightness(1.04)' }}>
                <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(0,0,0,0.1)' }}>
                  <input value={note.title} onChange={e => updateNote(note.id, { title: e.target.value }, false)}
                    ...
                    disabled={!editable} placeholder={tr('shared.title')}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 8px 6px', borderTop: '1px solid rgba(0,0,0,0.08)', position: 'relative' }}>
                  ...
                  <button onClick={() => setShowColorPicker(showColorPicker === note.id ? null : note.id)} title={tr('admin.tsbaRka')}
                  ...
                  <div style={{ flex: 1 }} />
                  <button onClick={() => openDistribute(note.id)} title={tr('admin.hptsLnmanym')}
                    style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer', fontSize: '10px', flexShrink: 0 }}>{tr('admin.hpts')}</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '10px', padding: '20px 24px', width: '320px', direction: 'rtl', boxShadow: '0 20px 50px rgba(0,0,0,0.7)', textAlign: 'center' }}>
            ...
            {(() => { const n = notes.find(x => x.id === confirmDelete); return (!n || n.creator_preset_id === presetId || n.allow_all_edit); })() ? (
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                <button onClick={() => setConfirmDelete(null)} style={{ background: '#334155', color: 'white', border: 'none', borderRadius: '6px', padding: '8px 18px', cursor: 'pointer' }}>{tr('shared.cancel')}</button>
                <button onClick={() => deleteNote(confirmDelete!)} style={{ background: '#dc2626', color: 'white', border: 'none', borderRadius: '6px', padding: '8px 18px', cursor: 'pointer' }}>{tr('shared.delete2')}</button>
              </div>
            ) : (
              <button onClick={() => setConfirmDelete(null)} style={{ background: '#334155', color: 'white', border: 'none', borderRadius: '6px', padding: '8px 18px', cursor: 'pointer' }}>{tr('shared.close')}</button>
            )}
          </div>
        </div>
      )}
      ...
      {showDistribute !== null && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3100, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => { if (e.target === e.currentTarget) setShowDistribute(null); }}>
          <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '12px', padding: '24px', width: '380px', direction: 'rtl', boxShadow: '0 25px 60px rgba(0,0,0,0.7)' }}>
            <h3 style={{ margin: '0 0 16px', color: 'white', fontSize: '16px' }}>{tr('admin.hptsPtkytLnmanym')}</h3>
            {peers.length === 0 ? (
              <p style={{ color: '#94a3b8', fontSize: '13px', marginBottom: '16px' }}>{tr('admin.aynAmdvtBkbvtsvtHabvdh')}<br/>{tr('admin.hgdrKbvtsvtAbvdhBnyhvl')}</p>
            ) : (
              <>
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowDistribute(null)} style={{ background: '#334155', color: 'white', border: 'none', borderRadius: '6px', padding: '8px 18px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.cancel')}</button>
              {peers.length > 0 && (
...
export const WorkGroupsManager = ({ presets }: { presets: any[] }) => {
  ...
  return (
    <div style={{ direction: 'rtl' }}>
      <h2 style={{ margin: '0 0 20px', fontSize: '18px' }}>{tr('admin.kbvtsvtAbvdh')}</h2>
      ...
      <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', marginBottom: '20px', display: 'flex', gap: '8px', alignItems: 'center' }}>
        <input value={newGroupName} onChange={e => setNewGroupName(e.target.value)} onKeyDown={e => e.key === 'Enter' && createGroup()}
          placeholder={tr('admin.shmKbvtshChdshh')}
      ...
      {groups.length === 0 ? (
        <div style={{ textAlign: 'center', color: '#64748b', padding: '40px', fontSize: '14px' }}>{tr('admin.aynKbvtsvtAbvdhTsvr')}</div>
      ) : (
            ...
            return (
              <div key={group.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', border: '1px solid #1e3a5f' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                  {editingId === group.id ? (
                    <>
                      ...
                      <button onClick={() => renameGroup(group.id)} style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '5px', padding: '5px 12px', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.save')}</button>
                      <button onClick={() => setEditingId(null)} style={{ background: '#334155', color: 'white', border: 'none', borderRadius: '5px', padding: '5px 10px', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.cancel')}</button>
                    </>
                  ) : (
                    <>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: group.members.length > 0 ? '10px' : 0 }}>
                  ...
                  {group.members.length === 0 && <span style={{ color: '#64748b', fontSize: '12px', fontStyle: 'italic' }}>{tr('admin.aynAmdvtBkbvtsh')}</span>}
                </div>
                ...
                {group.members.length > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', borderTop: '1px solid #1e3a5f', paddingTop: '8px' }}>
                    <span style={{ fontSize: '11px', color: '#94a3b8', flexShrink: 0 }}>{tr('admin.amdtNyhvlMdnyvt')}</span>
                    <select
                      ...
                      <option value="">{tr('admin.llaAmdtNyhvl')}</option>
...
export const TableModesManager = () => {
  ...
  return (
    <div>
      <h2 style={{ margin: '0 0 20px 0', fontSize: '18px' }}>{tr('admin.mvdyTblh')}</h2>
      ...
      <div style={{ background: '#0f172a', borderRadius: '8px', padding: '20px', marginBottom: '24px' }}>
        ...
        <input
          ...
          placeholder={tr('admin.shmHmvdLdvgmhTblh')}
        ...
        {modes.map(mode => (
          <div key={mode.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => startEdit(mode)} style={{ padding: '6px 14px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.edit')}</button>
                <button onClick={() => deleteMode(mode.id)} style={{ padding: '6px 14px', background: '#dc2626', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.delete')}</button>
              </div>
            </div>
          </div>
        ))}
        {modes.length === 0 && (
          <div style={{ textAlign: 'center', color: '#64748b', padding: '40px' }}>{tr('admin.aynMvdyTblhTsvr')}</div>
        )}
      </div>
    </div>
  );
...
export const AidsManager = ({ presets }: { presets: any[] }) => {
  ...
  return (
    <div style={{ display: 'flex', gap: '20px', direction: 'rtl', minHeight: '400px' }}>
      ...
      <div style={{ flex: 1 }}>
        {!selectedPresetId && <div style={{ color: '#64748b' }}>{tr('admin.bchrAmdh')}</div>}
        {selectedPresetId && loading && <div style={{ color: '#64748b' }}>{tr('shared.loading')}</div>}
        {selectedPresetId && !loading && !aidGroup && (
          <div>
            <div style={{ color: '#94a3b8', marginBottom: '12px', fontSize: '13px' }}>{tr('admin.aynKbvtstAzrymLamdh')}</div>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <button onClick={createNewGroup} style={btnPrimary}>{tr('admin.tsvrKbvtstAzrymChdshh')}</button>
        ...
        {selectedPresetId && !loading && aidGroup && (
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
              ...
              <button onClick={() => { setShowShareModal(true); setShareMode('duplicate'); setShareTargets(new Set()); }} style={btnSecondary}>{tr('admin.shkpl2')}</button>
              <button onClick={() => { setShowShareModal(true); setShareMode('link'); setShareTargets(new Set()); }} style={btnSecondary}>{tr('admin.kshr2')}</button>
              <button onClick={unlinkGroup} style={btnDanger}>{tr('admin.ntk')}</button>
            </div>
                  ...
                  {editingItemId === item.id ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      {item.type === 'text' ? (
                        ...
                        <div>
                          {editingContent && <img src={editingContent} alt={tr('admin.ttsvghMkdymh')} style={{ maxWidth: '200px', maxHeight: '100px', borderRadius: '4px', display: 'block', marginBottom: '6px' }} />}
                      ...
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button onClick={() => saveItem(item.id)} style={btnPrimary}>{tr('shared.save')}</button>
                        <button onClick={() => setEditingItemId(null)} style={btnSecondary}>{tr('shared.cancel')}</button>
                      </div>
                    </div>
                  ) : (
            ...
            {/* Add item */}
            {!addingItem ? (
              <button onClick={() => setAddingItem(true)} style={{ ...btnPrimary, marginTop: '12px' }}>{tr('admin.hvsfAzr')}</button>
            ) : (
              <div style={{ marginTop: '12px', background: '#0f172a', border: '1px solid #475569', borderRadius: '8px', padding: '12px' }}>
                <div style={{ display: 'flex', gap: '8px', marginBottom: '8px', alignItems: 'center' }}>
                  <input value={newItemName} onChange={e => setNewItemName(e.target.value)} placeholder={tr('admin.shmHazr')}
                  ...
                  <select value={newItemType} onChange={e => setNewItemType(e.target.value as 'text'|'image')}
                    ...
                    <option value="text">{tr('admin.tkst2')}</option>
                    <option value="image">{tr('admin.tmvnh')}</option>
                  </select>
                </div>
                {newItemType === 'text' ? (
                  <textarea value={newItemContent} onChange={e => setNewItemContent(e.target.value)} rows={3} placeholder={tr('shared.content')}
                  ...
                  <div>
                    {newItemContent && <img src={newItemContent} alt={tr('admin.ttsvghMkdymh')} style={{ maxWidth: '200px', maxHeight: '100px', borderRadius: '4px', display: 'block', marginBottom: '6px' }} />}
                ...
                <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                  <button onClick={addItem} style={btnPrimary}>{tr('admin.hvsf')}</button>
                  <button onClick={() => { setAddingItem(false); setNewItemName(''); setNewItemContent(''); }} style={btnSecondary}>{tr('shared.cancel')}</button>
                </div>
              </div>
            )}
          </div>
      ...
      {showShareModal && aidGroup && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => { if (e.target === e.currentTarget) setShowShareModal(false); }}>
          <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '12px', padding: '24px', width: '360px', direction: 'rtl', boxShadow: '0 20px 50px rgba(0,0,0,0.7)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={doShare} disabled={shareTargets.size === 0} style={{ ...btnPrimary, opacity: shareTargets.size === 0 ? 0.5 : 1 }}>{tr('shared.acknowledge')}</button>
              <button onClick={() => setShowShareModal(false)} style={btnSecondary}>{tr('shared.cancel')}</button>
            </div>
          </div>
        </div>
      )}
...
export const SerialsAdminTab = ({ initialUndoDurationMs }: { initialUndoDurationMs?: number | null }) => {
  ...
  return (
    <div style={{ direction: 'rtl', color: 'white' }}>
      <h2 style={{ margin: '0 0 20px', fontSize: '18px' }}>{tr('admin.nyhvlSprvrym')}</h2>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 5000, display: 'flex', alignItems: 'stretch', justifyContent: 'center', background: 'rgba(0,0,0,0.6)' }}>
      <div style={{ background: bg, width: '100%', maxWidth: '900px', display: 'flex', flexDirection: 'column', direction: 'rtl', boxShadow: '0 8px 40px rgba(0,0,0,0.5)' }}>
        ...
        >
          ...
          <button onClick={e => { e.stopPropagation(); onClose(); }} style={{ background: '#dc2626', border: 'none', color: 'white', borderRadius: '6px', padding: '4px 12px', cursor: 'pointer', fontSize: '13px', marginRight: '8px' }}>{tr('shared.close2')}</button>
        </div>
        ...
        {showTimeFilter && (
          <div style={{ background: lightMode ? '#e2e8f0' : '#1e293b', padding: '12px 20px', display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap', borderBottom: `1px solid ${border}` }}>
            <span style={{ color: textMain, fontSize: '13px', fontWeight: 'bold' }}>{tr('admin.htsgSprvrymShnvtsrvB')}</span>
            {[3, 6, 12, 24, 48].map(h => (
            ...
            {hoursFilter !== null && (
              <button onClick={() => setHoursFilter(null)} style={{ background: '#64748b', color: 'white', border: 'none', borderRadius: '6px', padding: '5px 10px', cursor: 'pointer', fontSize: '12px' }}>{tr('admin.llaSynvn')}</button>
            )}
          </div>
        )}
        ...
        <div style={{ padding: '10px 20px', background: bg2, borderBottom: `1px solid ${border}`, display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ color: textSub, fontSize: '12px' }}>{tr('admin.tayShlyth')}</span>
          <button onClick={() => setSelectedStations(new Set(allStations))} style={{ background: 'transparent', border: `1px solid ${border}`, color: textSub, borderRadius: '4px', padding: '2px 8px', cursor: 'pointer', fontSize: '11px' }}>{tr('admin.hkl')}</button>
          <button onClick={() => setSelectedStations(new Set())} style={{ background: 'transparent', border: `1px solid ${border}`, color: textSub, borderRadius: '4px', padding: '2px 8px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.clear')}</button>
          {allStations.map(st => (
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          {Object.keys(grouped).length === 0 ? (
            <div style={{ color: textSub, textAlign: 'center', marginTop: '40px', fontSize: '14px' }}>{tr('admin.aynSprvrymLhtsgh')}</div>
          ) : (
            (Object.entries(grouped) as [string, any[]][]).map(([station, stSerials]) => (
  ...
  return (
    <div style={{ padding: '16px', direction: 'rtl', color: text, background: bg, minHeight: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700 }}>{tr('admin.tchkyrYvmnPaylvt')}</h2>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '12px', padding: '10px 12px', background: cardBg, borderRadius: '8px', border: `1px solid ${border}` }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          <span style={{ fontSize: '11px', color: muted }}>{tr('admin.svgArva')}</span>
          <select value={filterEventType} onChange={e => setFilterEventType(e.target.value)} style={inputStyle}>
            <option value=''>{tr('admin.hkl')}</option>
            {Object.entries(EVENT_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          <span style={{ fontSize: '11px', color: muted }}>{tr('admin.mtaryk')}</span>
          <input type='date' value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)} style={inputStyle} />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          <span style={{ fontSize: '11px', color: muted }}>{tr('admin.adTaryk')}</span>
          <input type='date' value={filterDateTo} onChange={e => setFilterDateTo(e.target.value)} style={inputStyle} />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          <span style={{ fontSize: '11px', color: muted }}>{tr('admin.amdh')}</span>
          <select value={filterPresetId} onChange={e => setFilterPresetId(e.target.value)} style={inputStyle}>
            <option value=''>{tr('admin.klHamdvt')}</option>
            {presets.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          <span style={{ fontSize: '11px', color: muted }}>{tr('admin.mshtmsh')}</span>
          <select value={filterCrewId} onChange={e => setFilterCrewId(e.target.value)} style={inputStyle}>
            <option value=''>{tr('admin.klHmshtmshym')}</option>
            {crewMembers.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        </div>
        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
          <button onClick={() => { setFilterEventType(''); setFilterDateFrom(today); setFilterDateTo(today); setFilterPresetId(''); setFilterCrewId(''); }} style={{ ...inputStyle, background: lightMode ? '#f1f5f9' : '#334155', cursor: 'pointer' }}>{tr('shared.reset')}</button>
        </div>
      </div>

      {/* Legend */}
      <div style={{ display: 'flex', gap: '12px', marginBottom: '10px', fontSize: '11px', flexWrap: 'wrap' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><span style={{ width: '12px', height: '12px', borderRadius: '2px', background: '#450a0a', border: '2px solid #ef4444', display: 'inline-block' }} />{tr('admin.kvnplyktKryty')}</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><span style={{ width: '12px', height: '12px', borderRadius: '2px', background: '#431407', border: '2px solid #f97316', display: 'inline-block' }} />{tr('admin.azhrhAvms')}</span>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', direction: 'rtl' }}>
          ...
          <tbody>
            {rows.length === 0 && !loading && (
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: '24px', color: muted }}>{tr('admin.aynRshvmvt')}</td></tr>
            )}
            {rows.map((row, i) => {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
                  {col.key !== '__queue__' && (
                    <button
                      ...
                      title={tr('admin.hvsfStrypLamvdh')}
                    >+</button>
                  )}
      ...
      {dnLoading ? <div style={{ color: '#64748b' }}>{tr('shared.loading')}</div> : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
          <div>
            ...
            {defArmNames.length === 0 && <div style={{ fontSize: '12px', color: '#475569', padding: '8px' }}>{tr('admin.aynShmvtMvgdrym')}</div>}
            <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
              <input value={newArmName} onChange={e => setNewArmName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addArm()} placeholder={tr('admin.shmChymvshChdsh')} style={inpStyle} />
              <button onClick={addArm} style={{ padding: '5px 14px', background: '#d97706', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}>{tr('shared.add')}</button>
            </div>
          </div>
          <div>
            <h3 style={{ fontSize: '14px', color: '#2dd4bf', margin: '0 0 12px 0' }}>{tr('admin.shmvtMarkvt')}</h3>
            ...
            {defSysNames.length === 0 && <div style={{ fontSize: '12px', color: '#475569', padding: '8px' }}>{tr('admin.aynShmvtMvgdrym')}</div>}
            <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
        ...
        <div key={cell.id} onClick={e => { e.stopPropagation(); setSelCellId(cell.id); }} title={cell.hint || undefined}
          ...
          {isSel && (
            <div style={{ display: 'flex', gap: '2px', position: 'absolute', bottom: '1px', left: 0, right: 0, justifyContent: 'center' }}>
              ...
              <button onMouseDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); mutate(t => sgSplit(t, cell.id, 'v')); }} title={tr('admin.ptslAnky')} style={{ fontSize: '9px', padding: '1px 3px', background: '#0e7490', color: 'white', border: 'none', borderRadius: '2px', cursor: 'pointer', lineHeight: 1 }}>⇅</button>
              {parentSplit && <button onMouseDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); mutate(t => sgRemove(t, cell.id)); setSelCellId(null); }} title={tr('shared.remove')} style={{ fontSize: '9px', padding: '1px 3px', background: '#7f1d1d', color: 'white', border: 'none', borderRadius: '2px', cursor: 'pointer', lineHeight: 1 }}>✕</button>}
            </div>
          )}
        </div>
      );
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: '#0a1628', border: '1px solid #1e3a5f', borderRadius: '12px', width: '90vw', maxWidth: '960px', height: '80vh', display: 'flex', flexDirection: 'column', color: '#e2e8f0', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: '1px solid #1e3a5f', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <button onClick={onClose} style={{ padding: '5px 10px', background: 'transparent', color: '#94a3b8', border: '1px solid #334155', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.close2')}</button>
          </div>
        </div>
        {/* Tabs */}
        ...
        {loading ? <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#475569' }}>{tr('shared.loading')}</div> : (
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                ...
                <div style={{ width: '260px', flexShrink: 0, borderInlineEnd: '1px solid #1e3a5f', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px', overflow: 'auto', background: '#070e1a' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: '10px', color: '#475569' }}>{tr('admin.grvrBgryd')}</span>
                  </div>
                </div>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '10px 12px 0', gap: '6px', overflow: 'hidden', minHeight: 0 }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.lchtsAlTaLbchyrh')}</div>
                  ...
                  {selCell ? (
                    <>
                      ...
                      <details open style={{ border: '1px solid #1e3a5f', borderRadius: '8px' }}>
                        <summary style={sumStyle}>{tr('admin.shdh2')}</summary>
                        <div style={bodyStyle}>
                      ...
                      <details style={{ border: '1px solid #1e3a5f', borderRadius: '8px' }}>
                        ...
                        <div style={bodyStyle}>
                          ...
                          {selCell.showTitle && <>
                            <div>
                              <label style={lbl}>{tr('admin.tkstKvtrtRykShm')}</label>
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <div style={{ flex: 1 }}><label style={lbl}>{tr('admin.rkaKvtrt2')}</label>
                              ...
                              <div style={{ flex: 1 }}><label style={lbl}>{tr('admin.tsbaTkst2')}</label>
                            ...
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
                              <div style={{ flex: 1 }}><label style={lbl}>{tr('admin.gvpnPx')}</label>
                            ...
                            <div><label style={lbl}>{tr('admin.yyshvrKvtrt')}</label>
                      ...
                      {/* 🎨 סגנון תוכן */}
                      <details open style={{ border: '1px solid #1e3a5f', borderRadius: '8px' }}>
                        <summary style={sumStyle}>{tr('admin.sgnvnTvkn')}</summary>
                        <div style={bodyStyle}>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <div style={{ flex: 1 }}><label style={lbl}>{tr('admin.rka2')}</label>
                          ...
                          <div>
                            <label style={lbl}>{tr('admin.rkaTkst')} <span style={{ fontSize: '10px', color: '#475569' }}>(highlight)</span>:</label>
                            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                              ...
                              {selCell.textBgColor && (<button onClick={() => mutate(t => sgUpdate(t, selCell.id, (n: SGCell) => ({ ...n, textBgColor: undefined })))} style={{ padding: '3px 7px', background: '#334155', color: '#94a3b8', border: '1px solid #475569', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', flexShrink: 0 }} title={tr('admin.hsrRkaTkst')}>✕</button>)}
                            </div>
                          </div>
                          <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
                            <div style={{ flex: 1 }}><label style={lbl}>{tr('admin.gvpnPx')}</label>
                          ...
                          <div><label style={lbl}>{tr('admin.yyshvr2')}</label>
                            <div style={{ display: 'flex', gap: '4px' }}>
                              {(['right','center','left'] as const).map(a => (
                      ...
                      <details style={{ border: '1px solid #1e3a5f', borderRadius: '8px' }}>
                        ...
                        <div style={bodyStyle}>
                          ...
                          {selCell.blink && <>
                            ...
                            <div><label style={lbl}>{tr('shared.rateSeconds')}</label>
                      ...
                      <details style={{ border: '1px solid #1e3a5f', borderRadius: '8px' }}>
                        ...
                        <div style={bodyStyle}>
                          <label style={lbl}>{tr('admin.tkstAzrhBrychvfTooltip')}</label>
                          <textarea value={selCell.hint ?? ''} onChange={e => mutate(t => sgUpdate(t, selCell.id, (n: SGCell) => ({ ...n, hint: e.target.value })))} rows={2} placeholder={tr('admin.tkstChvpshyShyvpyaBrychvf')}
                    ...
                    <div style={{ color: '#475569', fontSize: '12px', paddingTop: '20px', textAlign: 'center' }}>{tr('admin.lchtsAlTaLarykt')}</div>
                  )}
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'conditions' && (
              <div style={{ flex: 1, padding: '16px', overflow: 'auto', display: 'flex', flexDirection: 'column', gap: '14px', direction: 'rtl' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '13px', color: '#93c5fd', fontWeight: 'bold' }}>{tr('admin.kllyAytsvbMvtnh')}</span>
                ...
                {conditions.map(c => {
                  ...
                  return (
                    <div key={c.id} style={{ background: '#0f172a', border: '1px solid #1e3a5f', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      {/* Header row */}
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: '12px', color: '#60a5fa', fontWeight: 'bold' }}>{tr('admin.kllAytsvb')}</span>
                        <button onClick={() => { removeCondition(c.id); setDirty(true); }} style={{ padding: '2px 8px', background: '#7f1d1d', color: '#fca5a5', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px' }}>{tr('admin.mchk')}</button>
                      </div>
                      {/* Query Builder */}
                      <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '-4px' }}>{tr('admin.tnayHshaylth')}</div>
                      ...
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center', borderTop: '1px solid #1e293b', paddingTop: '8px' }}>
                        <span style={{ fontSize: '11px', color: '#64748b' }}>{tr('admin.hchlAl')}</span>
                        <select value={c.target} onChange={e => { updateCondition(c.id, { target: e.target.value as any, targetCellId: undefined }); setDirty(true); }}
                          ...
                          <option value="strip">{tr('admin.klHkrtys')}</option>
                          <option value="all">{tr('admin.klHtaym')}</option>
                          <option value="cell">{tr('admin.taSptsypy')}</option>
                        </select>
                        {c.target === 'cell' && (
  ...
  return (
    <div style={{ padding: '16px', maxWidth: '960px', direction: 'rtl' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
        <span style={{ fontWeight: 'bold', fontSize: '15px', color: '#e2e8f0' }}>{tr('admin.sgyrvt')}</span>
        <button onClick={openNew} style={{ padding: '5px 14px', background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>{tr('admin.sgyrhChdshh')}</button>
        <button onClick={() => importFileRef.current?.click()} style={{ padding: '5px 14px', background: '#065f46', color: '#6ee7b7', border: '1px solid #059669', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '5px' }}>{tr('admin.tanMkvbtsExcel')}</button>
        <input ref={importFileRef} type="file" accept=".xlsx,.xls" style={{ display: 'none' }} onChange={handleImportFile} />
        {loading && <span style={{ color: '#64748b', fontSize: '11px' }}>{tr('shared.loading')}</span>}
      </div>
        ...
        <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '12px' }}>
            ...
            <div>{lbl('סטטוס סגירה')}
              <select value={form.closure_status} onChange={e => setForm(f => ({ ...f, closure_status: e.target.value }))} style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '4px', color: '#e2e8f0', padding: '5px 8px', fontSize: '12px', width: '100%' }}>
                <option value="coordinated">{tr('admin.mtvamt')}</option>
                <option value="approved">{tr('admin.mavshrt')}</option>
                <option value="pending">{tr('admin.mmtynhLayshvr')}</option>
                <option value="cancelled">{tr('admin.bvtlh')}</option>
              </select>
            </div>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
              ...
              <button onClick={addDate} style={{ padding: '5px 12px', background: '#1e3a5f', color: '#93c5fd', border: '1px solid #3b82f6', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', whiteSpace: 'nowrap' }}>{tr('shared.add')}</button>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
              ...
              {form.dates.length === 0 && <span style={{ color: '#475569', fontSize: '11px' }}>{tr('admin.aynTarykym')}</span>}
            </div>
          </div>
          ...
          <div style={{ marginBottom: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
              ...
              <button onClick={openMapPicker} style={{ padding: '3px 10px', background: '#1a3a4a', color: '#38bdf8', border: '1px solid #0ea5e9', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', whiteSpace: 'nowrap', marginBottom: '3px' }}>{tr('admin.dkvrAlMph')}</button>
            </div>
            <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
              ...
              <button onClick={addPoint} style={{ padding: '5px 12px', background: '#1e3a5f', color: '#93c5fd', border: '1px solid #3b82f6', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', whiteSpace: 'nowrap' }}>{tr('shared.add')}</button>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                <thead>
                  <tr style={{ color: '#64748b' }}>
                    ...
                    <th style={{ textAlign: 'right', padding: '3px 6px', fontWeight: 'normal' }}>{tr('admin.kvRvchb')}</th>
                    <th style={{ textAlign: 'right', padding: '3px 6px', fontWeight: 'normal' }}>{tr('admin.kvAvrk')}</th>
            ...
            {form.polygon_geo.length === 0 && <span style={{ color: '#475569', fontSize: '11px' }}>{tr('admin.aynNkvdvtPvlygvn')}</span>}
          </div>

          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={save} style={{ padding: '6px 18px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.save2')}</button>
            <button onClick={cancel} style={{ padding: '6px 18px', background: '#374151', color: '#e2e8f0', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>{tr('shared.cancel')}</button>
          </div>
        </div>
      )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          ...
          <tbody>
            {closures.length === 0 && !loading && (
              <tr><td colSpan={10} style={{ textAlign: 'center', padding: '24px', color: '#475569' }}>{tr('admin.aynSgyrvtLchtsSgyrh')}</td></tr>
            )}
            {closures.map(c => (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.82)', zIndex: 9200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '10px', width: '92vw', maxWidth: '1100px', height: '88vh', display: 'flex', flexDirection: 'column', direction: 'rtl', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 16px', borderBottom: '1px solid #334155', flexShrink: 0 }}>
              {mapPicker.step === 'draw' && (
                <button onClick={() => setMapPicker(p => p ? { ...p, step: 'list', selected: null, points: [] } : p)} style={{ background: '#374151', border: 'none', color: '#94a3b8', padding: '3px 10px', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', whiteSpace: 'nowrap' }}>{tr('shared.back')}</button>
              )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '10px', padding: '20px', width: '90vw', maxWidth: '1000px', maxHeight: '85vh', display: 'flex', flexDirection: 'column', direction: 'rtl', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-start', borderTop: '1px solid #334155', paddingTop: '12px' }}>
              ...
              <button onClick={() => setImportRows(null)} disabled={importing} style={{ padding: '7px 18px', background: '#374151', color: '#e2e8f0', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.cancel')}</button>
            </div>
          </div>
        </div>
      )}
...
export const StripWindowAdmin = ({ apiUrl }: { apiUrl: string }) => {
    ...
    return (
      <div key={node.id} onClick={() => setSelLeafId(node.id)}
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', padding: '4px', flexWrap: 'wrap' }}>
          <button onClick={e => { e.stopPropagation(); mutate(t => swSplit(t, node.id, 'h')); }} style={btnSm('#1d4ed8')} title={tr('admin.chlkLalyvnVtchtvn')}>{tr('admin.shvrh')}</button>
          <button onClick={e => { e.stopPropagation(); mutate(t => swSplit(t, node.id, 'v')); }} style={btnSm('#065f46')} title={tr('admin.chlkLshmalVymyn')}>{tr('admin.amvdh')}</button>
        </div>
      </div>
    );
  };
  ...
  const sidebar = (
    <div style={{ width: '220px', flexShrink: 0, background: T.card, borderInlineEnd: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '10px', borderBottom: `1px solid ${T.border}` }}>
        <div style={{ fontSize: '13px', fontWeight: 'bold', marginBottom: '8px', color: T.text }}>{tr('admin.chlvnvtStrypym')}</div>
        <div style={{ display: 'flex', gap: '4px' }}>
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder={tr('admin.shmChdsh')}
        ...
        <div style={{ marginTop: '6px' }}>
          <button type="button" onClick={() => setShowTplPicker(p => !p)}
            ...
            <span>{tr('admin.tbnyt')} <strong style={{ color: T.text }}>{SW_TEMPLATES.find(t => t.id === newTplId)?.label || 'ריק'}</strong></span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px' }}>
        {layouts.length === 0 && <div style={{ color: T.muted, fontSize: '11px', textAlign: 'center', padding: '16px 0' }}>{tr('admin.aynChlvnvt')}</div>}
        {layouts.map(lay => (
          <div key={lay.id}>
            {renamingId === lay.id ? (
  ...
  return (
    <div style={{ display: 'flex', height: '100%', direction: 'rtl', color: T.text, overflow: 'hidden', position: 'relative' }}>
      {sidebar}
      {!selLay ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.muted, fontSize: '14px' }}>{tr('admin.bchrChlvnMhrshymhAv')}</div>
      ) : (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '8px 14px', background: T.card, borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
            ...
            <div style={{ marginInlineStart: 'auto', display: 'flex', gap: '6px' }}>
              {dirty && <button onClick={save} style={{ background: '#16a34a', color: 'white', border: 'none', padding: '5px 12px', borderRadius: '5px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>{tr('shared.save2')}</button>}
              <button onClick={() => setFullScreen(true)} style={{ background: '#7c3aed', color: 'white', border: 'none', padding: '5px 12px', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' }}>{tr('admin.arvkBgdvl')}</button>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: '#070d1a', zIndex: 9999, display: 'flex', flexDirection: 'column', direction: 'rtl' }}>
          <div style={{ background: '#1e293b', borderBottom: '1px solid #334155', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
            ...
            <div style={{ marginInlineStart: 'auto', display: 'flex', gap: '8px' }}>
              {dirty && <button onClick={save} style={{ background: '#16a34a', color: 'white', border: 'none', padding: '6px 14px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}>{tr('shared.save2')}</button>}
              <button onClick={() => { setFullScreen(false); setSelLeafId(null); }} style={{ background: '#334155', color: '#f1f5f9', border: 'none', padding: '6px 14px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>{tr('shared.close2')}</button>
            </div>
          </div>
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            {selLeaf && (
              <div style={{ width: '270px', background: '#1e293b', borderInlineStart: '1px solid #334155', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
                <div style={{ padding: '10px 12px', borderBottom: '1px solid #334155', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ fontWeight: 'bold', fontSize: '13px' }}>{tr('admin.hgdrvtTa')}</span>
                  <button onClick={() => { if (confirm('למחוק תא זה?')) { mutate(t => swRemove(t, selLeaf.id)); setSelLeafId(null); } }}
                    style={{ marginInlineStart: 'auto', background: 'transparent', color: '#f87171', border: '1px solid #f87171', borderRadius: '4px', padding: '2px 7px', cursor: 'pointer', fontSize: '11px' }}>{tr('shared.delete3')}</button>
                </div>
                <div style={{ flex: 1, overflowY: 'auto', padding: '10px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <div>
                    ...
                    <select value={selLeaf.waypoint || ''} onChange={e => mutate(t => swUpdate(t, selLeaf.id, (n: SWLeaf) => ({ ...n, waypoint: e.target.value, waypoint_mode: e.target.value ? n.waypoint_mode : undefined })))}
                      ...
                      <option value="">{tr('admin.llaNkvdtMabr')}</option>
                      {swSectors.map((s: any) => (
                    ...
                    {selLeaf.waypoint && (
                      <div style={{ marginTop: '6px' }}>
                        <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '4px' }}>{tr('admin.kyvvnNkvdtMabr')}</div>
                        <div style={{ display: 'flex', gap: '6px' }}>
                        ...
                        {!selLeaf.waypoint_mode && (
                          <div style={{ fontSize: '10px', color: '#64748b', marginTop: '4px', textAlign: 'center' }}>{tr('admin.bchrKyvvnKdyLsnn')}</div>
                        )}
                      </div>
                    )}
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '3px' }}>{tr('admin.rkaTa')}</div>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '3px' }}>{tr('admin.rkaKvtrt')}</div>
                      <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '5px' }}>{tr('admin.mrkmRka')}</div>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '3px' }}>{tr('admin.tsbaTkstKvtrt')}</div>
                      <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '4px' }}>{tr('admin.shaylttSynvn')}</div>
                  ...
                  <div style={{ borderTop: '1px solid #1e293b', paddingTop: '10px' }}>
                    <div style={{ fontSize: '10px', color: '#94a3b8', marginBottom: '6px', fontWeight: 'bold', letterSpacing: '0.5px' }}>{tr('admin.kvtrtTvknMalHstrypym')}</div>
                    <input value={selLeaf.content_title || ''} onChange={e => mutate(t => swUpdate(t, selLeaf.id, (n: SWLeaf) => ({ ...n, content_title: e.target.value })))}
                      placeholder={tr('admin.tkstHkvtrtRykLla')}
                    ...
                    {selLeaf.content_title && (<>
                      <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '3px' }}>{tr('admin.tsbaTkst')}</div>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '3px' }}>{tr('admin.rka')}</div>
                          <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          ...
          <div style={{ background: '#0f172a', borderTop: '1px solid #1e293b', padding: '5px 14px', fontSize: '10px', color: '#475569', display: 'flex', gap: '16px' }}>
            <span>{tr('admin.lchtsAlTaLbchyrh2')}</span>
            <span>{tr('admin.grvrMchytshShynvyGvdl')}</span>
            <span><b style={{ color: '#94a3b8' }}>{tr('admin.shvrh')}</b> = עליון/תחתון &nbsp;·&nbsp; <b style={{ color: '#94a3b8' }}>{tr('admin.amvdh')}</b> {tr('admin.shmalYmyn')}</span>
          </div>
        </div>
      )}
    </div>
