    ...
    <div style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '6px' }}>
        ...
        <span
          ...
          title={tr('transfers.clickToUpdateAltitude')}
      ...
      <div style={{ fontSize: '10px', color: '#92400e', marginTop: '3px', opacity: 0.75 }}>{tr('transfers.awaitingAcknowledgement')}</div>
      <button 
...
// --- כרטיס מוסר בסקטור ---
export const OutgoingTransferCard = ({ t, isConflict, isAltViolation = false, onCancel, onDismiss, onUpdateStripField, lightMode = false, presetId, onUpdateNote }: {
  t: any;
  isConflict: boolean;
  ...
  // מצב ההעברה אצל המוסר: אושר=ירוק · נדחה=כתום · קונפליקט=אדום (אדום מנצח)
  ...
  return (
    <>
      <div style={{
        ...
        border: isConflict ? '1px solid #ef4444' : isRejected ? '1px solid #f97316' : isAck ? '1px solid #22c55e' : isAltViolation ? '1px solid #f97316' : (lightMode ? '1px solid #d97706' : '1px solid #78350f'),
        background: isConflict ? (lightMode ? '#fef2f2' : '#450a0a') : isRejected ? (lightMode ? '#fff7ed' : '#2a1400') : isAck ? (lightMode ? '#f0fdf4' : '#052e16') : isAltViolation ? (lightMode ? '#fff7ed' : '#1c0800') : (lightMode ? '#fffbeb' : '#0d0800'),
      }}>
        {(isAck || isRejected) && (
          <div style={{ fontSize: '8px', fontWeight: 'bold', marginBottom: '2px', color: isRejected ? (lightMode ? '#c2410c' : '#fb923c') : (lightMode ? '#15803d' : '#4ade80') }}>
            {isRejected ? '✗ נדחה' : '✓ אושר'}
          </div>
        )}
        {/* שורה 1: 💬 | callsign | sq */}
          ...
          <div style={{ marginBottom: '3px' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '3px', marginTop: '2px' }}>
              <button onClick={e => { e.stopPropagation(); if (onUpdateNote) onUpdateNote(String(t.id), editBuffer); setNoteOpen(false); }}
                style={{ flex: 1, fontSize: '9px', padding: '2px', background: '#1e40af', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontWeight: 'bold' }}>{tr('shared.save')}</button>
        ...
        {isRejected && t.reject_note && (
          <div style={{ fontSize: '9px', color: lightMode ? '#9a3412' : '#fdba74', background: lightMode ? '#ffedd5' : '#3a1c00', borderRadius: '3px', padding: '2px 5px', marginBottom: '3px', whiteSpace: 'pre-wrap', lineHeight: 1.4, border: `1px solid ${lightMode ? '#fb923c' : '#7c2d12'}`, direction: 'rtl' }}>
            📌 {t.reject_note}
          </div>
        )}
        {isRejected ? (
          <button
            ...
            title={tr('transfers.gotItRemoveThe')}
...
export const CompactTransferRow = ({ t, dir, isConflict, isAltViolation = false, onUpdateStripField, onAction, lightMode = false, shrunk = false }: {
  ...
  const isAck = t.status === 'acknowledged';
  const isRejected = t.status === 'rejected';
  // צבע ראשי: קונפליקט אדום · מוסר: נדחה כתום / אושר ירוק / רגיל ענבר · מקבל ירוק
  const main = isConflict ? '#ef4444' : isOut ? (isRejected ? '#f97316' : isAck ? '#22c55e' : '#f59e0b') : '#22c55e';
  const txt = isConflict ? (lightMode ? '#b91c1c' : '#fca5a5') : isOut ? (isRejected ? (lightMode ? '#c2410c' : '#fdba74') : isAck ? (lightMode ? '#15803d' : '#86efac') : (lightMode ? '#92400e' : '#fcd34d')) : (lightMode ? '#15803d' : '#86efac');
  const bg = isConflict ? (lightMode ? '#fef2f2' : '#3f0a0a') : isOut ? (isRejected ? (lightMode ? '#fff7ed' : '#2a1400') : isAck ? (lightMode ? '#f0fdf4' : '#052e16') : (lightMode ? '#fffbeb' : '#140c00')) : (lightMode ? '#f0fdf4' : '#031106');
  return (
    <>
...
export const IncomingTransferCard = ({ t, isConflict, onAccept, onReject, onAcknowledge, onUpdateStripField, onReply, onSendDirectReply }: {
  t: any;
  isConflict: boolean;
  ...
  return (
    <div className={isConflict ? 'alt-conflict-flash' : ''} style={{
      background: isConflict ? '#7f1d1d' : isAck ? '#cffafe' : '#dcfce7',
      border: `1px solid ${isConflict ? '#ef4444' : isAck ? '#06b6d4' : '#22c55e'}`,
      borderRadius: '4px',
      padding: '5px',
      marginBottom: '4px',
      direction: 'rtl'
      ...
      {countdown !== null && (
        <div style={{ textAlign: 'center', marginBottom: '3px' }}>
          <span title={tr('transfers.timeUntilArrivalAt')} style={{ fontSize: '12px', fontWeight: 'bold', color: countdownOver ? '#ef4444' : '#15803d', background: countdownOver ? '#450a0a' : '#bbf7d0', border: `1px solid ${countdownOver ? '#dc2626' : '#22c55e'}`, borderRadius: '4px', padding: '1px 8px', fontVariantNumeric: 'tabular-nums', letterSpacing: '0.5px' }}>
            ⏱ {countdown}
          </span>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '2px' }}>
        <button onClick={(e) => { e.stopPropagation(); onAccept(t.id); }} title={tr('transfers.passedToMeAccept')}
          style={{ flex: 1, padding: '2px', background: '#22c55e', color: 'white', border: 'none', borderRadius: '2px', fontSize: '8px', cursor: 'pointer', fontWeight: 'bold' }}>{tr('shared.accept')}</button>
        {!isAck && onAcknowledge && (
          <button onClick={(e) => { e.stopPropagation(); onAcknowledge(t.id); }} title={tr('transfers.acknowledgeReceiptNotPassed')}
            style={{ flex: 1, padding: '2px', background: '#0891b2', color: 'white', border: 'none', borderRadius: '2px', fontSize: '8px', cursor: 'pointer' }}>{tr('shared.acknowledge')}</button>
        )}
        <button onClick={(e) => { e.stopPropagation(); setShowRejectBox(v => !v); setRejectNote(''); }} title={tr('transfers.rejectWithANote')}
          style={{ flex: 1, padding: '2px', background: showRejectBox ? '#991b1b' : '#dc2626', color: 'white', border: 'none', borderRadius: '2px', fontSize: '8px', cursor: 'pointer' }}>{tr('transfers.reject')}</button>
        {(onReply || onSendDirectReply) && (
          <button onClick={(e) => { e.stopPropagation(); setShowReplyBox(v => !v); setReplyText(''); }}
            title={tr('transfers.writeANoteTo')}
      ...
      {showRejectBox && (
        <div style={{ marginTop: '4px', display: 'flex', flexDirection: 'column', gap: '3px' }} onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
          <textarea autoFocus value={rejectNote} onChange={(e) => setRejectNote(e.target.value)}
            placeholder={tr('transfers.rejectionReasonRequired')} rows={2}
          ...
          <div style={{ display: 'flex', gap: '2px' }}>
            <button disabled={!rejectNote.trim()}
              ...
              style={{ flex: 1, padding: '2px', background: rejectNote.trim() ? '#dc2626' : '#6b7280', color: 'white', border: 'none', borderRadius: '2px', fontSize: '9px', cursor: rejectNote.trim() ? 'pointer' : 'not-allowed', fontWeight: 'bold' }}>{tr('transfers.sendRejection')}</button>
            <button onClick={() => { setShowRejectBox(false); setRejectNote(''); }}
              style={{ padding: '2px 6px', background: '#374151', color: '#9ca3af', border: 'none', borderRadius: '2px', fontSize: '9px', cursor: 'pointer' }}>{tr('shared.cancel')}</button>
          </div>
        </div>
      )}
      ...
      {showReplyBox && (
        <div style={{ marginTop: '4px', display: 'flex', flexDirection: 'column', gap: '3px' }} onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
          <textarea
            ...
            placeholder={tr('transfers.writeAMessageTo')}
            rows={2}
