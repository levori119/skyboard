...
// כפתורים נוצרים בקליק ימני על הלוח, נגררים למיקום חופשי (Pointer Events —
// מותאם Cintiq/עט), לכל כפתור מצבים עם צבע, טקסט חופשי, פונט/גודל, וטריגר
// התראה מתפרצת לעמדות אחרות (מנגנון workstation-messages הקיים).
...
export default function ButtonsBoard({ serviceName, state, onChange, presetId, presetName, allPresets, theme, onInteracting, postLog }: Props) {
  ...
  const pctFromEvent = (e: { clientX: number; clientY: number }) => {
    ...
    if (!r) return { x: 10, y: 10 };
    return { x: Math.min(95, Math.max(0, ((e.clientX - r.left) / r.width) * 100)), y: Math.min(95, Math.max(0, ((e.clientY - r.top) / r.height) * 100)) };
  };
        ...
        return (
          <div
            ...
            style={{
              ...
              border: '1px solid rgba(0,0,0,0.35)', borderRadius: 8,
        ...
        <>
          ...
          <div style={{ position: 'absolute', left: menu.x, top: menu.y, zIndex: 61, background: theme.panel, border: `1px solid ${theme.border}`, borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.4)', minWidth: 140, overflow: 'hidden' }}>
            {menu.btnId === null ? (
              ...
              <>
                <button onClick={() => { const b = buttons.find(x => x.id === menu.btnId); if (b) setEditing({ ...b, states: b.states.map(s => ({ ...s })) }); setMenu(null); }}
                  style={{ display: 'block', width: '100%', padding: '9px 14px', background: 'none', border: 'none', color: theme.text, cursor: 'pointer', fontSize: 14, textAlign: 'start' }}>
                  ✏️ {tr('missiondesk.editButton')}
                </button>
                <button onClick={() => deleteButton(menu.btnId!)}
      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 70, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setEditing(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: theme.panel, border: `1px solid ${theme.border}`, borderRadius: 12, padding: 18, width: 'min(440px, 92vw)', maxHeight: '86vh', overflowY: 'auto', color: theme.text }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 16 }}>{tr('missiondesk.buttonEditorTitle')}</h3>
