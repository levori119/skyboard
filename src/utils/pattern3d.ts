...
/**
 * מבט הפתיחה. `tilt: 10` - **הכרעת הפקח מהשטח**, אחרי 55 ואחרי 30.
 *
 * הרכיב האנכי של ההיטל הוא `cos(tilt)`, אבל גובה המסך של עצם הוא
 * `ry·sin(tilt) − גובה·cos(tilt)` - כלומר תלוי גם בעומק. באותו גובה בדיוק, עצם
 * דרומי מצויר נמוך יותר, והטעות הנראית ברגל היא `Δy·tan(tilt)/zScale`.
 * דווח מהשטח (2026-09-15): ב-30° מטוס תמונ"א ב-7000 מעל מרכז השדה נראה כמו
 * 2000 מול "שמשון" שבקצה הצפוני - "הרכיב האווירי תמיד נמוך מהמציאות".
 * ב-10° אותה טעות היא כ-1500 רגל, כשליש. המחיר: הקרקע שטוחה יותר ועומק צפון-
 * דרום נדחס; המצלמה עדיין מוטה ביד כשצריך לקרוא את המישור.
 */
export const DEFAULT_CAMERA: Camera3D = { yaw: 0, tilt: 10, zoom: 1 };
...
/**
 * האם לרנדר את ההקפה התלת מימדית.
 *
 * **הכלל: כפתור שנדלק חייב להראות משהו.** קודם המצב המלא (`overlay`) היה מותנה
 * גם ב-`imgBounds` - גבולות משטח המפה - ולכן בעמדה שבה המדידה לא הסתדרה הפקח
 * לחץ, הכפתור נצבע בתורכיז, ו**שום דבר לא קרה**: לא סצנה ולא סרגל בקרה. אין
 * דרך להבחין בין זה לבין פיצ'ר שבור, וזה בדיוק מה שדווח מהשטח.
 *
 * הסצנה אינה זקוקה ל-`imgBounds` כדי לעלות: היא מקבלת **יחס** בלבד, ו-
 * `boundsAspect(null)` מחזיר 1 - יחס תקין למסגרת ריבועית. לכן היחיד שנשאר
 * מהתנאי הוא המתג עצמו.
 */
