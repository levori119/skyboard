...
**מצב נוכחי (31.07.2026):** 112/120 ב-`public`, 59 בכל סכמת תרגול (טבלאות
תפעוליות בלבד). 9 FK עדיין חסומים בגלל שורות יתומות היסטוריות — הם מדווחים
בלוג בכל עלייה עד שהנתונים יטופלו.
...
## קישורי מסלולים בין עמדות — `route_link_groups` ו-`route_link_members`

אותו מסלול פיזי נראה בכמה עמדות ב**שמות שונים**. קישור מצהיר שהם אותו דבר, כדי
שהתראות (למשל המראה פעילה) יחצו עמדות.

**המודל הקודם, `route_links`, היה זוגי** (`preset_id_a/route_id_a` מול
`preset_id_b/route_id_b`): קישור בין שלוש עמדות דרש שלושה זוגות נפרדים, כל אחד
...
### טבלת `route_link_members` — עמדה + מסלול בתוך הקישור
...
| `preset_id` | INT → workstation_presets | העמדה (CASCADE) |
| `route_id` | INT → airfield_routes | המסלול (CASCADE) |

**`UNIQUE(group_id, preset_id, route_id)`** — אותו צמד לא נכנס פעמיים. **אותה עמדה
עם שני מסלולים שונים מותרת**: עמדה יכולה לראות שני מסלולים שמקושרים לאותו מסלול
פיזי אצל שכנתה.

**כללי תקינות (`src/utils/routeLinks.ts`, נאכפים גם בשרת):** לפחות שני חברים,
לכל חבר עמדה ומסלול, בלי אותו צמד פעמיים.
