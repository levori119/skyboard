...
## יומן הסנכרון — `local_sync_journal` (במאגר המקומי בלבד)

מה העמדה שינתה בזמן שהייתה מנותקת. **אינה קיימת במאגר המרכזי**: שם מה שנכתב
כבר נמצא במקור האמת. נוצרת ב-[`server/local.js`](server/local.js) **אחרי**
`initDb`/`seedDb`, כדי שנתוני האתחול לא ייכנסו לתור הסנכרון.

| עמודה | טיפוס | תיאור |
|---|---|---|
| `id` | BIGSERIAL | סדר הכתיבה — גם סדר האיחוד והדחיפה |
| `at` | TIMESTAMPTZ | מתי נכתב בעמדה |
| `table_schema` / `table_name` | TEXT | איפה. הסכמה נשמרת לתצוגה; **המרכז מחיל בסכמת הבקשה** ולא בזו שהעמדה שלחה |
| `op` | CHAR(1) | `I` / `U` / `D` |
| `pk` | JSONB | המפתח הראשי, גנרי (יש גם מפתחות מורכבים) |
| `before` / `after` | JSONB | השורה לפני ואחרי. ה-`rev` שב-`before` **הראשון** לשורה הוא הגרסה שהמרכז החזיק |
| `action_id` | TEXT | קישור לפעולת הביטול, כשהייתה |
| `status` | TEXT | `pending` → `synced` · `superseded` (המרכז עודכן אחרי, וגרסתו אומצה כאן) · `conflict` (לא ניתן היה להכריע) · `dropped` |
| `conflict_reason` · `server_row` | TEXT · JSONB | מדוע נפלה ההכרעה כך, ומה המרכז מחזיק — ההזנה של יומן ההכרעות |

- **ההכרעה: האחרון מנצח.** כששני הצדדים נגעו באותה שורה, מנצח מי שעדכן מאוחר
  יותר; העמדה שולחת את שעונה בדחיפה והמרכז מתרגם לשעון שלו לפני ההשוואה.
  מחיקה במרכז מנצחת תמיד. רק כשאין חותמת זמן נדרש אדם. ראה [ARCHITECTURE.md](ARCHITECTURE.md) §נתק 4.
- **הטריגר** `sync_journal_row` יושב על חמש הטבלאות של §מעקב גרסה, ועליהן בלבד.
- **`app.sync_apply`** מכבה את הרישום בזמן קליטת מראה ובאימוץ גרסת שרת. בלעדיו
  כל שורה שהמרכז שלח הייתה נדחפת אליו בחזרה — לולאה שמייצרת סתירות יש מאין.
  הסימון הוא `SET LOCAL`, ולכן **חייב טרנזקציה** (`withoutJournal`).
- העדכון מושווה **בלי** `rev` ו-`updated_at`: טריגר הגרסה כבר העלה אותם, ולכן
  בלי ההחרגה תנאי ה"לא השתנה דבר" היה מת והיומן היה מתמלא בכתיבות ריקות.
- **מסווגת `ignored`** ב-[`env-tables.js`](server/db/env-tables.js) ונמצאת
  **ברשימת החסימה של הביטול** ב-[`undoJournal.js`](server/db/undoJournal.js):
  היא control-plane, ו-CTRL+Z עליה היה מוחק שינוי שטרם הגיע למרכז.

### גוש המזהים המקומי

`strips.id` הוא `SERIAL`. עמדה מנותקת שיוצרת פ"מ הייתה מקבלת מזהה שהמרכז כבר
חילק לפ"מ אחר, ובסנכרון אחד מהם היה נדרס בשקט. לכן בעליית המאגר המקומי **כל**
רצפי המפתח הראשי מוזזים לגוש של העמדה:

```
1,500,000,000 + block × 100,000     (6,000 גושים · נכנס ב-int4)
```

`SKYKING_STATION_ORDINAL` קובע את הגוש במפורש; בלעדיו הוא נגזר מגיבוב יציב של
מפתח העמדה. ההזזה היא `setval(..., false)` ואינה זזה אחורה, ולכן ריצה חוזרת
אינה מחלקת שוב מזהה שכבר ניתן.
ראה [`server/db/localIds.js`](server/db/localIds.js).
...
| `mission_desk_view_tables` | JSONB DEFAULT `'{}'` | שירות **"טבלאות מתצוגה"**, פר-עמדה: `{ "<view_tables_service_id>": { tables[], airfield_id, stations{} } }`. `stations` - לפי איזו עמדה מוצגת כל טבלה (`{ messages: 12 }`; טבלה בלי ערך = העמדה הזו). `tables` מתוך `elements`/`trips`/`drivers`/`quantities`/`messages`/`container` (`MD_VIEW_TABLES`). `airfield_id` - לנסיעות ולנהגים של העמדה הזו, כי לעמדת דסק אין שדה משלה; לפי עמדה אחרת השדה מגיע ממנה (`mdViewTableSource`). טבלה שנבחרה בכמה משבצות מוצגת רק בראשונה בפריסה (`mdViewTableOwners`) |
...
| `alt_tol_above_ft` | INT | מעקב הקפה אוטומטי: כמה **מעל** הגובה המתוכנן על הצלע מותר, רגל. NULL = 1500 (בקוד) |
| `alt_tol_below_ft` | INT | מעקב הקפה אוטומטי: כמה **מתחת** לגובה המתוכנן מותר, רגל. NULL = 500 (בקוד) |
| `alt_tolerance_ft` | INT | **מיושן** - הוחלף במעל/מתחת. ערך שנשמר בו מועבר לשתיהן בעלייה ומתאפס. לא לכתוב |
...
| `expand_aircraft` | BOOLEAN NOT NULL DEFAULT FALSE | **מטוסים בלבד**: הטבלה בעמדה מציגה כל מטוס כשורה, בלי שורת הפ"מ (פעולות הפ"מ על המטוס הראשון במבנה) |
| `sort_order` | INT | סדר |
| `created_at` | TIMESTAMPTZ | חותמת |
...
**משותף לבסיס האב:** הדת"קים שייכים לבסיס ולא לשדה ("דת"ק 1" בשדה הקרקעי, האווירי וההקפה של
אותו בסיס הוא אותו דת"ק). לכן: (1) החלוקה מחפשת נקודות דת"ק בכל השדות שלהם אותו `airfields.base_id`,
כשהשדה של נקודת ההצטרפות ראשון; (2) שמירה (POST/PUT עם השדה) כותבת את הרשימה לאותו מספר דת"ק בשאר
שדות הבסיס; (3) `GET /api/airfields/:id/points` מחזיר לנקודת דת"ק בלי רשימה משלה את זו של הבסיס -
כדי ששמירה מהטופס לא תמחק אותה; (4) נקודת דת"ק חדשה בלי רשימה יורשת את זו של הבסיס. שדה בלי בסיס
רואה רק את עצמו. מימוש: `server/utils/baseDatkPoints.js` + `effectiveLandingPriority`.
...
| `driver_started_at` | TIMESTAMPTZ | הנהג לחץ "הפעל נסיעה" (`POST /api/driver-trips/:id/start`) - מותר רק לנסיעה מאושרת, מחצי שעה לפני עד חצי שעה אחרי `scheduled_at`. מקפיץ למגדל התראה. **אינה** עוברת לעותק |
| `driver_app_background_at` | TIMESTAMPTZ | אפליקציית הנהג ברקע מאז (2026-09-19). נכתב ב-`POST /api/driver-trips/:id/app-state` ובקריאת GPS עם `background:true`, מתנקה בחזרה לחזית. NULL = בחזית. המגדל מציג "האפליקציה ברקע אצל הנהג" במקום "אות אבד" עמום |
| `pending_change_prev_status` | VARCHAR(20) | עדכון מהנהג מחזיר את הנסיעה ל-`pending` (גם אם אושרה), וכאן נשמר הסטטוס שלפניו - פעם אחת, עדכון נוסף אינו דורס. אישור העדכון מחזיר אותו (אלא אם השתנו תחנות - אז הנתיב נמחק ונשאר `pending`); דחייה מחזירה אותו. NULL = אין עדכון ממתין. **אינה** עוברת לעותק |
...
### `airfield_elements.road_relevance` - על איזה נתיב נסיעה האלמנט שולט

| עמודה | טיפוס | הערות |
|---|---|---|
| `road_relevance` | JSONB | `[{route_id, cross_route_id}]` - מזהי `base_routes`. ריק = בלי הצהרה |

נקבע בטופס האלמנט בניהול ("נתיבי נסיעה שהאלמנט שולט עליהם"): נתיב הנסיעה שעליו
הרמזור/המחסום שולט, ואופציונלית **הצומת** שלפניו (`cross_route_id`; ריק = האלמנט
עומד באמצע נתיב ולא בצומת - מקרה לגיטימי, ולכן ההגדרה אינה חובה).

**למה:** עד כאן השיוך נוחש גאומטרית - המתכנן תפס כל אלמנט ברדיוס 150 מ' מצומת
שזוהתה ברדיוס 60 מ', והמפה של הנהג סימנה "על הנתיב" בפרוזדור 40 מ'. בשדה
הרמזורים עומדים 228 מ' ויותר מקו הנתיב, ולכן הפרוזדור החטיא והרדיוס תפס רמזורים
של צומת אחרת. **ההצהרה גוברת על הגאומטריה, ורק לאלמנט שהוגדר** - אלמנט בלי
הצהרה ממשיך בדיוק כפי שהיה, ולא נעלם בשקט. הכלל ב-[`shared/elementRoadRelevance.js`](shared/elementRoadRelevance.js).

הנתיב השמור נושא מעתה `routeId` בכל נקודה (`compactWaypoints`), ובלעדיו אי אפשר
לדעת באילו נתיבים הנסיעה עוברת: נסיעה שאושרה לפני השינוי חוזרת לכלל הגאומטרי.
**שכפול שדה** אינו מעתיק את ההצהרה - `base_routes` אינם משוכפלים, ושיוך לנתיב
של השדה המקורי היה שגוי בשקט.
...
### `status='pending_transfer'` הוא גלובלי - וההחזקה פר-עמדה

שורת `strips` אחת נושאת `status` ו-`workstation_preset_id` (שהוא גם **יעד ההעברה**),
אבל את אותו פ"מ מחזיקות כמה עמדות במקביל. לכן שליחה מעמדה אחת אינה אמורה להשפיע
על עמדה אחרת שמחזיקה אותו: הלקוח **מטיל** את שתי העמדות הללו לערכים "שלי" עבור
עמדה מחזיקה שאינה צד בהעברה (`src/utils/stripTransferView.ts`), ושומר את המקור
ב-`raw_status` / `raw_workstation_preset_id`. הצדדים - השולח והמקבל - רואים את
המצב האמיתי.
...
### `base_routes` - נתיבי נסיעה (כבישים / הסעה / טיסה לתכנון נסיעת רכב)

| עמודה | טיפוס | הערות |
|---|---|---|
| `waypoints` | JSONB | נקודות הנתיב **בסדר הציור** (`x`,`y` באחוזי מפה + `lat`,`lon` אחרי עיגון) |
| `route_type` | VARCHAR(20) | `vehicle` / `taxiway` / `runway` - לפי הרשאת הנהג |
| `direction` | VARCHAR(10), ברירת מחדל `'both'` | **כיוון נסיעה** (2026-09-19): `both` דו-כיווני, `forward` חד-כיווני בסדר הציור, `backward` הפוך. תכנון הנתיב (`server/utils/roadGraph.js`) לא נוסע נגד הכיוון. ערך זר נשמר כ-`both` |
