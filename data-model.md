...
| | |
|---|---|
| **קריאה בלבד** | `readOnly` ברישום. שורת מטוס היא **רשומת DB עם מפתח משלה** (`strip_id, idx`), ולא איבר במערך שנשמר בכתיבה אחת לשדה הפ"מ כמו `strips.targets`. העריכה במסלולים הייעודיים |
| **חימושים ומערכות** | משוטחים לטקסט אחד בתא ("פצצה x2", "מכ\"ם (לא שמיש)") - תא בטבלה אינו פורס טבלה שלישית |
