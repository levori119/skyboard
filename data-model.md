...
| `mission_desk_view_tables` | JSONB DEFAULT `'{}'` | שירות **"טבלאות מתצוגה"**, פר-עמדה: `{ "<view_tables_service_id>": { tables[], airfield_id } }`. `tables` מתוך `elements`/`trips`/`drivers`/`quantities`/`messages`/`container` (`MD_VIEW_TABLES`). `airfield_id` - לנסיעות ולנהגים, כי לעמדת דסק אין שדה משלה. טבלה שנבחרה בכמה משבצות מוצגת רק בראשונה בפריסה (`mdViewTableOwners`) |
