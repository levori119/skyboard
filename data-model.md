...
| **`restriction_alt_min`** | INTEGER | **הגבול התחתון של טווח הגבהים שההגבלה חלה עליו, ב**רום טיסה** (כמו `zone_altitude_ranges`)** |
| **`restriction_alt_max`** | INTEGER | **הגבול העליון. שני הגבולות NULL = ההגבלה חלה על **כל** הגבהים (סגירה גורפת)** |
...
>
> | מצב | שיוך פ"מ | התראה | `activity_log` |
> |---|---|---|---|
> | `closed` | **נחסם** | "אזור סגור - השיוך נדחה" | `zone_closed_blocked` |
> | `closed`, והאזור נסגר **מתחת** לפ"מ שכבר בו | נשאר | "אויש אזור סגור" | `zone_closed_manned` |
> | `restricted` | מותר | "אויש אזור מוגבל" | `zone_restricted_manned` |
>
...
>
> ⚠️ **המצב והטווח נכתבים בנפרד, והשרת הוא בעל המצב.** `PATCH /operational` עם
> `restriction` בלבד שומר את הטווח; עם טווח בלבד שומר את המצב (ואזור פתוח הופך
> ל-`restriction_if_open`, ברירת מחדל `restricted`); עם `restriction: ''` פותח
> ומנקה את הטווח. הלקוח **אינו** מצרף את המצב הנוכחי לכתיבת טווח - אחרת state
> ישן בו היה מוריד אזור סגור ל"מוגבל", וזה קרה בפועל (ראה REFACTOR_LOG).
>
