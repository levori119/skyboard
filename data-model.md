...
> (טהורה, 34 בדיקות): הבלוק שהפ"מ הוקצה לו גובר על הגובה הרשום בפ"מ, וכשאין
> לא בלוק ולא גובה - **ההגבלה חלה** (ברירת המחדל הבטוחה: התראה מיותרת היא רעש,
> התראה שלא נשמעה היא פ"מ באזור סגור). שיוך פ"מ לאזור כזה מוציא התראה
> "אויש אזור סגור/מוגבל" ורושם `zone_closed_manned`/`zone_restricted_manned`
> ב-`activity_log`.
>
> **גזירת ההגבלה נמשכת בפולינג** מ-`GET /api/map-zones/operational?map_id=` (כל
