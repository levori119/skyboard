...
**מייצא:** `setDockPreset`, `setDockEnabled`, `isDockEnabled`, `registerDockable`, `registerDockZone`, `setDockSlotEl`, `getDockSlotEl`, `dockLoad`, `dockPut`, `dockRemove`, `dockSetWidth`, `dockPlace`, `dockInsertIndex`, `dockHitTest`, `beginDockDrag`, `isDocked`, `DOCKED_ROOT_STYLE`, `DOCK_MIN_WIDTH`/`DOCK_MAX_WIDTH`/`DOCK_DEFAULT_WIDTH`.
...
**תפקיד:** העמודה עצמה — בין הפ"מים (order 4) לעזרים (order 6). גובה הקונטיינר מתחלק **שווה בשווה** בין המשבצות, וכל חלון מוקטן/מוגדל כיחידה אחת (`FitScaleBox mode="fill"`) כדי למלא את שלו. סידור מחדש בגרירת כותרת המשבצת, שחרור בגרירה החוצה או ב-↗, ורוחב נגרר בספליטר. **אין כאן `windowFrame`** וזה מכוון: זו עמודת פריסה ולא חלון צף, וקוד הצבע ממשיך לחיות על החלונות שבתוך המשבצות.
