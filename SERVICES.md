...
בסיס / פיינל = `set-leg` (הסטייה המותרת מהצלע ומהגובה - `leg_tolerance_nm` / `alt_tolerance_ft` של ההקפה, וכיוון טיסה עד 45° מהצלע; בסיס בלי השהיה); אחרי פיינל, מהירות < 60 קשר או נעלם עד מייל מקטע המסלול, 30 שניות
