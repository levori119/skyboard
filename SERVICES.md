...
**מצבי שורה:** `pending` → `synced` (נדחף) · `superseded` (המרכז עודכן אחרי, וגרסתו אומצה כאן) · `conflict` (לא ניתן היה להכריע) · `dropped` (הבקר סגר ידנית).
...
**תפקיד:** מחיל פעולת נטו במרכז, ו**מכריע לבדו** מי מנצח כששני הצדדים נגעו באותה שורה: `rev` תואם ל-`baseRev` → הוחל; שונה → **האחרון מנצח** לפי השוואת זמנים (`decideByTime`), עם תיקון הפרש שעונים (`skewMs`) שהעמדה שולחת בדחיפה. מחיקה במרכז מנצחת בלי להשוות - שחזור היה מחזיר מטוס רפאים למפה. רק כשאין חותמת זמן מוחזרת סתירה לאדם. `force` (היפוך ידני של הבקר) מדלג על ההשוואה. כל פעולה ב-`SAVEPOINT` משלה, כדי ששורה אחת שנכשלה לא תפיל דחיפה של משמרת שלמה.
**⚠️ הסכמה נקבעת בשרת** מהקשר הסביבה של הבקשה ולא מ-`table_schema` שהעמדה שלחה — אחרת עמדה בתרגול הייתה כותבת לסביבה האמיתית.
**למה לא "הבקר יכריע":** בפועל פעילה עמדה אחת מכל סוג, ולכן פ"מ שנגרר בנתק כמעט לעולם אינו נגוע גם במרכז. מסך הכרעה היה עוצר את הבקר אחרי כל נתק כדי לאשר את מה שממילא נכון.
**מייצא:** `applyOp`, `applyOps`, `decideByTime`, `POLICY`, `RESULT`, `REASON`.
...
### `server/routes/maps.js` — 35 routes
...
**שיכפול מפה (`POST /api/maps/:id/duplicate`):** **עותק נקי** — התמונה, שמונת ערכי העיגון ובסיס האב, וזהו. אזורי המפה, בלוקי הגובה שלהם, נקודות ההעברה ותת-המפות **אינם** מועתקים, וגם הקשר לאב (`parent_map_id`/`parent_rect`) נשמט: תת-מפה משוכפלת היא מפה עצמאית ולא סקטור נוסף שמסתנכרן חזרה מהאב. שם ברירת המחדל `"<שם> (העתק)"` (ומעליו `(העתק 2)`...), נקטע ל-100 התווים של העמודה; שם מפורש שכבר תפוס מוחזר 409. מאומת ב-`server/routes/maps.duplicate.test.js` מול Postgres אמיתי (PGlite) וב-e2e `admin-station-preview.spec.ts`.
...
**פ"מ שמוחזק בכמה עמדות:** `src/utils/stripTransferView.ts` (`projectStripsForStation`) - עמדה שמחזיקה פ"מ ו**אינה צד** בהעברה שעמדה אחרת שלחה ממשיכה לראות אותו **רגיל לגמרי**. ההטלה נעשית בכניסת הנתונים ב-`loadData` (ולא ב-~20 מסנני התצוגה), כי `strips.status` ו-`workstation_preset_id` גלובליים בזמן שההחזקה פר-עמדה. `/api/transfers/pending-all` מחזיר `from_preset_id`/`to_preset_id` בשביל ההכרעה הזו.
...
**`/candidates` מחזירה גם את מצב העמדה:** `active` ו-`merged_into_name`. `active` נמדד ב**טריות הדופק** (`station_sessions.last_seen` מול חלון של 240 שניות) ולא בקיום מקטע פתוח - מקטע נסגר רק ביציאה מפורשת, ובפרודקשן נמצאו מקטעים פתוחים בני 11 יום. `merged_into_name` הוא העמדה שמכסה אותה ב-`position_merges` פעיל. שני ה-JOIN בטוחים מריבוי שורות - לשניהם אינדקס ייחודי חלקי.
...
### `src/utils/elementStatus.ts`
**תפקיד:** **מקור אמת יחיד למצב אלמנט בשדה.** לאלמנט שני מצבים נפרדים: **כשירות** (`status` - שמיש/לא שמיש) ו**סטטוס תפעולי** (`display_state` - פתוח/סגור/מנצנץ, מוגבל ל-`allowed_statuses` של הסוג). שניהם נקבעים ב-POPUP שעל המפה, **והוא הקובע**. הפאנל הצדדי החזיק רשימת כשירויות משלו (`תקין`/`חלקי`/`לא תקין`/`תקול`) שהפופאפ לא ידע עליה, ולכן לחיצה בפאנל הכניסה אלמנט לערך שאי אפשר להגיע אליו - ולא לצאת ממנו - מהמפה (מחסום שנתקע על "חלקי"). מיפוי התווית העברית למפתח שנשמר ישב אף הוא משוכפל בתוך הפופאפ. ערך ישן **מוצג** אך אינו בר-בחירה, ו-`nextServiceability` חולץ ממנו בלחיצה אחת. מכוסה בדיקות (`elementStatus.test.ts`, 15). **מייצא:** `SERVICEABLE`, `UNSERVICEABLE`, `ELEMENT_SERVICEABILITY`, `serviceabilityStyle`, `nextServiceability`, `ALLOWED_STATUS_TO_DISPLAY_STATE`, `displayStateOptions`.

### `src/utils/elementTable.ts`
**תפקיד:** סינון ומיון לטבלת האלמנטים, בנפרד מה-DOM כדי שיהיו בדיקות אמיתיות. הבוררים **נגזרים מהנתונים** ולא מרשימה קבועה - כך גם ערך כשירות ישן שיושב בפועל ניתן לסינון במקום להיות בלתי נגיש. הפילטרים מצטברים (AND), ההשוואה **מדויקת ולא הכלה** (אחרת `שמיש` היה תופס גם `לא שמיש`), והמיון עברי עם ערך חסר שיורד תמיד לסוף. מכוסה בדיקות (`elementTable.test.ts`, 17). **מייצא:** `EMPTY_ELEMENT_FILTERS`, `elementFilterOptions`, `filterElements`, `sortElements`, `ElementRow`, `ElementFilters`.

### `src/components/ground/ElementsTableWindow.tsx`
**תפקיד:** **טבלת האלמנטים** - חלון צף ובר-עגינה שנפתח מתפריט "תצוגה" בעמדות המגדל. הפאנל הצדדי עונה על "מה קורה עם האלמנט הזה"; הטבלה עונה על השאלה ההפוכה - "מה **לא** שמיש עכשיו בכל הבסיס", "מה מהבהב", "מה מוסתר מהמפה" - ולכן היא חוצה שדות תעופה (כל שדות בסיס האב) ומסננת על כל עמודה. כשירות וסטטוס תפעולי נערכים מתוכה דרך `utils/elementStatus`, כדי שלא תיווצר בה שפה שלישית. `windowFrame('view')` (צפייה ותפעול).

### `server/utils/linkedRunways.js`
**תפקיד:** **התאמת קצוות בין שני מסלולים שמייצגים את אותו אספלט.** `matchEndName` מתאים **לפי המספר** ולא לפי המיקום, כי שדה אחד יכול להגדיר `heading_a='18'` והשני `heading_a='36'` (קיים בפועל ב-DB) - התאמה לפי מיקום הייתה סוגרת את הקצה ההפוך; אות הצד (L/R/C) אינה חלק מההשוואה. `matchEndSlot` עושה את אותו הדבר ל-NOTAM של קיצור, שנשמר לפי מיקום ('a'/'b'). מכוסה בדיקות (`linkedRunways.test.js`, 11). **מייצא:** `matchEndName`, `matchEndSlot`.

### `server/utils/runwayState.js`
**תפקיד:** **מצב המסלול נפתר בזמן קריאה, לא מועתק בזמן כתיבה.** מסלול מקושר הוא מסלול פיזי אחד, ולכן סגירה/קיצור, GRF, תאורות והמסלולים שבשימוש שייכים לו - לא להגדרה של שדה מסוים. המימוש הראשון **העתיק** את המצב לצדדים המקושרים ברגע הכתיבה, ונשבר בדיוק במקום שבו הפקח נתקל בו: **מידע שכבר היה שם לפני שהקישור נוצר לא זז**, וקישור חדש לקבוצה קיימת נשאר ריק "עד שמאפסים ומזינים מחדש". כאן אין עותקים: השורה נשמרת היכן שנכתבה, וכל קריאה מרכיבה את מצב הקבוצה (`RUNWAY_GROUP_SQL`) וממפה אותו למסלול ולשמות הקצוות של השדה ששואל - `runway_id` מקומי (הלקוח לא משתנה), `id` מקורי (עריכה ומחיקה חלות על שני הצדדים), ו-`source_*`/`is_linked` כדי שהפקח יראה **מי** סגר את המסלול שלו. הכרעה: NOTAM = איחוד · GRF = הדיווח האחרון לכל קצה · תאורות = העדכון האחרון · מסלולים בשימוש = כיוון אחד למסלול, האחרון גובר. מכוסה בדיקות (`runwayState.test.js`, 16 + שומר `runwayStateGuard.test.js` שנכשל על `SELECT ... FROM` ישיר בקובץ ראוט). **מייצא:** `RUNWAY_GROUP_SQL`, `runwayGroupPairs`, `mergeNotams`, `mergeGrf`, `mergeLighting`, `mergeEndUse`, `resolveNotams`, `resolveGrf`, `resolveLighting`, `resolveEndUse`, `airfieldOfRunway`, `LINKED_ROUTE_RUNWAY_SQL`, `mergeRouteNotams`, `resolveLinkedRouteNotams`.

> **מסלול קרקעי שאינו מסלול המראה.** `RUNWAY_GROUP_SQL` מגשר מסלול המראה למסלול המראה: שני הצדדים חייבים להיות מוגדרים ביישות "מסלולים" ולהחזיק מסלול ראי. בשדה **קרקעי** אותו אספלט משורטט לרוב כ**מסלול רגיל** על מפת הקרקע, ואז הקישור נשמר בניהול ולא קורה כלום - הפקח בקרקע ראה מסלול פתוח בזמן שהוא סגור באוויר. `LINKED_ROUTE_RUNWAY_SQL` + `mergeRouteNotams` סוגרים את הפער: מסלול בלי `source_runway_id` מקבל את ה-NOTAMים של מסלול ההמראה המקושר אליו, ממופים ל-`route_id` המקומי. `shorten_end` **נמחק** בדרך - הוא מיקום על מסלול המראה, ולמסלול קרקעי אין קצוות למפות אליהם. מוגש ב-`GET /api/route-notams` ומצויר ב-GroundView כשכבת סגירה על קו המסלול.
...
### `server/utils/roadGraph.js`
**תפקיד:** **גרף הכבישים של תכנון נתיב הנסיעה** (`planRoute` ב-`routes/driver.js`). המודל הקודם חיבר כל שני קודקודים של נתיבים שונים עד 80 מ' זה מזה, והמוצא/יעד בקו ישר לעד 8 קודקודים ברדיוס 300 מ' - והנתיב **חתך באלכסון** בין נקודות. כאן: (1) המוצא, התחנות והיעד מתחברים **בניצב** לנתיב הקרוב (רגל הניצב, גם באמצע קטע; נתיבים שקרובים כמוהו עד 5 מ' - גם הם); (2) הקשתות הן רק קטעי הנתיבים; (3) מעבר בין נתיבים רק ב**חיתוך הגאומטרי** שלהם, וקצה נתיב שנגמר עד 50 מ' מנתיב אחר מתחבר אליו בניצב (צומת T); (4) **כיוון נסיעה** (`base_routes.direction`: `both` / `forward` = בסדר הציור / `backward`) - נתיב חד-כיווני נותן קשתות רק בכיוונו. מזהי הצמתים שומרים על הקידומת `r<routeId>_`. מכוסה בדיקות (`roadGraph.test.js`, 14 + `routePlan.test.js`). **מייצא:** `buildRoadGraph`, `astarPath`, `shortestPath`, `haversineM`, `normalizeDirection`, `ENDPOINT_SNAP_M`, `ATTACH_TIE_M`.

### `server/utils/zoneAltInherit.js`
**תפקיד:** **הורשת בלוקי גבהים מאזור-אב לאזורי תת-המפות.** מפת סקטור נחתכת לתת-מפות (אזורי קרב → מרחבי 305 → אזורי ים/יבשה) והילד מצביע על האב ב-`map_zones.parent_zone_id`, אבל הסנכרון שהיה קיים העביר **שם, צבע ופוליגון בלבד** - אזור בתת-מפה הופיע בלי אף בלוק ולא היה אפשר להציב בו פ"מ לפי גובה. שני מצבים בכוונה: `mirror` (עריכת בלוק על האב - מראה מלאה לכל הצאצאים בכל עומק, כולל מחיקה) ו-`fill` (סנכרון שם/צבע/פוליגון ויצירת אזור-ילד - מילוי בלבד, ילד עם בלוקים משלו לא נדרס, כי שינוי שם לא אמור למחוק חלוקת גבהים שנקבעה בתת-מפה). ההתאמה **לפי שם הבלוק** והשורה מתעדכנת במקומה: `strip_zone_assignments.altitude_range_id` הוא `ON DELETE SET NULL`, ומחיקה+הכנסה הייתה מפילה כל פ"מ מוצב מהבלוק שלו בכל עדכון. עובר על צאצאים ב-BFS עם הגנה מפני מעגל, ואינו זורק - כשל מוחזר כסטטוס כדי לא להפיל את הבקשה שקראה לו. מכוסה בדיקות (`zoneAltInherit.test.js`, 16). **מייצא:** `planAltRangeMirror`, `inheritAltRanges`.
...
**`GET /driver/logic.js`:** מגיש את [`shared/driverLogic.js`](shared/driverLogic.js) - הלוגיקה הטהורה של אפליקציית הנהג (`greetingFor`, `nearestBase`, `groupDriverTrips`, `sortHistory`, `buildTripRequest`, `requestFormFrom`, `nextDeparture`, `buildTemplate`, ולקו הנתיב: `splitRouteLegs` - לג לכל קטע בין תחנות, `legColor`/`LEG_COLORS`, `routeArrows` - חצי כיוון לאורך הקו), מודול ES שהדף טוען ונבדק ב-vitest בלי DOM. `public/driver.html`: **כל כניסה לדף מתחילה ב-LOGIN** (האסימון בזיכרון בלבד, לא ב-sessionStorage), שער כניסה מונפש, ברכה, דף בית עם טאבים (מאושרות / ממתינות / היסטוריה / **תבניות**) בשורות מצומצמות (בכל כרטיס: ✏️ שינוי, ⧉ שכפול, 📋 שמירה כתבנית), טופס בקשה חדשה שמשמש גם לתבנית (`requestFormFrom`, `nextDeparture`, `buildTemplate`), ו"כניסה לשער עכשיו" לזרימה הקיימת.
**תפקיד:** מערכת נהג/רכב — בקשות רכב, GPS, הודעות, מסלולי בסיס, חישוב נתיב (A*), אפליקציית נהג (`/driver`).
**בקשות כניסה לפי זהות הנהג:** בקשה מאפליקציית DRIVER נחתמת ב-`requester_national_id` מהאסימון (לא מהגוף). אסימון נהג רואה ב-`GET /api/vehicle-requests` רק את הבקשות שלו, ומעדכן/מוחק רק אותן (בקשה של אחר = 404). בעדכון הנהג רשאי רק `destination`/`supply_type`/`notes` ו-`status` מסוג `cancelled`/`arrived` - אישור, מסלול וקישור למרשם הם הכרעת המגדל. עמדה רואה ומכריעה בכל התור. אסימון נהג בלי ת"ז = 403 `driver_identity_required`.
...
**Endpoints:** `GET/POST /api/permit-params`, `PUT/DELETE /api/permit-params/:id`, `GET /api/entry-permits` (`?airfield_id=`), `GET/PUT/DELETE /api/entry-permits/:id`, `POST /api/entry-permits`, `POST /api/entry-permits/:id/vehicles`, `PUT/DELETE /api/entry-permit-vehicles/:id`, `GET/POST /api/entry-permits/:id/trips`, `GET /api/trips` (`?airfield_id=&scope=upcoming`), `POST /api/trips`, `POST /api/trips/duplicate` (שכפול בודד וקבוצתי), `PUT/DELETE /api/entry-permit-trips/:id`, `POST /api/entry-permit-trips/:id/alerted`, `POST /api/entry-permit-trips/:id/change/:decision` (approve/reject), `GET /api/driver-trips` (לפי **הת"ז שבאסימון הנהג** בלבד - נהג מהמרשם דרך `driver_id`, נהג מזדמן דרך `driver_national_id`; פרמטרים מהלקוח מתעלמים; בלי ת"ז = 403), `POST /api/driver-trips/:id/ack`, `POST /api/driver-trips/:id/change` (רק לנסיעה של הנהג עצמו; של אחר = 404), `GET /api/driver-trips?view=history` (רק נסיעות שבוצעו, האחרונה ראשונה), **`POST /api/driver-trips`** - בקשת נסיעה מהנהג: שדות מותרים בלבד (סוג נסיעה/רכב, שם רכב, מועד, מוצא/יעד/תחנות, מבקש, טלפון, נלווים, הערה); הזהות מהאסימון, סטטוס תמיד `pending`, `driver_requested_at=NOW()`; שדה של בסיס לא מורשה = 403 `base_not_permitted`, נקודה/סוג שאינם של השדה = 400 `invalid_reference`, בלי מועד/מוצא/יעד = 400 `missing_fields`. **`POST /api/driver-trips/:id/start`** - "הפעל נסיעה": נסיעה מאושרת בלבד (אחרת 409 `not_approved`), בחלון ±30 דק' מהמועד (`startWindowState` מ-`shared/driverLogic.js`, אותו כלל שהאפליקציה מציגה; מחוץ לחלון 409 `outside_start_window` + `window: early|late|none`). **עדכון מהנהג** (`/change`) מחזיר את הנסיעה ל-`pending` ושומר `pending_change_prev_status`; `change/:decision` מחזיר את הסטטוס (אישור שמשנה תחנות מוחק את הנתיב ומשאיר `pending`). **תבניות נסיעה:** `GET/POST /api/driver-trips/templates`, `PUT/DELETE /api/driver-trips/templates/:id` - רק של הנהג (ת"ז מהאסימון) ובבסיסים המורשים; של אחר = 404; גוף מנוקה ב-`buildTemplate` המשותף; נקודה/סוג של שדה אחר = 400 `invalid_reference`; תקרה 50 = 409 `template_limit`. **`POST /api/route-plan`** מקבל `alternatives` (עד 3): חלופות שונות פיזית לנתיב הקצר, כל אחת עם `waypoints`/`routeSegments`/`segmentPath`/`totalDistM` משלה (`planRoute` ב-`routes/driver.js`). **`GET /api/driver-trip-options/:baseId`** (`driverBaseGuard`) - שדות הבסיס, הנקודות שלהם וסוגי הרכב והנסיעה הפעילים לטופס הנהג. `TRIP_SELECT` מחזיר גם `base_id`/`base_name`. (ראה `entry_permit_*` ב-data-model.md)

**מעקב נסיעה חי** ([TRIP_LIVE_TRACKING_SPEC.md](TRIP_LIVE_TRACKING_SPEC.md)): `GET /api/driver-trips/:id/live` (נתוני המפה לנהג בקריאה אחת - עוגן, נתיב שאושר, אלמנטים על הדרך עם מצב חסימה, מסלולי טיסה והסעה) · `POST /api/driver-trips/:id/gps` (קריאה; השרת מחשב סטייה ואלמנט חוסם) · `POST /api/driver-trips/:id/end` · `GET /api/trips/live?airfield_id=` (למגדל: נסיעות פעילות עם מיקום, סטייה, אות אבד ואלמנט חוסם). כל נתיבי הנהג עוברים `driverScope`+`ownsTrip`. **מייצא גם:** `GPS_HISTORY_LIMIT`.
...
**כניסת נהג - `POST /api/auth/driver` `{nationalId, password}`:** שואל את המיראז' על אפליקציה **נפרדת** `MIRAGE_DRIVER_APP_NAME` (ברירת מחדל `SKY-KING DRIVER`), עם **הת"ז** (ספרות בלבד) כשם המשתמש. הרשאה ל-SKY-KING אינה הופכת בקר לנהג. ההרשאה נושאת **בסיסים** (`bases: [{id, name}]`, נבחרים במסך הניהול של המיראז' מרשימת הבסיסים של SKY-KING); נהג בלי בסיס → 403 `no_base_permitted`. מצליח → `{token, expiresInMs, driver:{name, nationalId, bases}}`, והאסימון `{role:'driver', nationalId, name, baseIds}` תקף 8 שעות. **לפי הבסיסים:** `GET /api/aviation-bases` מחזיר לנהג רק אותם, `/api/{airfields,airfield-points,airfield-elements}/by-base/:baseId` מחזירים 403 על בסיס אחר (`driverBaseGuard`), הנסיעות מסוננות לפי `airfields.base_id` של שדה הנסיעה, ובקשת כניסה לבסיס אחר → 403 `base_not_permitted`. הת"ז שבאסימון (מנורמלת ל-9 ספרות, [`server/auth/driverIdentity.js`](server/auth/driverIdentity.js)) היא הזהות היחידה שממנה נגזרים הנסיעות ובקשות הכניסה של הנהג. פירוש תשובת המיראז' משותף לשתי הכניסות (`sendMirageDenial`): 400 `missing_national_id`/`missing_password`, 401 `bad_credentials`/`password_not_set`, 403 `not_authorized`, 429 `rate_limited`, 502 `mirage_unavailable`, 503 `driver_offline` (עמדה מנותקת). קוד הגישה המשותף `DRIVER_ACCESS_CODE` **בוטל**.
...
**הנהגים המורשים לבסיס (ניהול נהגים):** `GET /api/auth/mirage-drivers?airfield_id=N` → `{baseId, drivers:[{nationalId, firstName, lastName, fullName}]}` - משתמשי מיראז' עם הרשאת `SKY-KING DRIVER` (תפקיד + הבסיס של השדה ברשימת `bases`), ת"ז מנורמלת ל-9 ספרות, ממוין לפי שם (`driversForBase`). **כן** מוחזרת ת"ז - היא המזהה שהאישור והנסיעות נרשמים אליו; נתיב עמדה בלבד (נהג = 403). שדה בלי בסיס = 409 `airfield_without_base`; מיראז' לא זמין = 502.
...
**מגמה אנכית:** ה-snapshot נושא גם `trends` (מזהה מטוס → טיפוס/נמיכה/מפולס),
שמחושב כאן ב-`setSnapshot` ולא ברכיבים - כך שהמבט מלמעלה והסצנה התלת מימדית
לעולם לא יראו חץ שונה לאותו מטוס. ראה `trend.ts`.
...
**`trackLabelLines(t, fields?)`** - שתי שורות התווית, **מקור אמת אחד** לקנבס
השטוח ולסצנה התלת מימדית. `fields` הוא בחירת הפקח (או"ק / גובה / מהירות); כשהאו"ק
כבוי הנתונים עולים לשורה הראשונה ולא נשארת שורה ריקה מרחפת מעל הסמל.
**מייצא:** `MAX_TRACKS` (300), `deadReckon`, `place`, `visible`, `applyFilters`,
`capNearest`, `prepare`, `ageSec`, `STALE_AFTER_SEC`, `trackLabelLines`, `trackSymbolPoints`.
...
**חץ המגמה** מצויר בנתיב ולא כטקסט, ולכן אינו עובר דרך מטמון התוויות: המגמה
משתנה בלי שהתווית משתנה, והוא נשאר גלוי גם כשהתוויות כבויות. **בחירת השדות נכנסת
למפתח המטמון** - בלעדיה כיבוי "מהירות" לא היה נראה עד ששאר התווית משתנה מסיבה אחרת.
...
**`fields`** - אילו נתונים מוצגים בתווית (או"ק / גובה / מהירות). `labels` הוא
המתג הראשי ואלה הפריטים: עמדה עמוסה רוצה או"ק בלבד, עמדת גישה רוצה גובה ומהירות.
שדה חסר = דלוק, כדי שהעדפה שנשמרה לפני התוספת לא תמחק לפקח את התוויות.
**מייצא:** `DEFAULT_PREFS`, `DEFAULT_LABEL_FIELDS`, `mergePrefs`, `loadPrefs`, `savePrefs`, `AirPicturePrefs`, `LabelFields`.
...
בהירות, תוויות, **בחירת הנתונים בתווית** (או"ק / גובה / מהירות - שלושה מתגים
שמושבתים כשהתוויות כבויות), סינון סיווג/גובה/אחראיות, וחיווי מצב חיבור וגיל התמונה.
צבעי המשטח נגזרים מהתמה; צבעי הסיווג והמצב הם **צבעי סטטוס** ולכן קבועים.

### `src/airPicture/trend.ts`
**תפקיד:** **מגמה אנכית - נגזרת, לא מדווחת.** חוזה המאגר מוסר גובה רגעי בלבד ואין
בו קצב טיפוס, ולכן המגמה מחושבת מהשוואת דגימות. שתי הכרעות: **סף ולא נגזרת רגעית**
(150 רגל מול נקודת ייחוס - הגובה מגיע מעוגל ומטוס מפולס "נושם" עשרות רגלים בין
דגימות), ו**שמירת המגמה 20 שניות** בין חציות סף, אחרת החץ מהבהב כי טיפוס אינו עולה
במדרגות אחידות. מטוס שירד מהאוויר נושר מהמפה - אחרת היא גדלה בלי גבול לאורך משמרת.
הגאומטריה של החץ משותפת לשני המבטים (הקנבס בנתיב, ה-SVG ב-`<polygon>`), בדיוק כמו
`trackSymbolPoints`. מכוסה בדיקות (`trend.test.ts`, 11).
**מייצא:** `VertTrend`, `TrendRef`, `TREND_MIN_FT`, `TREND_HOLD_MS`, `TREND_COLOR`, `TREND_OFFSET`, `nextTrendRef`, `updateTrendRefs`, `trendsOf`, `trendArrowPoints`.
...
מעצמו סטטוס שנדרס.
**מצטרף ל-poller בזכות עצמו:** כשהתמונה כבויה `AirPictureLayer` אינו מרונדר ואיש
אינו דוגם את המאגר, ואז "התראות גם בלי תמונה" לא היה מקבל נתונים כלל. ה-poller
סופר מנויים, ולכן כשהשכבה כן רצה זו אינה תעבורה כפולה.
**`offenders`** - הרכיבים האוויריים שמאחורי ההתראות, אחרי השלכה למפה. מתעדכנים
בחתימה **נפרדת** מזו של ההתראות: המיקום זז גם כשרשימת ההתראות זהה. זה מה שמצויר
כשהתמונה כבויה. **`dismissAll`** משתיק את כל ההתראות החיות - לפי מפתח ולא כדגל
...
בסיס / פיינל = `set-leg` (הסטייה המותרת מהצלע ומהגובה - `leg_tolerance_nm` ומעל/מתחת לגובה `alt_tol_above_ft` / `alt_tol_below_ft` של ההקפה (ריק: 0.5 מייל, 1500 מעל, 500 מתחת), וכיוון טיסה עד 45° מהצלע; בסיס בלי השהיה); אחרי פיינל, מהירות < 60 קשר או נעלם עד מייל מקטע המסלול, 30 שניות
...
### `src/utils/optimisticOverlay.ts`
**תפקיד:** עדכון מיידי ש**נשאר** בעמדה. מחזיק את העדכונים הפתוחים (נלחצו, טרם הגיעה
עליהם תמונה מהשרת) ומחיל אותם מחדש על כל תמונה מהפולינג, עד שהשרת מאשר ומגיעה תמונה
שנשלפה **אחרי** האישור. `seq` מבדיל בין לחיצות, TTL מונע עדכון תקוע.
**מייצא:** `createOptimisticOverlay`. בשימוש: ירוקים וצלע בטבלת "בהקפה".
...
## מז"א על המפה — `src/weather/`

**תפקיד:** פריסת שכבת מזג אוויר מ-**Windy** מעל מפת השדה, בעמדת הבקר ובעמדת
המגדל — **אותו רכיב בדיוק**. נפתח מ"תצוגה → הצג מז"א".

**שתי צורות תצוגה, מנוע אחד:** על מפה **מעוגנת** זו שכבה שמתלכדת עם העוגן;
בתצוגה בלי מפה (טבלה, בלוקים, דסק משימה, מפה בלי נ"צ) זהו חלון צף אינטראקטיבי.
תפריט השכבות זהה בשתיהן.

**למה iframe ולא אריחי מפה:** ה-CSP של העמדה מתיר מסגרות חיצוניות
(`frame-src 'self' https: http:`) אבל **חוסם תמונות חיצוניות**
(`img-src 'self' data: blob:`). שכבת אריחים הייתה דורשת לפתוח את `img-src`
ואת `connect-src` לשרתי Windy — הרחבת משטח התקיפה עבור תצוגה. ההטמעה עובדת
כמו שהיא, **בלי מפתח API ובלי תלות npm חדשה** (SK-30).

### `src/weather/windy.ts`
**תפקיד:** קטלוג השכבות ובניית כתובת ההטמעה. 15 שכבות: 10 מהתפריט המהיר של
Windy (מכ"ם, לוויין, רוח, גשם ורעם, טמפרטורה, הוריקן, עננים, גלים, הצטברות
גשם, סופות רעמים) + 5 תעופתיות (משבי רוח, ראות, בסיס עננים, איזותרם 0°, CAPE).
**מזהי השכבות אומתו מול ה-bundle של embed.windy.com** ולא נוחשו. יחידות `kt`
ו-`°C` — אלו שעל הסדק ובכריזת ה-ATIS.
**מייצא:** `WEATHER_LAYERS`, `windyEmbedUrl`, `weatherLayer`, `isWindyOverlay`,
`WINDY_EMBED_BASE` (ניתן לדריסה ב-`VITE_WINDY_EMBED_URL` למראה פנימי ברשת מבודדת).

### `src/weather/fit.ts`
**תפקיד:** התאמת מפת Windy (Leaflet, מרקטור, זום שלם) לעיגון של מפת השדה
(מיפוי לינארי לפי שני עוגנים). בוחר את הזום השלם הקרוב לקנה המידה ומתקן את
השארית ב-CSS `scale`; ציר הרוחב מקבל תיקון `1/cos(φ)` של מרקטור. עיוות קיצוני
נחסם בפי 3, שלא ימרח את המסך על עוגן שגוי.
**⚠️ `MAX_ZOOM = 11` — תקרת Windy, נמדדה.** ההטמעה מתעלמת מכל זום מעל 11
ומרנדרת 11 במקומו (פריימים ב-11..15 יצאו זהים בפיקסל, בשתי נקודות הקצה).
בלי הקיבוע מפת שדה של ~8 ק"מ קיבלה מז"א שנפרס על ~35 ק"מ. הסקייל נגזר מהזום
ולכן משלים את ההפרש אוטומטית — **המחיר הוא חדות, לא דיוק**.
**אומת מול דפדפן אמיתי:** ההפרש תל-אביב↔ירושלים יצא ~410px מול 413px חזויים,
וקנה המידה בעמדת שדה מעוגנת נמדד מול סרגל ק"מ של השרטוט.
**מייצא:** `fitWindyToMap`, `anchorCenter`, `OVERSCAN`.

### `src/weather/prefs.ts`
**תפקיד:** העדפות בעמדה — **בסשן, לא ב-DB** (אותה תבנית של התמונ"א).
**מיזוג ברירת המחדל נמדד ולא הונח:** רקע המפה של Windy הוא **אפור בהיר**
(~`#8a8a8a`) ולא כמעט-שחור, ולכן `screen` מלבין סדק כהה עד שכיתוב נבלע —
ברירת המחדל בתמה כהה היא `normal`, ובתמה בהירה `multiply`. בהירות 0.45.
**מייצא:** `DEFAULT_PREFS`, `mergePrefs`, `loadPrefs`, `savePrefs`,
`defaultBlendFor`, `WeatherPrefs`, `WeatherBlend`.

### `src/weather/WeatherLayer.tsx`
**תפקיד:** השכבה המעוגנת. יושבת **בתוך** ה-transform של המפה, לפני התמונ"א
בסדר ה-DOM (מזג האוויר הוא רקע, המטוסים הם מה שמסתכלים עליו) — ולכן פאן וזום
של המפה מזיזים אותה **בלי לטעון את ה-iframe מחדש**. `pointerEvents:'none'`.
ה-iframe נבנה גדול פי 1.5 מאזור התצוגה וממורכז בו, וה-`overflow:hidden` חותך
את הטבעת העודפת — ואיתה את הלוגו, סרגל הזמן והמקרא של Windy.
**ארגז חול:** `sandbox="allow-scripts allow-same-origin"` — חוסם ניווט של
העמדה כולה. **נבדק בדפדפן אמיתי:** Windy עולה תחתיו זהה לחלוטין, בלי שגיאות.
**מייצא:** `WeatherLayer` (default), `WeatherStatus`, `WINDY_SANDBOX`.

### `src/weather/WeatherMenu.tsx` · `WeatherWindow.tsx`
**התפריט:** עמודת גלולות עם עיגול צבע לכל שכבה (CSS gradient ולא תמונה —
`img-src` חוסם מקור חיצוני). נגרר ב-`useDragPosition` (עט ואצבע), מסגרת
תורכיז לפי קוד הצבע. לחיצה על השכבה הפעילה מכבה.
**החלון:** נגרר + שינוי גודל ב-Pointer Events, עם אותו תפריט בדיוק בצד.
נפתח על מרכז המפה המעוגנת של העמדה, ובהיעדרה על מרכז הארץ.
...
**תפקיד:** **מנוע הסנכרון בלקוח** — הוא שמריץ את שני הכיוונים, כי הדפדפן הוא היחיד שמחזיק אסימון תקף גם למרכז וגם למאגר המקומי (דרך `/api/__remote` ו-`/api/__local`). סדר קשיח: **קודם דוחפים, רק אחר כך מושכים מראה**. סיבוב כל 10ש', מראה כל 45ש', ובנתק (אמיתי או מדומה) לא מנסה דבר. שולח `stationNow` בכל דחיפה - זה מה שמאפשר למרכז לתרגם את שעון העמדה לשעון שלו לפני ההכרעה.
**מייצא:** `startSyncClient`, `pushPending`, `pullMirror`, `resolveConflict`, `getSyncState`, `subscribeSync`, `type SyncConflict`.
...
**תפקיד:** **כיוון אחד למסלול בפאנל "מסלולים בשימוש".** שני קצוות של אותו מסלול פיזי הם כיוונים **מנוגדים** (המראה מ-15L מול נחיתה ל-33R = תנועות זו מול זו על אותו אספלט), אבל הפאנל החזיק שתי רשימות חופשיות ולכן איפשר לסמן את שניהם. כאן הכלל: כיוון אחד למסלול, **חוצה את שתי השורות**. לחיצה על הקצה הנגדי אינה נחסמת אלא **מחליפה כיוון** (זו הפעולה התכופה בשדה), ו-`endUseState` מחזיר `opposed` כדי שהכפתור יסומן כתום **לפני** הלחיצה. מכוסה בדיקות (`runwayEnds.test.ts`, 15). **מסלול סגור ב-NOTAM:** הסגירה חלה על ה**מסלול** ולא על קצה, ולכן `closedRunwayIds`/
`isRunwayClosed` הם המקור היחיד לשאלה "סגור?" - למפה השטוחה, לסצנה התלת מימדית
ולסינון ההקפות הפעילות. קודם הכלל היה משוכפל inline ב-GroundView, והתלת מימד פשוט
לא ידע על סגירה (REFACTOR_LOG #046).
**מייצא:** `oppositeEnd`, `runwayEndsInUse`, `setEndInUse`, `endUseState`, `orderedRunwayGroups`, `closedRunwayIds`, `isRunwayClosed`, `closedRunwayEnds`, `RunwayEnds`, `EndsInUse`, `UseRow`, `EndUseState`, `RunwayGroup`, `RunwayWithId`, `RunwayNotam`.
...
### `src/utils/mapDrawing.ts`
**תפקיד:** **הלוגיקה הטהורה של הציור על המפה** - משותפת לעמדת המפה ולעמדת השדה (רכיבי התצוגה ב-`components/map/MapDrawLayer.tsx`). כל נקודה נשמרת כ**שבר** (0..1) מגודל משטח הציור ולא בפיקסלים, ולכן הציור נשאר על אותו מקום במפה בשינוי גודל חלון, בזום/פאן של המפה, בסקייל גודל המסך (`--s`) ובין עמדות ברזולוציות שונות; ערכים ישנים שנשמרו בפיקסלים (> 1.5) ממשיכים לעבוד כפי שהם. המחק רחב פי 10 מהעט - בעובי העט מחיקה בעט איטית מדי לתפעול. מכוסה בדיקות (`mapDrawing.test.ts`, 12). **מייצא:** `DrawTool`, `PenStroke`, `MapShape`, `CanvasSize`, `DRAW_PALETTE`, `ERASER_WIDTH_FACTOR`, `isFrac`, `fracToPx`, `pxToFrac`, `strokeLineWidth`, `applyStrokeStyle`, `drawStrokeFrac`, `redrawStrokes`, `shapeFromDrag`, `shapeToPx`, `PolyTool`, `isPolyTool`, `POLY_MIN_POINTS`, `POLY_SNAP_SCREEN_PX`, `polyTapAction`, `polyShapeFromPoints`, `polyPointsToPx`. **פוליגון סגור/פתוח:** דוקרים נקודות שמתחברות בקו; הצורה נשמרת כ-`MapShape` עם `points` בשברים + מלבן תוחם ב-x/y/w/h (תאימות לקוד שמכיר רק מלבן). סיום בדקירה חוזרת על האחרונה, ובסגור גם על הראשונה. **סגנון קו** (`LineStyle`, `LINE_STYLES`, `dashArray`, `outlinePoints`, `crossMarks`, `crossSpacing`, `crossSize`): רציף / קווים / נקודות / קו-נקודה כתבנית `stroke-dasharray` ביחס לעובי הקו, ו**איקסים** כסימנים שנבנים מקו המתאר של הצורה (עיגול נדגם ל-64 נקודות) ומסתובבים עם כיוון הקו. צורה ישנה בלי `lineStyle` נשארת רציפה. **המחק מול צורה** (`isShapeHit`, `shapeAtPoint`, `eraseShapesAt`, `eraserRadius`): מגע בקו המתאר (ובצורה מלאה - גם בפנים) מוחק את הצורה **כולה**, כי צורה היא אובייקט ולא פיקסלים; בערבוביה נמחקת העליונה. **סמן המחק**: טבעת (`EraserCursorSvg`) ברדיוס `eraserRadius` - בדיוק מה שהמחק מוחק; היא מוצגת במרחב התוכן ולכן נעה ומתכווצת עם זום המפה, ומחליפה את סמן העכבר (`cursor:none`) כדי שלא יוצגו שני גדלים סותרים. **שיתוף עמדה**: `mergeRemoteShapes` ממזג דו-כיוונית - נוספו שם → מגיעות, נמחקו שם → נעלמות גם כאן, וצורה שטרם נשלחה לא נמחקת.
...
`RUNWAY_CLOSED_COLOR` הוא **צבע סטטוס** ולכן קבוע בכל תמה, ויושב כאן ולא בכל מבט
בנפרד - מסלול סגור נראה אותו דבר במפה השטוחה ובסצנה התלת מימדית.
**מייצא:** `RunwayGeo`, `RunwayAxis`, `ThresholdBar`, `Designator`, `AidLabel`, `RUNWAY_CLOSED_COLOR`, `DEFAULT_RUNWAY_WIDTH`, `MIN_RUNWAY_WIDTH`, `MAX_RUNWAY_WIDTH`, `derivedRunwayWidth`, `runwayAxis`, `runwayQuad`, `thresholdBars`, `centerlineDashes`, `designatorFontSize`, `designatorText`, `aidLabels`.
...
### `src/utils/windowForm.ts`
**תפקיד:** מידות החלון הצף וסגנונות הטופס שבתוכו - **גובה אחיד לכל פקד** (`input`, `select` ובמיוחד `input[type=date]` מקבלים מהדפדפן גבהים פנימיים שונים, ובלי קיבוע כל שורת טופס יוצאת מדורגת), `color-scheme` שמפעיל את בורר התאריכים של הדפדפן בצבעי התמה, ומידת "כמעט מסך מלא". פונקציה טהורה ולא קוד בתוך הרכיב: אלה ההכרעות שהמפעיל רואה בעיניים, ובלי jsdom אי אפשר להגיע לטופס דרך רינדור כדי לבדוק אותן. משותף לחלונות האחים "ניהול נהגים" ו"ניהול נסיעות" (`VehiclePermitsWindow` מייצא אותם הלאה בשמם ההיסטורי).
**מייצא:** `CTRL_H`, `WINDOW_SIZE`, `formStyles(C, themeMode)`.
...
**מייצא:** `useDockableWindow(id, title, opts)` (default), `DockableWindow` (רכיב render-prop לחלונות שנולדים ב-`map()`), `Dockable`, `DockableOptions`.

### `src/utils/signalSeverity.ts`
**תפקיד:** **חומרת הודעה** בלוח ההודעות — `normal` (ירוק) / `severe` (אדום) / `critical` (אדום מהבהב). מקור אמת יחיד לצבעים, משותף ללוח ההודעות בעמדה ולמאגר ההודעות הקבועות בהגדרת העמדה. הצבעים **קבועים בכל התמות** ככל צבע סטטוס. ערך לא מוכר (הודעה מלפני הפיצ'ר, לקוח ישן) נופל ל-`normal` ולא מגיע לתצוגה בלי צבע. ההבהוב מתבצע דרך `.sig-critical-blink` ב-`App.css`, שמחליף בין אדום בהיר לאדום כהה ולא ל-`opacity: 0` — כך הכיתוב נשאר קריא לאורך כל המחזור.
**מייצא:** `SignalSeverity`, `SIGNAL_SEVERITIES`, `normSeverity`, `severityPaint`, `severityDot`, `CRITICAL_BLINK_CLASS`.
...
`groupRecipientsByBase` הוא הקיבוץ ל**בורר הנמענים** של לוח ההודעות: הבסיס של השולח ראשון (`isMine` - הקבוצה היחידה שנפתחת כברירת מחדל), שאר הבסיסים לפי שם, "ללא בסיס אב" אחרון, ו**סדר הקלט נשמר בתוך הקבוצה** כדי שהקורא ישלוט במיון הפנימי (שכיחות לפני שם) בלי שהקיבוץ ידרוס אותו. לצדו `groupCheckState` (`none`/`some`/`all` - קבוצה ריקה היא `none` ולא `all`) ו-`toggleGroupIds`, שמסמן/מנקה את עמדות הבסיס בלי לגעת בנמענים מבסיסים אחרים.
**מייצא:** `presetStamp`, `groupPresetsByBase`, `shouldShowGroupHeaders`, `formatStationTime`, `baseKeyOf`, `allowedBaseKeys`, `isBaseAllowed`, `filterByAllowedBases`, `groupItemsByBase`, `groupRecipientsByBase`, `groupCheckState`, `toggleGroupIds`, `PresetLike`, `BaseLike`, `StationGroup`, `BaseScoped`, `BaseItemGroup`, `RecipientBaseGroup`, `GroupCheckState`.
...
### `src/utils/geo.ts`
**תפקיד:** המרות גיאו (פיקסל↔lat/lon) + פורמטי נ"צ - DMS לתצוגה, ו-**DDM** (`NDDMM.mmm EDDDMM.mmm`) לרשימת הנ"צ של אזור, שבה הבקר מקליד ומעתיק. הפירוק סובלני (אות בסוף, רווח באמצע, שבר חלקי, מפריד `/` או בלעדיו) כי הנ"צ מועתק ממסמך ומוקלד בעט. מכוסה בדיקות (`geo.test.ts`, 13). **מייצא:** `MapGeoAnchor`, `buildGeoAnchor`, `geoToImagePct`, `imagePctToGeo`, `fmtDms`, `fmtDdm`, `fmtCoordPair`, `parseDdm`, `parseCoordPair`. **הדבקת נ"צ לעיגון:** `parseAnyCoordPair` (עשרוני של Google Earth/Maps, DMS של Google Earth Pro או DDM) ו-`decimalToDmsFields` (לשדות N/E מעלות-דקות-שניות, עם נשיאה ב-60).
...
### `src/utils/joiningPoints.ts`
**תפקיד:** **נקודות הצטרפות (STAR)** - בניית טבלת בלוקי הגבהים. הגובה נשמר **ברגל** (4000) ומוצג **במאות** (040), כמו על הסדק. ההפרש בין בלוקים **אינו קבוע לאורך הטווח**: אפשר 1000 רגל בין 4000 ל-7000 ו-500 בין 7000 ל-10000, ולכן `buildBlocks` מחלקת לקטעים כשקטע שלא כוסה נופל להפרש ברירת המחדל, וגבולות הטווח תמיד נכללים. הפרש 0 או שלילי אינו יוצר לולאה אינסופית אלא מייצג את הקטע בגבולותיו (הגדרה שבורה נראית למשתמש במקום להקפיא את העמדה), ותקרת `MAX_BLOCKS` חוסמת טווח ענק עם הפרש זעיר. `conflictBlocks` מסמן בלוק עם שני פ"ממים ומעלה, ויורד מהאדום רק כש**כל** משתתפיו סומנו כמתואמים - התיאום הוא בין השניים ולא של אחד לבדו. `allAircraftInPattern` נבדק מול מספר המטוסים במבנה ולא מול השורות שהתקבלו, כדי ששורת מצב של מטוס שכבר אינו במבנה (אחרי פיצול) לא תזייף "כולם בהקפה". מכוסה בדיקות (`joiningPoints.test.ts`, 37).
**מייצא:** `JoiningPoint`, `AltStep`, `JoiningPointStripRow`, `JoiningAircraftRow`, `altToDisplay`, `displayToAlt`, `buildBlocks`, `findStepOverlaps`, `blockOf`, `nearestBlock`, `isAltInPoint`, `formationsInBlocks`, `conflictBlocks`, `allAircraftInPattern`.
...
`tableCellScroll` נותן לתא את גובה **שורה אחת**, תקרה של שלוש שורות וגלילה פנימית (ב-`em`, כי גודל הגופן בטבלה נשלט בעמדה) - בלעדיו הערה בת חמש שורות מתחה את כל שורת הפ"מ.
**מייצא:** `cellEditKeyAction`, `handleCellEditKeyDown`, `insertNewline`, `editableCellUnderline`, `defaultEditableCols`, `tableCellScroll`, `TABLE_CELL_MAX_LINES`, `CellEditKeyAction`. **שימוש:** `SectorDashboard` (מוד טבלה), `StripControl` (וריאנט טבלה).
...
**תפקיד:** **יומן ההכרעות בסנכרון** - ולא מסך שעוצר את הבקר. ההכרעה עצמה אוטומטית ("האחרון מנצח"), וכאן רואים **מה** הוכרע ולמה: שתי הגרסאות זו מול זו, ורק השדות שבאמת שונים (`rev`/`updated_at`/`created_at` מוסתרים, אחרת הם היו מסמנים "שונה" בכל שורה ומטביעים את ההבדל האמיתי). "החזר את הגרסה שלי" הופך הכרעה בדחיפה בכפייה. שורות שלא ניתן היה להכריע מסומנות "דורש הכרעה" ומקבלות שני כפתורים. נסגר מעצמו כשלא נשאר מה להציג. **מייצא:** `SyncConflictsModal` (default), `titleOf`, `differingFields`.
...
### `src/hooks/useAnchorCoordPaste.ts`
**תפקיד:** הדבקת נ"צ (Ctrl+V או כפתור "הדבק נ\"צ") לטופס עיגון המפה, והמרה לפורמט חה"א. משותף לעיגון בניהול השדות (`ManagementPage`) ולעורך אזורי המפה (`MapZoneEditor`). היעד: כרטיס העוגן שבו הפוקוס (`data-anchor-step`), אחרת העוגן שסומן אחרון על המפה. ההדבקה לא מזיזה את העוגן הפעיל. **מייצא:** `useAnchorCoordPaste`, `DmsFields`.
...
### `src/components/shared/StationScreenFrame.tsx`
**תפקיד:** **מסך של עמדה בתוך קופסה** — `<iframe src="/?peek=<presetId>">` שנפרס בגודל לוגי 1600×900, מוקטן ב-`transform: scale()` לפי גודל הקופסה ("contain", ממורכז) ו-`pointer-events: none` (קריאה בלבד). מידות ידועות מראש (`boxW`/`boxH`) או מדידה בפועל ב-`ResizeObserver` כשאינן ידועות (חלון מוגדל / מסך מלא). שני צרכנים לאותו רכיב — סרגל ההצצה בעמדה ו"מסך לדוגמה" במסך הניהול — ולכן אין שכפול של לוגיקת הרינדור ולא של קנה המידה. **מייצא:** `StationScreenFrame` (default), `FRAME_W`, `FRAME_H`.

### `src/components/admin/StationScreenPreview.tsx`
**תפקיד:** **"הצג מסך לדוגמה"** — המסך האמיתי של העמדה על **כל המסך**, מתוך טופס העמדה במסך הניהול, כדי שמי שמגדיר עמדה יראה מה יצא בלי להתחבר אליה. כותרת: שם העמדה, "קריאה בלבד", ו-Esc/✕ לסגירה. **מה שמוצג הוא מה שנשמר** — המסגרת קוראת את הגדרות העמדה מהשרת, ולכן כשיש בטופס שינויים שטרם נשמרו מוצגת אזהרה בכותרת. מסגרת תורכיז (`frameColor('view')`) כי זה חלון צפייה, z-index 9800 (מעל מודל הגדרות העמדה). **מייצא:** `StationScreenPreview` (default).

### `src/components/shared/SignalBoard.tsx`
**תפקיד:** **לוח ההודעות** — חלון צף (מסגרת תורכיז, חלון צפייה ותפעול) עם כפתורי הודעה שהעמדה מדליקה וכיבה, ומתחתיהם ההודעות הנכנסות מעמדות אחרות, מקובצות לפי עמדת המקור וניתנות לסידור מחדש. גודל החלון נשמר לעמדה (`sigBoardScale_<presetId>`), הגרירה ושינוי הגודל ב-Pointer Events (עט/אצבע).
**חומרת הודעה:** לכל הודעה חומרה — **רגיל** ירוק, **חמור** אדום, **קריטי** אדום מהבהב. כבויה נשארת אפורה בכל חומרה, ונקודת חיווי על הכפתור מראה באיזו חומרה היא תידלק. החומרה נקבעת בעמדה (לחיצה על נקודת החיווי → בורר שלוש אפשרויות), ולהודעות הקבועות גם מראש ב**מאגר ההודעות שבהגדרת העמדה**; הודעה קבועה שנפרסת לכפתור נולדת עם החומרה שהוגדרה לה. הצבעים מ-`src/utils/signalSeverity.ts`.
**נמענים:** לכל הודעה `to_all` או רשימת עמדות, עם דיאלוג חיפוש שמקדים נמענים שכיחים (`sigRecipFreq_<presetId>` ב-localStorage). העמדות **מקובצות תחת בסיס האב** שלהן (`groupRecipientsByBase`): הבסיס של השולח ראשון ופתוח, השאר מכווצים, ולכל בסיס תיבת סימון עם מצב ביניים (`indeterminate`) שמסמנת/מנקה את כל העמדות שתחתיו בבת אחת. הכותרת מציגה `נבחרו/סה"כ` כדי שגם בסיס מכווץ יראה כמה נמענים נבחרו בו. חיפוש פעיל פותח כל קבוצה שיש בה התאמה ותופס גם **שם בסיס** - אחרת התוצאה מסתתרת מאחורי כותרת מכווצת. בסיס אב יחיד → רשימה שטוחה בלי כותרות. סימון בסיס שלם אינו מקדם את מונה השכיחות: הכוכב מסמן נמען שנבחר **אישית** שוב ושוב.
**הודעה ארוכה אינה חורגת:** טקסט הכפתור עובר ב-`FitText` ומוקטן עד שהוא נכנס לתא (רצפה 6px, `fitMin`), במקום לחרוג ממנו ולהישפך על המפה. חיתוך ב-`ellipsis` נפסל — הוא מסתיר מידע תפעולי; הטקסט המלא זמין גם ב-`title`. בכפתורי "הודעות שלי" נשמר `paddingBottom` לשורת אייקוני הנמענים/החומרה.
**גובה הלוח מחולק ב---s:** `maxHeight: calc(78vh / var(--s, 1))`. הלוח יושב תחת `#root { zoom: var(--s) }` והזום מכפיל גם יחידות חלון — ב-24" הגובה המרבי יצא 129% מהמסך והשורות התחתונות עם ידית ה-⇲ נפלו מתחת לקצה (לא נראה ב-15.6", שם `--s=1`).
**מייצא:** `SignalBoard` (default).
...
### `src/components/shared/FitText.tsx`
**תפקיד:** **הקטנת פונט עד שהטקסט נכנס לקופסה** — אח של `FitScaleBox`, אבל ברמת גודל הפונט ועם **גלישת שורות**: הודעה היא משפט שמותר לו לרדת שורה, לא דיאגרמה שחייבת להישאר במידה אחת. נועד ללוח ההודעות, שבו הודעה ארוכה חרגה מהכפתור. **מייצא:** `FitText` (default + named), `fitFontSize`.
**מדידה אימפרטיבית, בלי state:** `fitFontSize` הוא חיפוש בינארי טהור (~4 מדידות לטווח 7-12) שרץ בתוך `useLayoutEffect` וכותב את `fontSize` ישירות ל-DOM — ולכן אין לולאת מדידה→state→רנדר→מדידה שצריך להגן עליה במונים ובאפסילון. החיפוש נבדק ביחידה בלי DOM; המדידה עצמה (`scrollWidth` לרוחב, `offsetHeight` לגובה אחרי הגלישה) אומתה בדפדפן על מידות התא האמיתיות של לוח ההודעות.
**רצפת קריאות:** כשגם הגודל המינימלי אינו נכנס — הרצפה גוברת והטקסט נחתך בגבול הקופסה (`overflow:hidden`), כי טקסט 3px אינו טקסט.
...
### `src/components/map/ZoneCoordList.tsx`
**תפקיד:** **רשימת הנ"צ של אזור** בעורך אזורי המפה - הצגה ועריכה של הקודקודים בפורמט `NDDMM.mmm EDDDMM.mmm`. אזור מגיע לבקר מהפרסום כרשימת נ"צ ולא כציור, ולכן כאן הוא משווה מול המקור, מתקן קודקוד שהונח בעכבר בקירוב, מוסיף קודקוד לפי נ"צ, מעתיק את הרשימה ומדביק רשימה שלמה במקום 12 לחיצות על המפה. אותו רכיב משרת גם אזור חדש בציור וגם אזור שמור בעריכה (`minPoints=3` חוסם ירידה מ-3 קודקודים). בלי כיול גיאוגרפי אין מהיכן לגזור נ"צ - ואז מוצגת **הסיבה** במקום שדות מתים. מכוסה בדיקות (`ZoneCoordList.test.tsx`, 6). **מייצא:** `ZoneCoordList` (גם default), `COORD_SAMPLE`. **שימוש:** `MapZoneEditor`.
...
### `src/components/map/MapDrawLayer.tsx`
**תפקיד:** **סרגל הציור על המפה - רכיב משותף לעמדת המפה (`SectorDashboard`) ולעמדת השדה (`GroundView`)**. אותו כלי, אותה התנהגות ואותו עיצוב בשתי העמדות; מה שמשתנה הוא רק על מה מציירים. שלושה חלקים: `MapDrawToolbar` (סרגל תצוגה בלבד - עט/מחק/עיגול/מלבן, פלטה, עובי, מילוי, נקה/סגור; מקבל `tools` לבחירת הכלים, `toolsExtra` ו-children לתוספות של העמדה), `useMapDrawing` (מנוע ציור לקנבס בודד - מודד את עצמו מול ההורה ב-ResizeObserver ומצייר מחדש מהשברים בכל סיזור) ו-`MapDrawSurface` (הקנבס + שכבת הצורות). כל הגרירה ב-Pointer Events + `touchAction:'none'` + `setPointerCapture` - עט ואצבע ב-Cintiq, לא רק עכבר. מותאם לשלוש התמות ולסקייל גודל המסך (`useToolbarScale`). עמדת המפה מריצה מנוע ציור משלה (הציור שם מסונכרן בין חברי העמדה ומזין את זיהוי הכתב) ומחוברת רק לסרגל, ומזריקה דרך הפתחים את כלי האו"ק, בדיקת MyScript ושיתוף העמדה. **פוליגון סגור/פתוח:** `usePolyDraft` (טיוטת הדקירות - משותף לשני המנועים; Enter מסיים, Backspace מוחק נקודה, Esc מבטל טיוטה בלי לסגור את מצב הציור), `PolyDraftSvg` (הטיוטה עם גומייה וקו סגירה מקווקו) ו-`MapShapeSvg` (רינדור כל סוגי הצורות בשתי העמדות); הסרגל מקבל `polyDraft` לכפתורי סיום / נקודה אחורה, ו-`lineStyle`/`onLineStyleChange` לשורת **סגנון הקו** (מוצגת לכלי צורה בלבד; כל כפתור מצייר את הסגנון עצמו דרך `LineStylePreview` ולא כותב את שמו). **מייצא:** `MapDrawToolbar` (גם default), `MapDrawToggle`, `MapDrawSurface`, `useMapDrawing`, `usePolyDraft`, `PolyDraftSvg`, `MapShapeSvg`, `CrossMarksSvg`, `LineStylePreview`, `SHAPE_TOOLS`, `polySnapTol`, `toolbarColors`, `drawCursor`, `DEFAULT_DRAW_TOOLS`, `MapDrawingEngine`, `ThemeMode`. מכוסה בדיקות (`MapDrawLayer.test.tsx`, 12; ו-`e2e/map-draw.spec.ts`, 8 - ציור בעט/אצבע/עכבר, מיקום נכון תחת זום מפה ומסך 24", ועיגון אחרי שינוי גודל).

### `src/components/map/MapsManager.tsx`
**תפקיד:** ניהול מפות — העלאה (תמונה/PDF, או **הדבקה מהלוח** - Ctrl+V בכל מקום במסך או כפתור "הדבק" למגע, עם תצוגה מקדימה), מחיקה, embed של MapZoneEditor, ו**שיוך מפה לבסיס אב**. כשנפתח מתוך מסך הניהול (props `bases`/`assignableBases`/`allowedBases`) הרשימה מקובצת לפי בסיס אב וראש צוות רואה רק את המפות של המכלולים שלו; בלי הפרופס (מודל עצמאי) ההתנהגות נשארת רשימה שטוחה בלי סינון. **מייצא:** `MapsManager` (default). **שימוש:** admin.
...
### `src/utils/mirageDrivers.ts`
**תפקיד:** ניהול נהגים - הנהג נבחר מהמורשים במיראז' לבסיס. **מייצא:** `normalizeNationalId` (אותו כלל כמו השרת), `driverChoices` (המורשים בלי מי שכבר רשום בשדה; `currentMissing` לנהג רשום שאינו מורשה), `pickDriverError` (הוספה/החלפה חייבות נהג מהרשימה, או `unavailable`; נהג שלא הוחלף נשמר תמיד). **נבדק ב:** `mirageDrivers.test.ts`, `e2e/permits-mirage.spec.ts`.

### `src/components/ground/VehiclePermitsWindow.tsx`
**תפקיד:** **הנהג נבחר מבורר המורשים במיראז' לבסיס** (`/api/auth/mirage-drivers`) - שם ות"ז נגזרים ואינם מוקלדים. **חלון ניהול הנהגים ואישורי הכניסה** בעמדת ניהול שדה תעופה (`ground_mgmt`), נפתח מתת-הפריט "ניהול נהגים" שמתחת ל"כניסת רכבים" בתפריט תצוגה. רשימת נהגים מימין (חיפוש לפי שם / ת"ז / רישוי, תג סטטוס בכל שורה) ופרטי הנהג משמאל: ת"ז, תפקיד הסעה, אישור לאזורים (צ'יפים), תאריכי אישור ופקיעה, **סטטוס אישור שנגזר מהתאריכים וניתן לדריסה ידנית** (והמסך אומר *למה* הוא כזה), מאשר, הערה ותאריך עדכון אחרון. תחתיו רכבי הנהג (סוג, קבוע/לא קבוע, רישוי) וטבלת נסיעות מפוצלת ל**עתידיות** ו**היסטוריה** לפי `scheduled_at` מול השעון. חלון **עריכה** (מסגרת כתומה), נגרר בעט ובאצבע ובר-עגינה לקונטיינר. **מייצא:** `VehiclePermitsWindow` (default), `VehiclePermitsWindowProps`.
...
**תפקיד:** **טבלת נקודת ההצטרפות** - התצוגה שמבדילה אותה מנקודת ההעברה. הפריסה לקוחה מהסדק: עמודת הגבהים בצד ההתחלה והפ"ממים לצדה, מהגבוה למטה. השורה העליונה היא מה שמועבר מנקודת המעבר המקושרת ועוד לא שובץ. **שלוש דרכים לשבץ לבלוק**, כי כל אחת מהירה במצב אחר: "קבל" -> טופס גובה מטווח הנקודה · גרירת הכרטיס מהשורה העליונה לבלוק (קבלה + גובה בתנועה אחת) · גרירת פ"מ שכבר שלי מרשימת הפ"ממים שבצד. קונפליקט מסומן **גם ב-⚠ ולא בצבע בלבד**, והסרת פ"מ מהנקודה מחייבת אישור. **גרירה בטבלה (Pointer Events בלבד):** שורת הפ"מ -> כל המבנה עובר לבלוק; מטוס פרוס -> רק הוא (מבנה של מטוס אחד - כולו; מחוץ לטבלה - אל ההקפה). `expand_aircraft` של הנקודה פורס את המטוסים כברירת מחדל. `+` פורס את מטוסי הפ"מ עם דת"ק, בורר מסלול (רק מסלולים **פעילים לנחיתות**), תפריט מצב שהוא גם הכניסה להקפה ("בנקודת הצטרפות" ברירת מחדל · עה"ר/בסיס/פיינל = בהקפה, נעול בלי מסלול) וסטטוסי ירוקים / אישור לנחות / נחיתה. **מייצא:** `JoiningPointPanel` (default), `JoiningPointView`, `LandingRunway`.

### `src/components/ground/JoiningPointOverlay.tsx`
**תפקיד:** החלון הצף שעוטף את טבלת נקודת ההצטרפות - Portal ל-body, נפתח מעל הסמן או מתחתיו, מוצמד לגבולות המסך ונגרר בכותרת (Pointer Events). **בר-עגינה לקונטיינר החלונות** (`useDockableWindow`, מזהה `joiningPoint:<id>` - משבצת לכל נקודה); במצב מעוגן המדידה מול המסך מושבתת, כדי שהמיקום הצף לא יידרס. **מייצא:** `JoiningPointOverlay` (default).
...
**תפקיד:** **ההקפה התלת מימדית - הסצנה.** SVG יחיד: הקפה, מסלול, בלוקי גבהים, מטוסים ותמונ"א, בהיטל של מצלמה משלה (הטיה/כיוון/זום). **תצוגה נוספת, לא מחליפה** - המבט מלמעלה נשאר מקור האמת למיקום האופקי. ארבעה רמזי עומק אינם קישוט אלא דרישת הנדסת אנוש: צל ההקפה על הקרקע, קווי הורדה אנכיים, רשת קרקע וציר גבהים. מקבלת **יחס** (`aspect`) ולא גבולות מפה, ולכן עולה גם בשדה בלי מפת רקע.
**משטחי גובה (`p3d-alt-shelf`):** לכל תווית על ציר הגבהים מצויר מישור אופקי בגובה
שלה - הדאטום של אותו גובה. זו אינה קישוט אלא **תיקון לקריאות**: בהיטל מוטה גובה
המסך הוא `ry·sin(tilt) − גובה·cos(tilt)`, והאיבר הראשון תלוי במיקום האופקי, ולכן
סרגל אנכי בודד אומר את האמת רק בעומק שבו הוא עומד. המשטח מוזז בדיוק כמו מישור
הקרקע, ולכן **כל** עצם באותו גובה יושב עליו בלי קשר למרחקו לעומק. מצוירים מעל
הקרקע ומתחת לתוכן, כדי שרשת הייחוס לא תעבור מעל סמלי המטוסים. לציר עצמו יש כף
רגל על המישור. ראה REFACTOR_LOG #046.
**מסלול סגור:** אותה מוסכמה של המפה השטוחה - מתאר אדום, מספר כיוון אדום ו-X על כל
האורך (`p3d-runway-closed`). הדגל מגיע ב-`runways[].is_closed`.
**מגמה אנכית:** חץ לבן ליד הסמל (`p3d-trend`), מאותו `store.trends` של המבט מלמעלה.
**מייצא:** `Pattern3DScene` (default), `ThemeMode`.
...
**תפקיד:** מצב התצוגה והגאומטריה **בסשן ולפי עמדה** (`sessionStorage`), ממוזג משלוש שכבות: ברירת מחדל בקוד → ברירת המחדל של העמדה → מה שהפקח שינה. ברירת המחדל היא `overlay` - ההתנהגות ההיסטורית. **מייצא:** `DEFAULT_PATTERN3D_PREFS`, `loadPattern3DPrefs`, `savePattern3DPrefs`, `mergePattern3DPrefs`, `clampWinPos`, `clampWinSize`, `clampSplitRatio`, `nextSplitRatio`, `inlineDelta`, `smallWinInArea`, `SMALL_WIN_MARGIN`, `Pattern3DPrefs`, `Pattern3DMode`.
**`smallWinInArea(area, viewport, s, rtl)`** — הגאומטריה שבה נפתח החלון הקטן: רבע (חצי רוחב × חצי גובה) בפינה התחתונה-**שמאלית** של שטח המפה, כולו בתוכה. הרבע נמדד מול אזור המפה ולא מול החלון — פינת מסך קבועה נחתה חלקית מחוץ למפה או על פאנל השכבות. הקלט בפיקסלים פיזיים, הפלט ביחידות מוגדלות; ב-RTL `x` נמדד מהקצה הימני, כי כך מעוגן `Pattern3DWindow`. "שמאל" כאן הוא **פיזי** ואינו מתהפך עם השפה: המפה היא מרחב, לא זרימת טקסט.
...
**`shouldRenderPattern3D(show3D)`** - **הכלל: כפתור שנדלק חייב להראות משהו.** קודם הרינדור היה מותנה גם ב-`imgBounds`, ובשדה בלי מפת רקע הערך נשאר `null` לצמיתות - הכפתור נצבע בתורכיז ולא עלו לא הסצנה ולא הסרגל (REFACTOR_LOG #045). **`descentSpans(g)` וההנמכה הרציפה** - קודם כל הגובה שבין "עם הרוח" ל"בסיס" אבד על
**שארית צלע העם-הרוח בלבד** (הקטע הקצר שאחרי "מול הסף"), ואז הבסיס נטס **מפולס**:
צניחה חדה, מדף, ועוד צניחה בפיינל - פרופיל שאינו דומה להקפה אמיתית. מעכשיו ההנמכה
מתפרשת על **כל המרחק** ממול-הסף ועד סוף הבסיס בקצב אחיד, ו-`baseAlt` נשמר כעוגן
במשמעות "הגובה שבו מתייצבים על הפיינל". האורכים נלקחים מפרמטרי הגאומטריה (`baseExt`
לשארית העם-הרוח, `width` לבסיס - לפי `patternLegs`) ולכן אינם תלויים ב-`aspect`.
**מייצא:** `DEFAULT_CAMERA`, `TILT_MIN/MAX`, `VERT_SPAN`, `project`, `sceneBounds`, `viewBoxFor`, `zScaleFor`, `patternPath3D`, `descentSpans`, `altOnLeg`, `altProfileOf`, `aglOf`, `clampTilt`, `clampZoom`, `normYaw`, `northArrow`, `shouldRenderPattern3D`, `Camera3D`, `PatternAltProfile`, `Projected`, `Vec3`.
...
### `src/components/views/GroundView.tsx` (6,037 ש')
...
**פאנל השכבות — גובה נגיש:** הפאנל חסום ל**חיתוך של אזור המפה עם החלון** (נמדד ב-effect, בחלוקה ב-`--s` כי `getBoundingClientRect` מחזיר פיקסלים פיזיים) וגליל פנימית, עם `RUNWAY_PANEL_RESERVE` שמור לפאנל "מסלולים בשימוש" שצף מעליו (z=8900, portal ל-body). בלי זה השורות התחתונות שלו — פקדי הזום, כפתור התלת מימד וכפתור הציור — נחתכו מחוץ למסך או נבלעו בקליק. כשהפאנל נגרר מטה הגובה הפנוי נגזר מחדש מהקצה העליון בפועל (`layersAvailH`).
**פאנל השכבות — נגרר ונסגר:** הכותרת היא ידית גרירה מלאת-רוחב (`startPointerDrag` — עט ואצבע, חלוקה ב-`--s`, `setPointerCapture`), והמיקום נשמר כהיסט **בתוך אזור המפה** (`position:absolute` ב-`mapRef`) ולכן `useDragPosition`, שמחזיר קואורדינטות מסך לחלון `position:fixed`, אינו מתאים כאן. הגרירה חסומה ל-`LAYERS_KEEP_VISIBLE` בתוך שטח המפה, ↩ או לחיצה כפולה מחזירים לפינה. ה-✕ שבכותרת סוגר דרך `onCloseLayersPanel`, כלומר מעדכן את **אותו** מתג שבתפריט "תצוגה" — פאנל שנסגר בלי לעדכן אותו היה משאיר שם "✓ מוצג" מול מסך ריק. עד כה הדרך היחידה לפנות את הפינה הייתה לסגור את הפאנל כולו, כלומר לוותר גם על מתגי השכבות, הזום, התלת מימד והציור שבתוכו.
**סרגל לפי סוג העמדה (`hidePatternControls`):** עמדת **ניהול שדה** (`preset_type='ground_mgmt'`) מנהלת רכבים, אלמנטים, נקודות ומצלמות - לא תנועה אווירית. לכן יורדים אצלה מפאנל השכבות **"הצג שמות הקפה"** ומתג ה**תלת מימד** (שהוא מבט על ההקפה), ו**מז"א** יורד בכך ש-SectorDashboard פשוט אינו מעביר את ה-prop `weather` - מה שמוריד בבת אחת את הפקד, את תפריט השכבות **ואת השכבה עצמה**, ולא משאיר שכבה דלוקה בלי מתג. במקביל מוסתר גם פריט "מז"א" מתפריט **תצוגה** (וכך גם בחלון העזרה), אחרת הוא היה נדלק ושום דבר לא היה קורה. **התוויות כבות עם הפקד** דרך `shouldShowPatternLabels`: `showPatternNames` הוא `true` כברירת מחדל, והסתרת הצ'קבוקס לבדה הייתה משאירה את שמות ההקפה על המפה לנצח. מתג התלת מימד בטוח להסתרה כי `show3D` נולד כבוי ורק הכפתור מדליק אותו.
...
**`drawableRunways`:** רשימת המסלולים עם `is_closed`, **אחת לשני המבטים** - המפה השטוחה והסצנה התלת מימדית. קודם כל מבט בנה לעצמו רשימה, והתלת מימד לא קיבל את דגל הסגירה כלל. **הפתיחה תמיד במצב המלא** (`toggle3D` מאפס את המצב ל-`overlay`) — פתיחה היא פעולה חדשה, ולא חזרה לחלונית שנשארה בפינה מוקדם יותר במשמרת; המעבר למצב החלון (`setP3dMode`) ממקם אותו ברבע התחתון-שמאלי של המפה דרך `smallWinInArea`.
...
| `SeizureDrawLayer.tsx` | **שני רכיבים:** שכבת הציור (Pointer Events, `touchAction:'none'` + `setPointerCapture`; נגיעה מוסיפה קודקוד, גרירה מזיזה, נגיעה בראשון סוגרת) יושבת בתוך מכולת המפה, ו-`SeizureDrawToolbar` יושב **מחוץ** לה לצד סרגל כלי המפה - אחרת הוא נצבע מתחתיו. הפוליגון מוחזק בעמדה, ושניהם צופים בו | 🟠 עריכה |
...
| `SeizureLayer.tsx` | שכבת ה-SVG של הפוליגונים על המפה. הקו הוא **קו-נקודה** (`SEIZURE_DASH`) שנבדל מגבול אזור ומאזור סגור; הלחיצה תופסת **רק על הקו** (`pointer-events:stroke`) ועל התווית, כדי שפנים המרחב יישאר יעד שחרור של פ"מ | - |
| `SeizureDetails.tsx` | פרטי ההלאמה - נפתח בלחיצה על קו המרחב. שעון רץ, ולעמדה היוצרת גם "אישורי עמדות" (הדרך חזרה לטופס שנסגר) ו"סיים הלאמה" | 🔵 צפייה |
...
#### joiningPoints.js
DELETE /api/joining-point-strips/:pointId/:stripId
DELETE /api/joining-points/:id
DELETE /api/joining-points/:id/override/:presetId
GET /api/joining-point-strips
GET /api/joining-points
POST /api/joining-point-strips
POST /api/joining-points
PUT /api/joining-point-aircraft/:stripId/:idx
...
PATCH /api/map-zones/:id/operational
PATCH /api/maps/:id
...
PATCH /api/strips/:id/station-note
...
**תפקיד:** אחרי "הפעל נסיעה" נפתח מסך מלא: מפת השדה (ברירת מחדל, עם "עקוב" שמגדיל פי 3 סביב הרכב) או Google Maps (רק כשיש `GOOGLE_MAPS_API_KEY`; בלעדיו המתג כבוי והסיבה כתובה), הנתיב שאושר, האלמנטים שעל הדרך (אדום = סוגר, עם תווית; ירוק = פתוח), מוצא ויעד ונקודת הרכב עם כיוון. `watchPosition` בדיוק גבוה; שידור ל-`/gps` כל 5 ש' כולל **פעימה** של הקריאה האחרונה כל עוד היא טרייה; רענון `/live` כל 5 ש' (רמזור שהתחלף); התרעות מקומיות דרך `TripTracking.findHazards`+`nextAlertState` על אותו חלון התרעה עם צפצוף והקראה; נעילת מסך (`wakeLock`) כדי שה-GPS לא ייעצר בכיס. "לרשימה" ממשיך לעקוב; כרטיס נסיעה שהופעלה מציע "🗺 מפת נסיעה". **נבדק ב:** `server/routes/driverPage.test.js` (כולל קימפול הסקריפט).

### `src/components/ground/TripLiveVehicles.tsx`
**תפקיד:** מעקב נסיעה חי במגדל - הרכב **במיקומו בפועל** על מפת השדה מרגע הפעלת הנסיעה, עם הנתיב שאושר (קו מקווקו בצבע המצב). גוון לפי `liveVehicleTone`: כחול נוסע · סגול ממתין למיקום · אפור אות אבד · אדום סוטה · כתום מול אלמנט סוגר - תמיד עם טקסט. מרכז **העיגול** על המיקום (לא העיגול והתווית יחד), ושכבה לפי חומרה (`TONE_Z`) כדי שרכב מתריע לא יוסתר. בלי עוגן / בלי מיקום - בנקודת המוצא עם הסבר. **מייצא:** `TripLiveLayer` (default, מושך דרך `useLiveTrips`), `TripLiveVehicles` (מציג), `TONE_Z`. **נבדק ב:** `TripLiveVehicles.test.tsx`, `e2e/trip-live-tower.spec.ts`.

### `src/hooks/useLiveTrips.ts`
**תפקיד:** `GET /api/trips/live` בסקר **אחד** לשדה, משותף לשכבת המפה ולהתרעות. `subscribeLiveTrips` רושם עם המנוי הראשון ומסיר רק עם האחרון - `pollingRegistry.register` מחליף משימה באותו מפתח, ושני צרכנים ישירים היו משאירים אחד בלי עדכון. **מייצא:** `useLiveTrips` (default), `subscribeLiveTrips`, `LIVE_TRIPS_POLL_MS`.

### `src/utils/liveTrips.ts`
**תפקיד:** הנגזרת לתצוגה של מעקב חי. `liveTripAlerts` (חסימה לפני סטייה; מפתח החסימה כולל את האלמנט), `pruneDismissedLive` (התרעה שנסגרה חוזרת כשהתנאי חלף וחזר), `liveVehicleTone` (אות אבד גובר). **מייצא:** הנ"ל + `LIVE_TONE_COLOR`, טיפוס `LiveTrip`.

### `shared/elementRelevance.js`
**תפקיד:** למי אלמנט בבסיס רלוונטי - רכבים / מטוסים (`airfield_elements.relevant_for`). **מקור אמת יחיד** לשרת (רשימת האלמנטים לנהג, `planRoute`, `routeElements` במעקב החי), לטופס האלמנט בניהול ולחלון הרכב במגדל. ערך חסר או פגום = שניהם. **מייצא:** `ELEMENT_AUDIENCES`, `DEFAULT_RELEVANT_FOR`, `parseRelevantFor`, `relevantFor`, `isRelevantFor`, `onlyRelevantFor`, טיפוס `ElementAudience`.

### `shared/alertSound.js`
**תפקיד:** צליל התרעה - **מקור אמת יחיד** לאפליקציית הנהג (`/driver/alertSound.js`) ולערימת ההתראות של ניהול הנסיעות במגדל. הקשר אודיו **יחיד** (הקוד הקודם פתח אחד לכל התרעה, עבר את תקרת הדפדפן והשתתק), `unlockAudio()` במגע המשתמש הראשון (בלעדיו ההקשר נולד `suspended` ומנגן בשקט) ו-`audioState()` כדי שהמסך יוכל לומר שהקול חסום. ארבעה טונים: `stop` (4×880Hz), `caution` (2×660Hz), `clear` (סינוס עולה 660→990 - "רמזור ירוק") ו-`notify`. **מייצא:** `TONES`, `ALERT_TONE_BY_KIND`, `toneOf`, `beepSchedule` (טהורה, נבדקת), `unlockAudio`, `audioState`, `playTone`, `speakAlert`, `alertSound`.

### `shared/elementRoadRelevance.js`
**תפקיד:** על איזה **נתיב נסיעה** (`base_routes`) האלמנט שולט, ולפני איזו צומת (`airfield_elements.road_relevance`). ההצהרה של המגדיר **גוברת על הניחוש הגאומטרי** (רדיוס 150 מ' מצומת במתכנן, פרוזדור 40 מ' במפת הנהג) - ורק לאלמנט שהוגדר; בלי הצהרה הכלל הקיים נשאר. **מקור אמת יחיד** ל-`planRoute`, ל-`roadControlElements` (ומשם לנהג, שמקבל את הקובץ ב-`/driver/elementRoadRelevance.js`) ולטופס האלמנט בניהול. **מייצא:** `parseRoadRelevance`, `hasRoadRelevance`, `routeSequenceOf`, `elementAppliesToPath` (`null` = אין הצהרה), `relevanceLabel`, טיפוס `RoadRelevance`.

### `shared/tripTracking.js`
**תפקיד:** מעקב נסיעה חי - **מקור אמת יחיד** לשרת, לאפליקציית הנהג (`/driver/tracking.js`) ולמגדל. גאומטריה (`anchorFrom`, `pctToLatLon`/`latLonToPct`, `metersToSegment`, `metersToPolyline`), כלל "אלמנט סוגר את הדרך" (`effectiveBlockingStatuses`, `isElementBlocking`, `roadControlElements` - כל אלמנטי השליטה עם `on_route`; `compactWaypoints` - מראה של `compactRouteWaypoints`), והתרעות (`findHazards`, `nearOpenElements` - הקרובים ש**אינם** סוגרים, הבסיס להתרעת "נפתח", `nextAlertState` - התרעה בכניסה לאזור + `toClear` כשאלמנט נפתח והנהג עדיין מולו, `nextDeviationStreak`/`isDeviating`, `isFixStale`). הספים (150/100/50/200 מ', 2 קריאות) הם הכרעת אורי 2026-09-13. **מייצא:** כל הנ"ל + `RUNWAY_ALERT_M`, `TAXIWAY_ALERT_M`, `ELEMENT_ALERT_M`, `DEVIATION_M`, `DEVIATION_STREAK`, `ROUTE_CORRIDOR_M`, `MAX_ACCURACY_M`, `STALE_FIX_MS`, `ALERT_COOLDOWN_MS`, `DISPLAY_STATE_LABEL`.
