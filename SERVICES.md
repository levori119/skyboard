...
**כניסת נהג - `POST /api/auth/driver` `{nationalId, password}`:** שואל את המיראז' על אפליקציה **נפרדת** `MIRAGE_DRIVER_APP_NAME` (ברירת מחדל `DRIVER`), עם **הת"ז** (ספרות בלבד) כשם המשתמש. הרשאה ל-SKY-KING אינה הופכת בקר לנהג. מצליח → `{token, expiresInMs, driver:{name, nationalId}}`, והאסימון `{role:'driver', nationalId, name}` תקף 8 שעות. הת"ז שבאסימון (מנורמלת ל-9 ספרות, [`server/auth/driverIdentity.js`](server/auth/driverIdentity.js)) היא הזהות היחידה שממנה נגזרים הנסיעות ובקשות הכניסה של הנהג. פירוש תשובת המיראז' משותף לשתי הכניסות (`sendMirageDenial`): 400 `missing_national_id`/`missing_password`, 401 `bad_credentials`/`password_not_set`, 403 `not_authorized`, 429 `rate_limited`, 502 `mirage_unavailable`, 503 `driver_offline` (עמדה מנותקת). קוד הגישה המשותף `DRIVER_ACCESS_CODE` **בוטל**.
