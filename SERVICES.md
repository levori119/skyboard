...
**`/candidates` מחזירה גם את מצב העמדה:** `active` (יש מקטע משמרת פתוח ב-`station_sessions`) ו-`merged_into_name` (העמדה שמכסה אותה ב-`position_merges` פעיל). בלעדיהם היוצר מפיץ אל מסך חשוך וממתין לאישור שלא יגיע. שני ה-JOIN בטוחים מריבוי שורות - לשניהם אינדקס ייחודי חלקי.
