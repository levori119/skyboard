...
**`GET /driver/logic.js`:** מגיש את [`shared/driverLogic.js`](shared/driverLogic.js) - הלוגיקה הטהורה של אפליקציית הנהג (`greetingFor`, `nearestBase`, `groupDriverTrips`, `sortHistory`, `buildTripRequest`), מודול ES שהדף טוען ונבדק ב-vitest בלי DOM. `public/driver.html`: **כל כניסה לדף מתחילה ב-LOGIN** (האסימון בזיכרון בלבד, לא ב-sessionStorage), שער כניסה מונפש, ברכה, דף בית עם טאבים (מאושרות / ממתינות / היסטוריה), טופס בקשה חדשה, ו"כניסה לשער עכשיו" לזרימה הקיימת.
