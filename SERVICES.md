...
**התלת מימד:** מרונדר לפי `shouldRenderPattern3D(show3D)` בלבד. `imgBounds` **אינו** תנאי: הסצנה צריכה יחס בלבד ו-`boundsAspect(null)` מחזיר 1. ראה REFACTOR_LOG #045. **הפתיחה תמיד במצב המלא** (`toggle3D` מאפס את המצב ל-`overlay`) — פתיחה היא פעולה חדשה, ולא חזרה לחלונית שנשארה בפינה מוקדם יותר במשמרת; המעבר למצב החלון (`setP3dMode`) ממקם אותו ברבע התחתון-שמאלי של המפה דרך `smallWinInArea`.
