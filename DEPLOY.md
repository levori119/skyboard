...
3. **Variables** (Settings → Variables) - הוסף:
   ...
   | `MIRAGE_DRIVER_APP_NAME` | שם אפליקציית הנהג במיראז'. ברירת מחדל `SKY-KING DRIVER`. נהג נכנס עם ת"ז + סיסמה, ורק מי שיש לו הרשאה לאפליקציה הזו ולפחות בסיס אחד |

   > `DRIVER_ACCESS_CODE` **בוטל** - קוד משותף לא זיהה נהג ולכן לא יכול היה לסנן נסיעות. אם הוא עדיין מוגדר בפריסה, אפשר להסיר אותו.
