...
המאגר המקומי הופך את העמדה לעובדת. הסנכרון הוא מה שמחזיר את העבודה לשדה,
ובלעדיו היא נשארת כלואה בעמדה. שלושה חלקים, ושלושתם קיימים:

```
   מראה  ──> המרכז מצלם את מצב השדה → העמדה קולטת (withoutJournal)
   יומן  ──> טריגר על 5 הטבלאות רושם כל שינוי מקומי
   דחיפה ──> איחוד פר-שורה → בדיקת rev במרכז → הוחל / סתירה → הכרעת הבקר
```
...
| [server/sync/apply.js](server/sync/apply.js) | **ההחלה במרכז** - `rev` תואם → הוחל · שונה → סתירה. הסכמה נקבעת בשרת ולא בבקשה |
...
| [src/components/shared/SyncConflictsModal.tsx](src/components/shared/SyncConflictsModal.tsx) | **מסך יישוב הסתירות** - שתי הגרסאות זו מול זו, רק השדות השונים |
...
**שתי הכרעות בלבד, ואין "מיזוג":** פ"מ אינו מסמך טקסט. מיזוג של גובה מגרסה
אחת ועמדה מגרסה שנייה מייצר מצב שאיש משני הצדדים לא בחר בו.
