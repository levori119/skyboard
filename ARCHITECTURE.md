...
#### שני מצבי עלייה, ומה שמסך הכניסה אומר

בכל עליית עמדה השירות מריץ **סיבוב סנכרון מיד**, והתוצאה שלו קובעת את מצב
העלייה - פעם אחת, ואינה משתנה אחריו:

| מצב | מה קרה | מה מסך הכניסה מציג |
|---|---|---|
| `syncing` | הסיבוב הראשון רץ עכשיו | "מסנכרן מהמאגר המרכזי... (n/m)" |
| `synced` | הסיבוב הצליח | "העמדה מסונכרנת עם המאגר המרכזי" (ירוק) |
| `offline` | הסיבוב נכשל - אין קשר | "אין קשר למרכז - העמדה עובדת עצמאית מול המאגר המקומי (n פ"מים)" (כתום) |
| `off` | אין אסימון עמדה | אין חיווי - לא ממציאים הודעה על משהו שלא הותקן |

⚠️ **`GET /api/__localdb/startup` הוא הנתיב היחיד כאן שאינו דורש הזדהות**, וזו
כל התועלת שלו: מסך הכניסה שואל **לפני** שיש משתמש. הוא מחזיר **מצב בלבד**
(אין מידע שדה), קיים רק במאגר המקומי (`localOnly`), והשרת המקומי מאזין
ל-127.0.0.1 בלבד. הרכיב: [src/components/shared/StartupModeBanner.tsx](src/components/shared/StartupModeBanner.tsx).

**למה דווקא במסך הכניסה:** עמדה שעלתה בנתק עובדת מול המאגר המקומי בלבד. זו
אינה תקלה - זה מה שהמאגר המקומי נועד לו - אבל המפעיל חייב לדעת זאת **לפני**
שהוא מתחיל, ולא לגלות זאת באמצע משמרת כשמידע חסר.
...
**המדיניות: העמדה שעבדה בנתק מנצחת** (`POLICY.STATION`, ברירת המחדל מ-2026-09-24),
והמערכת מכריעה לבדה.

**למה לא "האחרון מנצח", שהייתה המדיניות עד אז:** השוואת זמנים מכריעה לפי **מי
כתב מאוחר יותר**, ולא לפי **מי ידע מה קורה בשדה**. בזמן שהעמדה מנותקת, הבקר
שיושב מולה הוא היחיד שרואה את המציאות; עדכון שנעשה במרכז באותו זמן נשען על
תמונה ישנה. בפועל זה גרם לעבודה שנעשתה בנתק "להיעלם" ברגע החיבור מחדש.

⚠️ **ההיקף הוא מה שהופך את זה לבטוח:** נדחפות **רק שורות שהעמדה נגעה בהן
בנתק** - אלה שיושבות ביומן. כל השאר ממשיך להגיע מהמרכז דרך המראה. זו אינה
"עמדה שדורסת את השדה" אלא "עמדה שדורסת את מה שהיא עצמה שינתה".

בפועל פעילה בכל רגע **עמדה אחת מכל סוג**, ולכן פ"מ שנגרר בעמדה מנותקת כמעט
לעולם אינו נגוע בו-זמנית במרכז. מסך שהיה עוצר את הבקר אחרי כל נתק כדי לאשר
את מה שממילא נכון הוא צעד נוסף מול הסדק, בדיוק מה ש-SKY-KING בא למנוע.

`POLICY.LWW` ו-`POLICY.ASK` נשמרו כאפשרויות מפורשות ועדיין נבדקות, כדי
ש"האחרון מנצח" יהיה החלטה שמישהו מקבל ולא קוד שנרקב.
...
| העמדה עדכנה אחרי המרכז | נכתב במרכז. `applied` |
| **המרכז עודכן אחרי העמדה** | **העמדה עדיין מנצחת** - היא זו שראתה את השדה. `applied` |
| **שורה שהעמדה לא נגעה בה** | אינה ביומן ואינה נדחפת כלל - **המרכז נשאר הקובע** |
| השורה נמחקה במרכז | המחיקה מנצחת - שחזור היה מחזיר מטוס רפאים למפה |
| אין חותמת זמן | לא רלוונטי עוד: אין מה להשוות כשההכרעה אינה לפי זמן |

> ב-`POLICY.LWW` המפורש שתי השורות המודגשות מתנהגות כמו קודם (`superseded`
> ו-`conflict`), ויש על כך בדיקות.
