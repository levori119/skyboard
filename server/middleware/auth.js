...
const RULES = [
  ...
  // מסך הכניסה שואל **לפני** שיש משתמש: האם העמדה עלתה מסונכרנת או בנתק.
  // מצב בלבד, בלי שום מידע שדה, וקיים רק במאגר המקומי (localOnly) שמאזין
  // ל-127.0.0.1. המפעיל חייב לדעת זאת לפני שהוא מתחיל לעבוד.
  { m: ['GET'], p: '/api/__localdb/startup', need: NEED.PUBLIC, why: 'מצב עליית העמדה למסך הכניסה - מוגבל למאגר מקומי' },
  ...
  // הקלטה מהדפדפן: העמדה פותחת מקטע ומעלה אליו נתחים. USER ולא ADMIN - זו
  // פעולה של המפעיל על העמדה שלו. היעד אינו בידיו: הוא שולח **מזהה בסיס**,
  // והשרת מפענח את הנתיב מה-DB בעצמו (routes/screenRecording.js).
  { m: ['POST'], p: /^\/api\/screen-recording\/sessions(\/|$)/, need: NEED.USER, why: 'העמדה מעלה את הקלטת המסך שלה; הנתיב נקבע בשרת ולא בבקשה' },
