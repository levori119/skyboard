...
//
// **המדיניות: העמדה שעבדה בנתק מנצחת.** מה שהבקר עשה בעמדה בזמן שהייתה
// מנותקת דורס את המרכז, והמערכת מכריעה **לבדה** ולא עוצרת אותו.
//
// למה לא "האחרון מנצח" (המדיניות הקודמת, הוחלפה 2026-09-24): השוואת זמנים
// מכריעה לפי **מי כתב מאוחר יותר**, ולא לפי **מי ידע מה קורה בשדה**. בזמן
// שהעמדה מנותקת, הבקר שיושב מולה הוא היחיד שרואה את המציאות; עדכון שנעשה
// במרכז באותו זמן נשען על תמונה ישנה. בפועל זה גרם לעבודה שנעשתה בנתק
// "להיעלם" ברגע החיבור מחדש.
//
 ...
 *
 * למה השתנה מ-LWW (2026-09-24, הכרעת אורי): "בחזרה מנתק מה שבוצע בעמדה
 * הספציפית דורס את המאגר המרכזי". השוואת זמנים נשמעת הוגנת, אבל היא מכריעה
 * לפי **מי כתב מאוחר יותר** ולא לפי **מי ידע מה קורה בשדה**. בזמן שהעמדה
 * מנותקת, הבקר שיושב מולה הוא היחיד שרואה את המציאות; עדכון שנעשה במרכז
 * באותו זמן נשען על תמונה ישנה. LWW גרם לכך שעבודה שנעשתה בנתק "נעלמה"
 * ברגע החיבור מחדש - וזה בדיוק מה שדווח.
 *
 * ⚠️ **ההיקף מצומצם מאוד, וזה מה שהופך את זה לבטוח:** רק שורות שהעמדה
 * **נגעה בהן בנתק** נמצאות ביומן ונדחפות בכלל. כל השאר ממשיך להגיע מהמרכז
 * דרך המראה. אין כאן "העמדה דורסת את השדה" אלא "העמדה דורסת את מה שהיא
 * עצמה שינתה".
 */
export const POLICY = {
  /** העמדה מנצחת - ברירת המחדל, והמדיניות התפעולית של SKY-KING */
  STATION: 'station',
  /** האחרון מנצח, לפי חותמות הזמן. נשמר לשימוש מפורש */
  LWW: 'lww',
 ...
 *   `policy` - `station` (ברירת מחדל: העמדה מנצחת), `lww` או `ask`.
...
export async function applyOp(client, op, schema, opts = {}) {
  ...
  const { force = false, policy = POLICY.STATION, skewMs = 0 } = opts;
  ...
  const decide = () => {
    ...
    // העמדה עבדה בנתק על השורה הזו, והיא זו שראתה את השדה. אין מה להשוות.
    if (policy === POLICY.STATION) return 'mine';
