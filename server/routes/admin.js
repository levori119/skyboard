...
// ── יחידות ─────────────────────────────────────────────────────────────────
// רשימת היחידות המבצעיות (יב"א / מגדל / בסיס / טייסת / אחר). **נפרדת מרשימת
// העמדות**: עמדה היא תצורת תצוגה במערכת, יחידה היא גוף בשטח — יש יחידות בלי
// עמדה במערכת.
// הקודים זהים לקודי סוגי המעורבים בתחקיר (INVOLVED_TYPES), כדי שיחידה שהוגדרה
// כאן תופיע ישירות ברשימת הערכים של אותו סוג בטופס התחקיר.
export const UNIT_KINDS = ['yaba', 'tower', 'base', 'squadron', 'other'];
