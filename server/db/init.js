...
async function applySchemaOnce() {
  ...
  // ── הדופק של העמדה ──────────────────────────────────────────────────────
  // מקטע פתוח **אינו** אומר שיושב שם אדם: הוא נסגר רק ב-logout מפורש, ומי
  // שסגר לשונית או כיבה מסך נשאר "מחובר" לנצח. בפרודקשן נמצאו מקטעים פתוחים
  // בני 11 יום. `last_seen` הוא האות החי - העמדה מרעננת אותו כל דקה כל עוד
  // היא על המסך, ומי שקורא מכריע לפי טריות ולא לפי קיום השורה.
  //
  // בלי DEFAULT בכוונה: מקטע ישן שנפתח לפני העדכון יישאר NULL, והקוראים
  // נופלים ל-`entered_at` - כך שמקטע בן דקה נחשב חי גם לפני הדופק הראשון,
  // ומקטע בן יומיים לא.
  await sq(`ALTER TABLE station_sessions ADD COLUMN IF NOT EXISTS last_seen TIMESTAMPTZ`);
