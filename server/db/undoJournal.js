...
export const UNDO_DENYLIST = [
  ...
  ['transfer_takeover_requests', 'הכרעה בתיאום בין שתי עמדות - ביטול שקט מחזיר לתור בקשה ששתיהן כבר סגרו, בלי להחזיר את ההעברה'],
  ...
  ['entry_permit_drivers', 'אישור כניסה לשדה - ביטול שקט עלול להחזיר תוקף לנהג שנפסל'],
  ['entry_permit_driver_zones', 'לאילו אזורים הנהג מורשה - הרחבת הרשאה בשקט'],
  ['entry_permit_trips', 'סטטוס ונתיב נסיעה - ביטול שקט מחזיר לתנועה רכב שהמגדל עצר, או מוחק אישור נהג שכבר ניתן'],
  ['entry_permit_trip_gps', 'קריאת GPS מהטלפון של הנהג - אינה פעולת מפעיל, וביטולה מוחק את מסלול הנסיעה בפועל'],
  ['entry_permit_trip_live', 'המיקום החי של הרכב - ביטול היה מחזיר את הרכב על המפה למקום שכבר עזב'],
  ['entry_permit_trip_templates', 'תבנית שהנהג שמר באפליקציה - אינה פעולת מפעיל, וביטול מהעמדה היה מוחק לנהג תבנית בלי שידע'],
  ...
  ['strip_flow_events', 'FLOW של פ"מ - מה שקרה למטוס קרה, גם אם הפעולה בוטלה אחר כך'],
  ['undo_actions', 'היומן של עצמו'],
  ['undo_journal', 'היומן של עצמו'],
  ['local_sync_journal', 'תור הסנכרון של עבודה מנותקת - ביטול שלו מוחק שינוי שטרם הגיע למרכז, בלי שאיש ידע'],
