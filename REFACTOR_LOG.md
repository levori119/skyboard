...
**הבקשה:** "DB GAPI מעביר ל-DB SKY-KING שמעביר למערכת... שניהם יושבים בנתיב רקיע (רשת סודי ביותר ללא חיבור לאינטרנט)... נתק נוסף אפשרי בין DB SKY-KING למחשב עצמו (כאילו ניתקתי את הכבל רשת). אני רוצה שגם במקרה קיצון זה נמשיך לעבוד על המערכת עם המידע שקיים באותו רגע - כמובן ללא שיתוף מידע בין העמדות. סוג של CASH מקומי."

**נתק GAPI↔DB - כבר עבד.** הבדיקה העלתה ש-`server/gapi/sync.js` (ענף `feature/gapi-integration`) כותב אירוע נכנס **ל-DB של SKY-KING** ולא מעביר אותו לאפליקציה, והאפליקציה קוראת אך ורק מ-DB SKY-KING. בתוספת אידמפוטנטיות (`event_id`+`version`), outbox עם `MAX_ATTEMPTS=12` ו-reconcile תקופתי - זה store-and-forward תקני. **לא נגענו.**
...
**4. כישלון רשת לא מוחק מצב** - תוקנו 6 אתרים: [SectorDashboard](src/components/views/SectorDashboard.tsx) (המראות פעילות, קונפליקטי מסלול, מפת השדה), [DraggablePanels](src/components/transfers/DraggablePanels.tsx) (טווחי גובה של השכן), [ManagementPage](src/components/admin/ManagementPage.tsx), [managers](src/components/admin/managers.tsx). אתר ה-`catch` סביב `JSON.parse` של localStorage ב-`SectorDashboard` **הושאר** - הוא לא רשת.
...
**פליקיות שנפתרה בשורש (ולא בהגדלת timeout):** מבחן הניגודיות נפל אקראית, כל פעם בתמה אחרת. הסיבה: הבאנר מתרנדר מחדש **כל שנייה** (שעון גיל המידע), ו-`getComputedStyle` שנוחת על אלמנט שהוחלף מחזיר מחרוזת ריקה - ומפרסר הצבע זרק. התיקון: `contrastRatio` מחזיר `null` על קלט לא-צבע, והמדידה עטופה ב-`expect.poll` שקורא שוב. 15/15 בשלוש חזרות מלאות, ובכל שלוש התמות נמדד אותו ערך: `rgb(30,41,59)` על `rgb(245,158,11)` = 6.49:1.

**QA:** `tsc` נקי ✅ · vitest **611/611** ✅ · `npm run build` ✅ · **בדיקת עשן על ה-build האמיתי**: שרת העמדה הגיש את `dist` (3.6MB bundle) והמשיך להגיש אותו כשה-upstream נפל, וה-`/api` החזיר 502 תוך **2ms** במקום להיתקע ✅ · **ניגודיות נמדדה ב-3 התמות** (סף 4.5:1) ✅ · **e2e אמיתי** ([e2e/offline-resilience.spec.ts](e2e/offline-resilience.spec.ts), 5/5) - `route.abort('connectionfailed')` מדמה שליפת כבל: הבאנר הופיע עם שעון גיל, המסך **לא התרוקן** אחרי מחזור פולינג, `POST /api/transfers` הוחזר 503 עם `OFFLINE_SHARED_WRITE`, `POST /api/activity-log` הוחזר 202 ונכנס לתור, `GET /api/sectors` הוגש מ-IndexedDB עם `x-skyking-from-cache: 1`, ובחזרת הקשר הופיע "הקשר לשרת חזר" ✅

---

## 2026-08-01 - זליגת תרגול→אמת באזורי מפה, ושרידות ל-failover של ה-DB

שני ממצאים שנפתחו בעבודת העמידות בנתק, סגורים.
...
**TDD:** הבדיקה נכתבה ל-[env-isolation.integration.test.js](server/db/env-isolation.integration.test.js) ו**נצפתה אדומה** (`relation "public.map_zone_operational_state" does not exist`) לפני המימוש. היא יוצרת מפה ואזור **ייעודיים** ולא נוגעת בנתונים אמיתיים - הלקח מ-`map_transfer_points` שנדרסו בעבודה אמיתית.
...
**QA:** `tsc` נקי ✅ · vitest **612/612** ✅ · `npm run build` ✅ · **smoke מול Neon אמיתי** (7 בדיקות): עדכון חלקי משמר את השדה השני, GET מחזיר את שניהם משורגים, **העמודות הישנות נשארו נקיות**, אזור לא קיים → 404, גוף ריק → 400, ומחיקת מפה גוררת CASCADE ✅ · **smoke למוכנות:** בזמן עלייה `health=200` (liveness) מול `ready=503 {reason:"booting"}`, ואחרי העלייה שניהם 200 עם `dbLatencyMs` ✅
...
**QA:** `tsc --noEmit` נקי ✅ · `vite build` נקי ✅ · vitest **520/520** ✅ · e2e `leo-logo.spec.ts` **6/6** ✅ (נוכחות + גובה בטווח + רוחב < 5% מהחלון בשלושת המסכים, וניגודיות ≥ 3:1 בשלוש התמות) · אימות ויזואלי בצילומי מסך של הסרגל בשלוש התמות ושל מסך ההתחברות.
