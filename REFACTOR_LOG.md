...
**QA:** `tsc` נקי ✅ · vitest **645/645** ✅ (11 חדשות ל-`helpTopics` - כולל "כל נושא **וכל כפתור** נגישים באיזשהו סוג עמדה", "כפתור שלא הוגדר לעמדה לא מקבל סעיף" ו"פריט בתוך תפריט נעלם לפי אותו תנאי שמרנדר אותו") · `npm run build` ✅ · אימות API מול ה-DB האמיתי: POST/GET/PATCH/DELETE, ולידציה (400 בלי שדות חובה), סטטוס לא חוקי נדחה, `created_at` timestamptz, הטבלה ב-public בלבד ✅ · e2e חדש [about-suggestions-help.spec.ts](e2e/about-suggestions-help.spec.ts) - 3 בדיקות: הטופס נשמר ל-DB עם חותמת זמן; העזרה ממוספרת ברצף, המונה כולל את סעיפי המשנה, "פתח הכל" חושף בדיוק את כל הכפתורים, החיפוש מוצא גם כפתור בתוך תפריט, וההתאמה לסרגל נבדקת אחד-לאחד; ההצעה מופיעה למנהל ברשימה וניתן לשנות סטטוס ✅ · אימות ויזואלי בשלוש התמות ✅.
...
## 2026-08-02 - תפקידים מקצועיים במיראז', יחידות, זמני עמדה ומסך כ"א ותחקירים

**הבקשות:** "במיראז תוסיף תפקידים - בקר, מש"ק מפעיל לכל איש צוות... לבקר, משגיח בקר, אחורי - מתפריט בקרים. מש"ק ומשגיח מש"קים מתפריט מש"ק. למפעיל ומשגיח מפעיל מתפריט מפעיל. הכל מהמיראז וכמובן הם צריכים להיות מורשים לעמדה." · "בדף תחקיר ביחידות נוספות תוסיף את שמות היחידות מתפריט יחידות שתוסיף במסך ניהול טכני (ולא עמדות)" · "יש למשוך זמן כניסה לעמדה וזמן סיום עמדה... במיראז נוסיף תפקיד חדש 'כח אדם'... כפתור 'כ"א ותחקירים'... 'תחקירים', 'כשירויות'".
...
**QA:** `tsc` נקי ✅ · vitest **634/634** ✅ (כולל שומר ה-i18n, בידוד סכמות מול Neon ו-2 בדיקות חדשות למיראז': positions מוחזר ב-authorize בלי לגעת ב-roles, ותפקיד לא מוכר נזרק) · `npm run build` ✅ · **e2e אמיתי 8/8**: [station-crew](e2e/station-crew.spec.ts) 3/3 - שלושה אנשים במיראז' (בקר/מש"ק/מפעיל) ואימות שכל אחד מופיע **רק** בשדות שלו (העמדה שנבדקה היא יב"א, כלומר גם מסלול המפעיל והמש"ק כוסה); [manpower](e2e/manpower.spec.ts) 2/2 - משמרת נפתחת בכניסה ונסגרת ביציאה **מול ה-API האמיתי**, ותפקיד "כח אדם" פותח את שני המסכים כולל הגרף; [emblems](e2e/emblems.spec.ts) 3/3 ללא רגרסיה.

**מלכודת שנתפסה בהרצה:** שתי בדיקות נכשלו מול **שרת ישן** ש-`reuseExistingServer` שימר מהרצה קודמת - ה-routes החדשים לא היו בו, ו-404 של express חזר כ-HTML. הריגה לפי פורט (3001) ולא לפי שם התהליך פתרה.
